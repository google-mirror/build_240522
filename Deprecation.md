...
* The [android-building@googlegroups.com] mailing list.
...
* `BUILD_PACKAGE` will be switched from `DEFAULT_WARNING_BUILD_MODULE_TYPES` to
  ...
  `BUILD_PACKAGE` into an error unless the device has overriden it.
* At some later point, after all devices no longer set
...
Just to be clear, the above process will happen on the AOSP master branch. So if
you're following Android releases, none of the deprecation steps will be in
Android Q, and the next yearly release will have jumped directly to the end for
many module types.
