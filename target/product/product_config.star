...
def _prodconf(name, parts, **kwargs):
    ...
    for part in parts:
        for name in dir(part):
            variables[name] = __updated_value(variables.get(name), getattr(part, name))
    for name in kwargs:
        variables[name] = __updated_value(variables.get(name), kwargs[name])
    return module(name, **variables)
