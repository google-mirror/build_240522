...
function mm()
(
    _trigger_build "modules-in-dir-no-deps" "$@"
)
...
function _add_build_commands()
{
    local -r build_cmds=("m" "mm" "mmm" "mma" "mmma")
    for bc in "${build_cmds[@]}"; do
      eval "function ${bc}()(_trigger_build \"${bc}\" \"\$@\")"
    done
}
