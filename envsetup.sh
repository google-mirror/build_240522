...
function gdbclient() {
  ...
  if [ -z "$DEVICE" ]; then
    echo "Error: Unable to get device name. Pleace check if device is connected and ANDROID_SERIAL is set."
    return -2
  fi
  ...
  if [[ $CPU_ABI =~ (^|,)arm64 ]]; then
    ...
    GDB=x86_64-linux-androideabi-gdb
# TODO: mips toolchain is not available in PATH for some reason
#  elif [[ $CPU_ABI =~ (^|,)mips64 ]]; then
#  elif [[ $CPU_ABI =~ (^|,)mips ]]; then
  else
    echo "Error: unrecognized cpu abi: $CPU_ABI"
    return -6
  fi
  ...
  if [ $(adb_get_traced_by $PID) -eq 0 ]; then
    ...
    # TODO: check if adb is already listening the $PORT
...
# TODO: there is no art-on, do we need dalvik.gdb?
