...
- allmod:     List all targets.
- gomod:      Go to the directory containing a target.
- refreshmod: Refresh list of targets for allmod/gomod.
...
function addcompletions()
{
    ...
    complete -F _complete-android-target-names gomod
...
function refreshmod() {
    >&2 echo "Refreshing targets (building module-info.json). Log at $ANDROID_PRODUCT_OUT/module-info.json.build.log."
...
function allmod() {
    if [ ! -f "$ANDROID_PRODUCT_OUT/module-info.json" ]; then
        echo "Could not find module-info.json. It will only be built once, and it can be updated with 'refreshmod'"
        refreshmod || return 1
    fi
