...
Usage:
    update_api [targets...]
Examples:
    update_api # update-api is default
    update_api update-api # update all APIs
    update_api ahat-docs-stubs-update-current-api android.car-stubs-docs-update-current-api # update subset of APIs
"
}
...
2. Pass to script as TARGET_PRODUCT=<target_product> update_api [targets...]
"
}

function _get_update_api_script_names() {
  $(gettop)/prebuilts/build-tools/linux-x86/bin/ninja -C $(gettop) -f out/combined-${TARGET_PRODUCT}.ninja -t query $@ |
    sed -n '/input/,/output/{/input/b;/output/b;p}'
}

function update_api() {
  local TARGETS=${@:-update-api}
  ...
  # Run m to generate udpate_api scripts
  m ${TARGETS}
  ...
  done < <(_get_update_api_script_names ${TARGETS})
}
