...
- apkmod:     Gets the location of a module's built APK output.
- installmod: Adb installs a module's built APK.
- refreshmod: Refresh list of modules for allmod/gomod/pathmod/apkmod/installmod.
...
function addcompletions()
{
    ...
    complete -F _complete_android_module_names apkmod
...
# Get the path of a module's apk output in the android tree, as cached in module-info.json. If any build change
# is made, and it should be reflected in the output, you should run 'refreshmod' first.
function apkmod() {
    if [[ $# -ne 1 ]]; then
        echo "usage: installmod <module>" >&2
        return 1
    fi
...
for output in module_info[module]['installed']:
    ...
    if [ -z "$relpath" ]; then
        echo "Could not find module '$1' (try 'refreshmod' if there have been build changes?), or it does not produce an apk." >&2
        return 1
    else
        echo "$ANDROID_BUILD_TOP/$relpath"
    fi
