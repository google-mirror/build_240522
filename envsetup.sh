...
function gdbclient()
{
    ...
    if [ $# -eq 3 ] ; then
        ...
        echo "usage: gdbclient <exe> <port> <pid> or gdbclient <port> <pid>"
        return 1
    fi

    if [ "$PID" ] ; then
        if [[ ! "$PID" =~ ^[0-9]+$ ]] ; then
            local PID_OR_NAME="$PID"
            PID=`pid $PID_OR_NAME`
            if [[ ! "$PID" =~ ^[0-9]+$ ]] ; then
                # that likely didn't work because of returning multiple processes
                # try again, filtering by root processes (don't contain colon)
                PID=`adb shell ps | \grep $PID_OR_NAME | \grep -v ":" | awk '{print $2}'`
                if [[ ! "$PID" =~ ^[0-9]+$ ]]
                then
                    echo "Couldn't resolve '$PID_OR_NAME' to single PID"
                    return 1
                else
                    echo ""
                    echo "WARNING: multiple processes matching '$PID_OR_NAME' observed, using root process"
                    echo ""
                fi
            fi
        fi
        ...
        if [[ -z "$EXE" ]] ; then
            EXE=`adb shell ps | grep $PID | awk '{print $9}' | sed "s#/\(.*\)[\r\n]#\1#g"`
            if [[ -z "$EXE" ]] ; then
                EXE="app_process"
            fi
        fi
