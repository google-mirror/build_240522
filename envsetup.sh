...
# strip the bottom two directories in a path
function getsymbols()
{
    echo $(dirname $(dirname $(get_abs_build_var $1)))
}
...
function is64bit()
{
    ...
    if [ "$PID" ] ; then
        ...
        local OUT_EXE_SYMBOLS=$(getsymbols TARGET_OUT_EXECUTABLES_UNSTRIPPED)
...
function gdbclient()
{
   ...
   if [ "$OUT_ROOT" -a "$PREBUILTS" ]; then
       ...
       echo "OUT_EXE_SYMBOLS/EXE IS $OUT_EXE_SYMBOLS  /  $EXE"
