...
function multitree_lunch_help()
{
    ...
    echo "    Set up android build enviornment based on a specific lunch combo file" 1>&2
...
function multitree_lunch()
{
    ...
    if $(echo "$1" | grep -q '^-') ; then
        ...
        # returns with the lunch_helper.py exit code.
        build/make/orchestrator/core/lunch_helper.py "$@"
        code=$?
        if [[ $code -eq 2 ]] ; then
          echo 1>&2
        ...
        # All other calls go through the --lunch variant of lunch_helper
        results=($(build/make/orchestrator/core/lunch_helper.py --lunch "$@"))
        code=$?
        if [[ $code -eq 2 ]] ; then
          echo 1>&2
          multitree_lunch_help
