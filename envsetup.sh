...
function refreshmod() {
    >&2 echo "Refreshing modules (building module-info.json). Log at $ANDROID_PRODUCT_OUT/module-info.json.build.log."
...
print module_info[module]['path'][0]" 2>/dev/null)
    ...
    if [ -z "$relpath" ]; then
        echo "Could not find module '$1' (try 'refreshmod' if there have been build changes?)."
        return 1
    else
