        ...
        if [ "$PID" ] ; then
            if [[ ! "$PID" =~ ^[0-9]+$ ]] ; then
                ...
                if [[ ! "$PID" =~ ^[0-9]+$ ]] ; then
                    ...
                    PID=`adb shell ps | \grep $PTEMP | \grep -v ":" | awk '{print $2}'`
                    if [[ ! "$PID" =~ ^[0-9]+$ ]]
                    then
                        echo "Couldn't resolve '$PTEMP' to single PID"
                        return 1
                    else
                        echo ""
                        echo "WARNING: multiple processes matching '$PTEMP' observed, using root process"
                        echo ""
                    fi
                fi
            fi
