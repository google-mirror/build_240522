...
class RunToolWithLoggingTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # Copy the envsetup.zip which contians the envsetup.sh and the dependent
    # scripts from 'non-code' resources.
    envsetup_zip = "envsetup.zip"
    dest_file = Path(self.working_dir.name).joinpath(envsetup_zip)
    with resources.as_file(
        resources.files("testdata").joinpath(envsetup_zip)
    ) as p:
      shutil.copy(p, dest_file)

    with zipfile.ZipFile(dest_file, "r") as zip_f:
      zip_f.extractall()
  ...
  def test_log_success_with_logger_enabled(self):
    ...
    expected_logger_args = (
        ...
        ' \d+\.\d+ --tool_params "arg1 arg2" --exit_code 0'
    )
  ...
  def test_log_success_when_tool_interrupted(self):
    ...
    expected_logger_args = (
        "--tool_tag FAKE_TOOL --start_timestamp \d+\.\d+ --end_timestamp"
        ' \d+\.\d+ --tool_params "arg1 arg2" --exit_code 130'
    )
    test_logger.assert_called_once_with_args(expected_logger_args)
