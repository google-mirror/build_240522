...
#   cd /sdx/blueline && ~/aosp/build/setup-android-build.sh aosp_blueline-userdebug
#   cd /sdx/cuttlefish && ~/aosp/build/setup-android-build.sh aosp_cf_arm64_phone-userdebug
# sets up build directories in /sdx/blueline and /sdx/cuttlefish respectively, 
# and after that, issuing
#   /sdx/blueline/run m
# builds blueline image, and
#   /sdx/cuttlefish atest CtsSecurityBulletinHostTestCases
# runs CTS tests. And there is no need to change to a specific directory for
# that.
# 
# Argument:
...
declare -r mydir="${0%/*}"
declare -r source="${mydir%/*}"
[[ "${mydir##*/}" == build ]] || die "$0 should be in build/ subdirectory of the source tree\n"
...
"$source/prebuilts/build-tools/linux-x86/bin/nsjail"\
 ...
 --hostname $(hostname) \
 --\
 /bin/bash -c ". build/envsetup.sh && lunch "$config" &&\$cmd"
EOF
  chmod +x ./run
else
