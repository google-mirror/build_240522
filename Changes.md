...
on a .PHONY target, it can get quite expensive for what should be a tiny build.
...
Those are often marked as PHONY because the Android build system doesn't have
enough dependencies to know when to run the other build system again during an
incremental build.
