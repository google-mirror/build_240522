...
> For general build system changes, see [build/soong/Changes.md]
...
including a majority of our automated build systems, do not do so. This means
that relying on environment variables only set up in envsetup.sh will produce
different output for local users and scripted users.
...
Use OUT_DIR instead, which is equivalent, but may be a relative path. If you
need an absolute path to the output directory, TODO.

### ANDROID_HOST_OUT  {#ANDROID_HOST_OUT}

### ANDROID_PRODUCT_OUT  {#ANDROID_PRODUCT_OUT}

### ANDROID_HOST_OUT_TESTCASES  {#ANDROID_HOST_OUT_TESTCASES}
...
$TOP is hardcoded to), or removed entirely.
...
Depending on the rule, passing the file path itself may not be feasible due
layers of scripts/binaries used in other cases as well, etc. In that case, be
sure to add the dependency, but modify the PATH within the rule itself:
...
[external/fonttools/Lib/fontTools/Android.bp]: https://android.googlesource.com/platform/external/fonttools/Lib/fontTools/Android.bp
