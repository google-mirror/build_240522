...
def VerifyAbOtaPayload(cert, package):
  ...
  def VerifySignatureBlob(hash_file, blob):
    ...
    # In Android, we only support one signature.
  ...
  package_zip = zipfile.ZipFile(package)
  if 'payload.bin' in package_zip.namelist():
    print('Verifying A/B OTA payload signatures...')

  payload_file = package_zip.extract('payload.bin')
  payload = Payload(open(payload_file, 'r'))
  payload.Init()
  ...
  VerifySignatureBlob(payload_sig_file, payload_signature)

  # Metadata signature verification.
  metadata_signature = payload.ReadDataBlob(
      -payload.header.metadata_signature_len,
      payload.header.metadata_signature_len)
  VerifySignatureBlob(metadata_sig_file, metadata_signature)
