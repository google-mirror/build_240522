...
def parseTransferList(package, name):
  print("simulating commands in '{}':".format(name))
  lines = package.read(name).strip().splitlines()
  assert len(lines) >= 4, \
  ...
  for line in lines[4:]:
    ...
    try:
      ...
      raise
  print("")
  print("blocks_written: {}".format(blocks_written))
  print("blocks_stashed: {}".format(my_stash.blocks_stashed))
  print("max_stash_needed: {}".format(my_stash.max_stash_needed))


def printDataInfo(package, partition):
  print("\nReading data info for {}:".format(partition))
  new_data = package.getinfo(partition + ".new.dat")
  patch_data = package.getinfo(partition + ".patch.dat")
  print("{:<30}{:<30}".format(new_data.filename, patch_data.filename))
  print("{:<30}{:<30}".format("compress_type: " + str(new_data.compress_type),
                              "compress_type: " + str(patch_data.compress_type)))
  print("{:<30}{:<30}".format("compress_size: " + str(new_data.compress_size),
                              "compress_size: " + str(patch_data.compress_size)))
  print("{:<30}{:<30}".format("file_size: " + str(new_data.file_size),
                              "file_size: " + str(patch_data.file_size)))


def analyzePartition(package, partition):
  assert partition in ("system", "vendor")
  assert partition + ".new.dat" in package.namelist()
  assert partition + ".patch.dat" in package.namelist()
  assert partition + ".transfer.list" in package.namelist()

  printDataInfo(package, partition)
  parseTransferList(package, partition + ".transfer.list")


def printMetaData(package):
  meta_path = "META-INF/com/android/metadata"
  print("reading meta data for '{}':".format(package.filename))
  meta_info = {}
  for line in package.read(meta_path).strip().splitlines():
    index = line.find("=")
...
def main(argv):
  parser = argparse.ArgumentParser(description='Analyze OTA packages.')
  parser.add_argument('--ota_package=', required=True, dest='ota_package')
  args, unknown = parser.parse_known_args()

  try:
    with zipfile.ZipFile(args.ota_package, 'r') as package:
      printMetaData(package)
      assert "system.new.dat" in package.namelist()
      analyzePartition(package, "system")
      analyzePartition(package, "vendor")
  except:
