...
def LoadOptions(input_zip):
  """
  Load information from input_zip to OPTIONS.

  Args:
    input_zip: path to the input zip file.
  """
  info = OPTIONS.info_dict = common.LoadInfoDict(zipfile.ZipFile(input_zip))
...
def main(argv):
  ...
  input_zip = args[0]
  LoadOptions(input_zip)
  input_tmp = common.UnzipTemp(input_zip, ["IMAGES/*", "OTA/*", "META/*"])
  # Images are likely over limits when they are unsparsed.
