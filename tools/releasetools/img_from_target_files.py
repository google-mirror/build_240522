...
def LoadOptions(input_files):
  ...
  for input_file in input_files:
    info_dicts.append(zipfile.ZipFile(input_file))

  info = OPTIONS.info_dict = common.MergeInfoDictsForReleaseTools(info_dicts)
...
def CopyImages(input_files, output_file, additional_entries):
  """Copies required images from the appropriate input_file archive to the output.

  Args:
    input_file: Path to the input target_files zip file.
    output_file: Path to the output zip file.
    additional_entries: Any user-specified images that should be copied.
  """
  ...
  for input_file in input_files:
    ...
    with zipfile.ZipFile(input_file) as input_zip:
      ...
      info_dict = common.LoadInfoDict(input_zip)
    ...
    for image_path in [name for name in namelist if name.startswith('IMAGES/')]:
      ...
      if image_path in additional_entries:
...
def ImgFromTargetFiles(input_files, output_file):
  ...
  for input_file in input_files:
    ...
    CopyImages(input_files, output_file, OPTIONS.additional_entries)
    ...
    if OPTIONS.build_super and OPTIONS.put_super:
      if OPTIONS.retrofit_dap:
        CopySplitSuperImages(input_file, output_file)
      else:
        rebuild_super = True
