...
def PrepareApexDirectory(inp):
  ...
  1. create {inp}/APEX as a root on the device
  2. copy apexes to {root}/{partition}/apex/*
  3. mount apexes under {root}/apex
  4. generate info files
  ...
  apex_dir = os.path.join(root_dir, 'apex')

  # Always create /apex directory for dirmap
  os.makedirs(apex_dir)

  dump_apex_info = False
  ...
  for device_path, target_files_rel_paths in DIR_SEARCH_PATHS.items():
    for target_files_rel_path in target_files_rel_paths:
      ...
      if os.path.exists(inp_partition):
        ...
        ExtractApexes(apex_dir, apex_dir)
        dump_apex_info = True
