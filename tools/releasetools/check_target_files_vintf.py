...
def GetArgsForSkus(info_dict):
  # Check compatibility for empty SKU and each SKU listed in ODM_MANIFEST_SKUS.
  # The empty SKU check ensures there is no required HALs in the ODM manifest.
...
def CheckVintfFromExtractedTargetFiles(input_tmp, info_dict=None):
  ...
  for sku_args in args_for_skus:
    ...
    if proc.returncode == 0:
      ...
      continue
    if out.strip() == "INCOMPATIBLE":
      logger.info("Command `%s` returns 'incompatible'", ' '.join(command))
      logger.info("stdout: %s", out)
      logger.info("stderr: %s", err)
      success = False
      continue
    logger.error("Command `%s` failed with error code %d", ' '.join(command), proc.returncode)
    logger.info("stdout: %s", out)
