...
Usage: check_target_files_vintf.py target_files
...
def GetArgsForSkus(info_dict):
  skus = shlex.split(info_dict.get("vintf_odm_manifest_skus", "").strip())
  if not skus:
    skus = [""]
...
def GetArgsForShippingApiLevel(info_dict):
  ...
  return [
      "--property", "ro.product.first_api_level=" + shipping_api_level
      ] if shipping_api_level else []
...
def CheckVintfFromExtractedTargetFiles(input_tmp, info_dict=None):
  """
  Check VINTF metadata of an extracted target files directory.
  ...
  for sku_args in args_for_skus:
    ...
    if proc.returncode != 0:
      error_procs.append((command, proc.returncode, out))
    elif OPTIONS.verbose:
      logger.info(out)

  if error_procs:
    for command, returncode, out in error_procs:
      logger.error("Command `%s` failed with code %d:\n%s", command, returncode,
                   out)
    return False
...
def GetVintfFileList():
  """
  Return a list of VINTF metadata files that should be read from a target files
  ...
  proc = common.Run(["checkvintf", "--dump-file-list"])
  out, _ = proc.communicate()
  if proc.returncode != 0:
    raise RuntimeError("checkvintf --dump-file-list failed")
...
def CheckVintfFromTargetFiles(inp, info_dict=None):
  """
  Check VINTF metadata of a target files zip.
...
def CheckVintf(inp, info_dict=None):
  """
  Check VINTF metadata of a target files zip or extracted target files
  directory.
