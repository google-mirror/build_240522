...
class CommonZipTest(test_utils.ReleaseToolsTestCase):
  ...
  def _test_UnzipTemp_createZipFile():
    ...
    with tempfile.NamedTemporaryFile() as entry_file:
      ...
      common.ZipWrite(output_zip, entry_file.name, arcname='Bar5')
      common.ZipClose(output_zip)
    common.ZipClose(output_zip)
    return zip_file
  ...
  def test_UnzipTemp(self):
    ...
    self.assertTrue(os.path.exists(os.path.join(unzipped_dir, 'Bar5')))
  ...
  def test_UnzipTemp_withPatterns(self):
    ...
    self.assertFalse(os.path.exists(os.path.join(unzipped_dir, 'Bar5')))
