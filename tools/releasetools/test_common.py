...
class AlternativeFingerprintTest(test_utils.ReleaseToolsTestCase):
  Condition = common.PartitionBuildProps.Condition
  ...
  def test_parseBuildProps_singleImportStatement(self):
    ...
    self.assertEqual({
        'ro.product.odm.device': {
            ('coral', self.Condition('ro.boot.product.device_name', 'std')),
            ('coralpro', self.Condition('ro.boot.product.device_name', 'pro')),
        },
        'ro.product.odm.name': {
            ('product1', self.Condition('ro.boot.product.device_name', 'std')),
            ('product2', self.Condition('ro.boot.product.device_name', 'pro')),
        },
  ...
  def test_parseBuildProps_multipleImportStatements(self):
    ...
    with zipfile.ZipFile(input_file, 'r') as input_zip:
      placeholder_values = {
          'ro.boot.product.device_name': ['std', 'pro'],
          'ro.boot.product.product_name': ['product1', 'product2'],
    ...
    self.assertEqual({
        ...
        'ro.product.odm.name': {
            ('product1', self.Condition('ro.boot.product.product_name',
                                        'product1')),
            ('product2', self.Condition('ro.boot.product.product_name',
                                        'product2')),
        },
    }, partition_props.prop_overrides)
  ...
  def test_CalculatePossibleFingerprints_no_dynamic_fingerprint(self):
    info_dict = copy.deepcopy(self.TEST_INFO_DICT)
    info_dict['build.prop'].build_props.update({
  ...
  def test_CalculatePossibleFingerprints_no_override(self):
    target_info = common.BuildInfo(self.TEST_INFO_DICT, None)
    self.assertEqual(set(),
                     target_info.CalculateAlternativeFingerprintPrefixes())
  ...
  def test_CalculatePossibleFingerprints_single_override(self):
    info_dict = copy.deepcopy(self.TEST_INFO_DICT)
    info_dict['vendor.build.prop'].prop_overrides = {
        'ro.product.vendor.name': [
            ('product-std', common.PartitionBuildProps.Condition('sku', 'std')),
            ('product-pro', common.PartitionBuildProps.Condition('sku', 'pro')),
        ],
    }
    target_info = common.BuildInfo(info_dict, None)
    self.assertEqual({
        'vendor-product-brand/product-std/vendor-product-device',
        'vendor-product-brand/product-pro/vendor-product-device',
    }, target_info.CalculateAlternativeFingerprintPrefixes())

  def test_CalculatePossibleFingerprints_multiple_overrides(self):
    # Both ro.product.name and ro.product.device are controlled by the
    # property sku; only two combinations are possible.
    info_dict = copy.deepcopy(self.TEST_INFO_DICT)
    info_dict['vendor.build.prop'].prop_overrides = {
        'ro.product.vendor.name': [
            ('product-std', common.PartitionBuildProps.Condition('sku', 'std')),
        ...
        'ro.product.vendor.device': [
            ('device-std', common.PartitionBuildProps.Condition('sku', 'std')),
            ('device-pro', common.PartitionBuildProps.Condition('sku', 'pro')),
        ]
    }
    target_info = common.BuildInfo(info_dict, None)
    self.assertEqual({
        'vendor-product-brand/product-std/device-std',
        'vendor-product-brand/product-pro/device-pro',
    }, target_info.CalculateAlternativeFingerprintPrefixes())

    # Name and device are overridden in different import statements,
    # four combinations are possible.
    info_dict['vendor.build.prop'].prop_overrides = {
        'ro.product.vendor.name': [
            ('product-std', common.PartitionBuildProps.Condition(
                'sku1', 'std')),
            ('product-pro', common.PartitionBuildProps.Condition(
                'sku1', 'pro')),
        ],
        'ro.product.vendor.device': [
            ('device-std', common.PartitionBuildProps.Condition(
                'sku2', 'std')),
            ('device-pro', common.PartitionBuildProps.Condition(
                'sku2', 'pro')),
        ]
    }
    target_info = common.BuildInfo(info_dict, None)
    self.assertEqual({
        'vendor-product-brand/product-std/device-std',
        'vendor-product-brand/product-std/device-pro',
        'vendor-product-brand/product-pro/device-std',
        'vendor-product-brand/product-pro/device-pro',
    }, target_info.CalculateAlternativeFingerprintPrefixes())
  ...
  def test_CalculatePossibleFingerprints_overrides_different_partitions(self):
    ...
    # comes from odm.
    info_dict = copy.deepcopy(self.TEST_INFO_DICT)
    info_dict['odm.build.prop'] = common.PartitionBuildProps.FromDictionary(
        'odm', {
            'ro.product.odm.device': 'odm-product-device'}
    )
    info_dict['odm.build.prop'].prop_overrides = {
        'ro.product.odm.device': [
            ('odm-device-std', common.PartitionBuildProps.Condition(
                'sku', 'std')),
            ('odm-device-pro', common.PartitionBuildProps.Condition(
                'sku', 'pro')),
        ]
    }
    info_dict['vendor.build.prop'].prop_overrides = {
        'ro.product.vendor.name': [
            ('vendor-product-std', common.PartitionBuildProps.Condition(
                'sku', 'std')),
            ('vendor-product-pro', common.PartitionBuildProps.Condition(
                'sku', 'pro')),
        ],
        'ro.product.vendor.device': [
            ('vendor-device-std', common.PartitionBuildProps.Condition(
                'sku', 'std')),
            ('vendor-device-pro', common.PartitionBuildProps.Condition(
                'sku', 'pro')),
        ]
    }
