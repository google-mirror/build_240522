...
def CreateCustomImageSigner(info_dict):
  signer = None
  if (info_dict.get("avb_enable") == "true" and
      info_dict.get("avb_custom_images_key_path") is not None):
    avbtool = info_dict.get("avb_avbtool")
    key_path = info_dict.get("avb_custom_images_key_path")
    algorithm = info_dict.get("avb_custom_images_algorithm")
    extra_args = info_dict.get("avb_custom_images_add_hashtree_footer_args")
    signer = CustomImageSigner(avbtool, key_path, algorithm, extra_args)
...
class CustomImageSigner(object):
  ...
  def __init__(self, avbtool, key_path, algorithm, extra_args):
    self.avbtool = avbtool
    self.key_path = key_path
    self.algorithm = algorithm
  ...
  def SignCustomImage(self, custom_image, partition_name, partition_size):
    """ Sign a custom image."""
    ...
    self.VerifyCustomImage(custom_image, partition_name)
