...
class VerifiedBootVersion1HashtreeInfoGenerator(HashtreeInfoGenerator):
  ...
  def __init__(self, partition_size, block_size, fec_supported):
    ...
    self.verity_image_builder = Version1VerityImageBuilder(
        partition_size, None, fec_supported, None, None, None, None)
...
def GetVeritySize(partition_size, fec_supported):
  ...
  if fec_supported:
    fec_size = GetVerityFECSize(partition_size)
...
def BuildVerityFEC(sparse_image_path, verity_path, verity_fec_path,
                   padding_size):
  cmd = ["fec", "-e", "-p", str(padding_size), sparse_image_path, verity_path,
         verity_fec_path]
  output, exit_code = RunCommand(cmd)
  if exit_code != 0:
    raise BuildVerityImageError(
...
def BuildVerityTree(sparse_image_path, verity_image_path):
  ...
  root_hash, salt = output.split()
  return root_hash, salt
...
class VerityImageBuilder(object):
  ...
  def SetDynamicPartitionSize(self, size):
    """Sets the size for a dynamic partition to the given value."""
    raise NotImplementedError
  ...
  def Create(prop_dict):
    """Returns a builder based on the given build properties.

    Args:
      prop_dict: A dict that contains the build properties. In particular, it
          will look for verity-related property values.

    Returns:
      A VerityImageBuilder instance for Verified Boot 1.0 or Verified Boot
          2.0; or None if the given build doesn't support Verified Boot.
    """
    ...
    if (prop_dict.get("avb_hash_enable") == "true" or
      ...
      if prop_dict.get("avb_hash_enable") == "true":
        return Version2VerityImageBuilder(
            prop_dict["partition_name"],
            partition_size,
            Version2VerityImageBuilder.AVB_HASH_FOOTER,
...
class Version1VerityImageBuilder(VerityImageBuilder):
  ...
  def Build(self, out_file):
    ...
    Append2Simg(
        out_file, verity_image_path, "Could not append verity data!")
...
class Version2VerityImageBuilder(VerityImageBuilder):
  ...
  def AVBCalcMinPartitionSize(self, image_size, size_calculator=None):
    """TODO: Calculates min partition size for a given image size.
  ...
  def SetDynamicPartitionSize(self, size):
    self.partition_size = self.AVBCalcMinPartitionSize(size)
