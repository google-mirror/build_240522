...
class VerityTreeInfo(object):
  ...
  def __init__(self, partition_name, tgt, info_dict):
    verity_partition = (info_dict.get("verity") == "true" and
  ...
  def GetVerityDataSize(self):
    ...
    result, meta_size = GetVerityMetadataSize(adjusted_size)
    if not result:
      return None
    assert meta_size % self.block_size == 0

    self.verity_tree_range = RangeSet(data=[adjusted_size / self.block_size,
        (adjusted_size + verity_tree_size) / self.block_size])
    self.file_system_range = RangeSet(data=[0, adjusted_size / self.block_size])

    return adjusted_size, verity_tree_size, meta_size

  def ParseVerityMetadata(self, meta_range, file_system_size):
  ...
  def GetVerityTreeInfo(self):
    ...
    self.ParseVerityMetadata(meta_range, file_system_size)
