...
def WriteBlockBasedOTAPackage(target_file, output_file, source_file):
  ...
  # -s / --device_specific, use that. Otherwise, use META/releasetools.py if
  # it is present in the target target_files. Otherwise, take the path of the
  # file from 'tool_extensions' in the info dict and look for that in the
  # local filesystem, relative to the current directory.
...
class AbOtaPackage(OtaPackage):
  ...
  def Generate(self, target_file, source_file=None, include_secondary=False):
  ...
  def Finalize(self):
    ...
    if self.secondary_payload is not None:
      self.secondary_payload.Sign(payload_signer)
      self.secondary_payload.WriteToZip(output_zip)
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  package.Generate(
      target_file, source_file, include_secondary=OPTIONS.include_secondary)
  package.Finalize()
