...
class PropertyFiles(object):
  ...
  Computing the final property-files string requires multiple passes. Because
  signapk.jar will possibly reorder the ZIP entries during each whole package
  signing, which may in turn invalidate earlier computed ZIP entry offset/size
  pairs.

  This class provides the helper functions to be called for each pass. The
  ...
  def Compute(self, input_zip):
    ...
    We reserve extra space for the offset and length of the metadata entry
    itself, although we don't know the final values until the package gets
    signed.
    ...
    Returns:
      A string with placeholders for metadata part, e.g.
  ...
  def Finalize(self, input_zip, reserved_length):
    ...
    info. Note that during this process, we must pad the property-files string
    ...
    Returns:
      A string with the metadata info, e.g.
