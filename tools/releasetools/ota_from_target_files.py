...
def GetTargetFilesZipForSecondaryImages(input_file, skip_postinstall=False):
  ...
  for info in input_zip.infolist():
    ...
    # Skip copying postinstall if requested.
...
def GetTargetFilesZipWithoutPostinstallConfig(input_file):
  ...
  This allows update_engine to skip all the postinstall hooks in the generated
  payload. The input target-files.zip file will be duplicated, with
  'META/postinstall_config.txt' skipped. If input_file doesn't contain the
  postinstall_config.txt entry, the same file will be returned.

  Args:
    input_file: The input target-files.zip filename.

  Returns:
    The filename of target-files.zip that doesn't contain
    META/postinstall_config.txt entry.
  """
