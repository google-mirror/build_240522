...
class BuildInfo(object):
  ...
  This class wraps up the build property querying, which abstracts away the
  logic of handling OEM properties (and caches the commonly used properties such
  as fingerprint).

  It takes the given info dict (which comes from a target_files zip), and a list
  of OEM dicts (which comes from ota_from_target_files.py command line argument
  '--oem_settings') as inputs. It then answers queries that could be affected in
  the presence of OEM dict, such as fingerprint, device name etc. Users can
  query other properties by calling GetBuildProp() or GetVendorBuildProp().

  Attributes:
    info_dict: The information dict for the given build.
    ...
    oem_props: A list of OEM properties that should be read from OEM dicts. None
        if the build doesn't use any OEM property.
    fingerprint: The fingerprint of the build, which could be calculated based
        on OEM properties if applicable.
    device: The device name, which could be loaded from OEM dicts if applicable.
  """
  ...
  def __init__(self, info_dict, oem_dicts):
    ...
    Arguments:
      info_dict: The base information dict for the given build.
      oem_dicts: A list of OEM dicts (should be parsed from --oem_settings).
          Note that it mostly uses the first dict to answer queries. The rest
          would be used in
    """
  ...
  def WriteDeviceAssertion(self, script):
...
def WriteFullOTAPackage(input_zip, output_zip):
  ...
  # We don't know what version it will be installed on top of. We expect the
  # API just won't change very often. Similarly for fstab, it might have changed
  # in the target build.
  target_version = target_info["recovery_api_version"]
  script = edify_generator.EdifyGenerator(target_version, target_info)
  ...
  target_info.WriteDeviceAssertion(script)
...
def WriteBlockIncrementalOTAPackage(target_zip, source_zip, output_zip):
  ...
  target_version = target_info["recovery_api_version"]
  source_version = source_info["recovery_api_version"]
  if source_version == 0:
    print("WARNING: generating edify script for a source that "
          "can't install it.")

  script = edify_generator.EdifyGenerator(
      source_version, target_info, fstab=source_info["fstab"])
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  metadata = {
      ...
      "post-build-incremental" : target_info.GetBuildProp(
          "ro.build.version.incremental"),
      "pre-device" : source_info.GetOemProperty("ro.product.device"),
  ...
  # source_info must be None for incrementals.
