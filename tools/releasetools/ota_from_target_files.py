...
Usage:  ota_from_target_files [flags] input_target_files output_ota_package
  ...
  --include_secondary
      ...
      Such a package needs to be applied in a two-stage manner. During the first
      stage, the updater applies the primary payload only. Upon finishing, it
      reboots the device into the newly updated slot, then proceeds to install
      the secondary payload to the inactive slot, but without switching the
      active slot (needs the matching support in update_engine, for
      SWITCH_SLOT_ON_REBOOT flag).
...
class Payload(object):
  ...
  def WriteToZip(self, output_zip, secondary=False):
    ...
    Args:
      ...
      secondary: Whether this should be saved as secondary payload (default:
          False).
    """
...
def GetTargetFilesZipForSecondaryImages(input_file):
  ...
  ones without _other suffix. Note that we cannot modify the names in
  ...
  Args:
    input_file: The input target-files.zip file for the build.
  ...
  for info in input_zip.infolist():
    ...
    elif info.filename.startswith("IMAGES/"):
      common.ZipWrite(target_zip, unzipped_file, arcname=info.filename)
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  if OPTIONS.include_secondary:
    ...
    # building incremental OTAs. See the comments for "--include_secondary".
