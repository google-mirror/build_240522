...
class PropertyFiles(object):
  ...
  def Finalize(self, input_zip, reserved_length):
    ...
    Raises:
      InsufficientSpaceException: If the reserved length is insufficient
...
def FinalizeMetadata(metadata, input_file, output_file, needed_property_files):
  ...
  try:
    ...
    # Even with the prelimniary signing, the entry orders may change
    # dramatically, which leads to insufficient reserved space during the first
    # call to ComputeAllPropertyFiles(). In that case, we redo all the prelim
    # signing works, based on the already ordered ZIP entries, to solve the
    # issue.
