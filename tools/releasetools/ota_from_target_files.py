...
def main(argv):
  ...
  # directory. We don't need to unzip the entire target-files zips, because it
  # won't be needed for generating A/B OTAs.
  # When loading the info dicts, we don't need to provide the second parameter
  # to common.LoadInfoDict(). Because specifying the second parameter allows
  # expanding some properties to their actual paths, such as 'selinux_fc',
  # 'ramdisk_dir', they are not needed during OTA generation.
