...
Common options that apply to both of non-A/B and A/B OTAs
  ...
  --downgrade
      ...
      to an older one (based on timestamp comparison). In addition to
      "post-timestamp", "ota-downgrade=yes" will be set in the metadata file. A
      data wipe will always be enforced, so "ota-wipe=yes" will also be included
      in the metadata file. The update-binary in the source build will be used
      in the OTA package, unless --binary flag is specified. Please also check
      the comment for --override_timestamp below.
  ...
  --override_timestamp
      ...
      know for sure this is NOT an actual downgrade case, but two builds happen
      to be cut in a reverse order (e.g. from two branches). A legit use case is
      that we cut a new build C (after having A and B), but want to enfore an
      update path of A -> C -> B. Specifying --downgrade may not help since that
      would enforce a data wipe for C -> B update. With this flag enabled,
      package metadata will have "ota-downgrade=yes" in addition to the
      "post-timestamp".
...
def GetPackageMetadata(target_info, source_info=None):
  ...
  # Detect downgrades and fill in the post-timestamp.
  if is_incremental:
