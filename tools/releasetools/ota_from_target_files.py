...
class PayloadSigner(object):
  ...
  def __init__(self):
    if OPTIONS.payload_signer is None:
      ...
      self.key_size = self._GetKeySizeInBytes(private_key)
    else:
  ...
  def _GetKeySizeInBytes(self, private_key):
    modulus_file = common.MakeTempFile(prefix="modulus-")
    cmd = ["openssl", "rsa", "--inform", "DER", "-in", private_key, "-modulus",
    ...
    modulus_string = modulus_string[len(modulus_prefix):].strip()
    key_size = len(modulus_string) / 2
    assert key_size == 256 or key_size == 512, "keysize {}".format(key_size)
    return key_size
