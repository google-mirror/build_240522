  ...
  if device_specific_diffs:
    ...
    for diff in device_specific_diffs:
      if diff.partition in block_diff_dict:
        logger.warning("Duplicate block diff found. Device specific update for"
                       " partition '%s' overrides the one in generic script.",
                       diff.partition)
...
""" % bcb_dev)
  ...
  required_cache_size = [diff.required_cache for diff in
