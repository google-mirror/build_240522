...
class AbPackagePropertyFiles(StreamingPropertyFiles):
  """The property-files that additionally includes payload_metadata.bin info.

  Since P, we additionally expose one more token (aka property-file) for a
  virtual entry of 'payload_metadata.bin'. The entry doesn't exist in the A/B
  OTA package, but TODO.
  """
  ...
  def _GetPayloadMetadataOffsetAndSize(input_zip):
    ...
    Update file format: A delta update file contains all the deltas needed to
    update a system from one specific version to another specific version. The
    update format is represented by this struct pseudocode:
