...
class Payload(object):
  ...
  @classmethod
  def FromTargetFiles(cls, target_file, source_file=None):
    """Generates a payload from the given target-files zips.
    ...
    p = common.Run(cmd, stdout=log_file, stderr=subprocess.STDOUT)
    stdoutdata, _ = p.communicate()
    assert p.returncode == 0, \
        "brillo_update_payload generate failed: {}".format(stdoutdata)
    return cls(payload_file)

  def SignPayload(self, payload_signer):
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  # Sign the payload.
  payload.SignPayload(PayloadSigner())
