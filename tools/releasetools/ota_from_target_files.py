...
A/B OTA specific options
  ...
  --payload_signer_maximum_signature_size <signature_size>
      The maximum possible signature size in bytes of the given payload signer.
...
class PayloadSigner(object):
  ...
  def __init__(self):
    if OPTIONS.payload_signer is None:
      ...
      self.maximum_signature_size = self._GetSignatureSizeInBytes(signing_key)
    else:
  ...
  def _GetSignatureSizeInBytes(signing_key):
    ...
    logger.info("{} returns the maximum signature size: {}".format(
        cmd[0], signature_size))
