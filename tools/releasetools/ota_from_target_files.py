...
def GetPackageMetadata(target_info, source_info=None):
  ...
  post_build_devices, post_build_fingerprint = \
      CalculateRuntimeDevicesAndFingerprints(target_info, boot_variable_values)
  metadata = {
      'post-build': separator.join(sorted(post_build_fingerprint)),
  ...
  if is_incremental:
    pre_build_devices, pre_build_fingerprint = \
        CalculateRuntimeDevicesAndFingerprints(source_info,
                                               boot_variable_values)
    metadata['pre-build'] = separator.join(sorted(pre_build_fingerprint))
