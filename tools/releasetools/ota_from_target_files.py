...
Common options that apply to both of non-A/B and A/B OTAs
  ...
  --retrofit_dynamic_partitions
      ...
      dynamic partitions (default False). This flag is implied when generating
...
def GetTargetFilesZipForRetrofitDynamicPartitions(input_file,
  ...
  """
  if not super_block_devices:
    return input_file

  replace = {'IMAGES/super_{}.img'.format(dev): 'IMAGES/{}.img'.format(dev)
  ...
  for src, dst in replace.items():
    if src not in namelist:
      logger.warning(
          'Missing {} in {}; {} is not written'.format(src, input_file, dst))
      continue
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  if OPTIONS.retrofit_dynamic_partitions:
    target_file = GetTargetFilesZipForRetrofitDynamicPartitions(target_file,
