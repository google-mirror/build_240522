...
def main(argv):
  ...
  def option_handler(o, a):
    ...
    elif o == "--spl-downgrade" or o == "--spl_downgrade":
  ...
  if not OPTIONS.no_signing or generate_ab:
    ...
    if not os.path.exists(private_key_path):
      logger.fatal("Private key %s doesn't exist. Make sure you passed the "
                   "correct key path through -k option", private_key_path)
      sys.exit(1)
  ...
  if OPTIONS.source_info_dict:
    ...
    if target_spl < source_spl and not OPTIONS.spl_downgrade:
      logger.fatal(
        "Target security patch level %s is older than source SPL %s applying "
        "such OTA will likely cause device fail to boot. Pass --spl-downgrade "
        "to override this check.", target_spl, source_spl)
      sys.exit(1)
  if generate_ab:
