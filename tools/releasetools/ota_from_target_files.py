...
def GetTargetFilesZipForPartialUpdates(input_file, ab_partitions):
    ...
    original_ab_partitions = common.ReadFromInputFile(
        input_file, AB_PARTITIONS)
...
def ValidateCompressionParam(target_info):
    ...
    if vabc_compression_param:
        minimum_api_level_required = VABC_COMPRESSION_PARAM_SUPPORT[vabc_compression_param.split(",")[
            0]]
...
def GenerateAbOtaPackage(target_file, output_file, source_file=None):
    ...
    if OPTIONS.disable_vabc and target_info.is_release_key:
        raise ValueError(
            "Disabling VABC on release-key builds is not supported.")
    ...
    if source_file is not None:
        ...
        if not source_info.is_vabc or not target_info.is_vabc:
            logger.info(
                "Either source or target does not support VABC, disabling.")
    ...
    if OPTIONS.vabc_cow_version:
        target_file = ModifyTargetFilesDynamicPartitionInfo(
            target_file, "virtual_ab_cow_version", OPTIONS.vabc_cow_version)
    if OPTIONS.compression_factor:
        target_file = ModifyTargetFilesDynamicPartitionInfo(
            target_file, "virtual_ab_compression_factor", OPTIONS.compression_factor)
    ...
    if source_file and OPTIONS.enable_lz4diff:
        ...
        additional_args += ["--erofs_compression_param",
                            erofs_compression_param]
    ...
    def option_handler(o, a):
        if o in ("-i", "--incremental_from"):
            ...
            if a[:-1].isdigit() and a in values and a.endswith("k"):
                ...
                raise ValueError(
                    "Please specify value from following options: 4k, 8k, 16k, 32k, 64k, 128k, 256k")
