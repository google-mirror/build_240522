...
def ProcessTargetFiles(input_tf_zip, output_tf_zip, misc_info,
  ...
  for info in input_tf_zip.infolist():
    ...
    # All images are eventually regenerated by add_img_to_target_files.
    # However, if using vendor_otatools:
    #   - non-vendor images are skipped now and regenerated by this package.
    #   - vendor images are copied verbatim now (so that the initial call to add_img_to_target_files
    #     does not attempt to regenerate them), then they are deleted and regenerated using
    #     add_img_to_target_files within the vendor otatools package.
    if filename.startswith("IMAGES/"):
      partition = filename.replace("IMAGES/", "").replace(".img", "")
      if not (OPTIONS.vendor_partitions and \
              OPTIONS.vendor_otatools and \
              partition in OPTIONS.vendor_partitions):
        continue
...
def main(argv):
  ...
  # Delete the stale vendor partitions, then rebuild them using vendor_otatools.
  ...
  if OPTIONS.vendor_partitions and OPTIONS.vendor_otatools:
    images_to_delete = [
        "IMAGES/{}.img".format(p) for p in OPTIONS.vendor_partitions
    ]
    common.ZipDelete(args[1], images_to_delete)
    cmd = [
