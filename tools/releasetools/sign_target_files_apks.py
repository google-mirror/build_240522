  ...
  for info in input_tf_zip.infolist():
    ...
    # Sign microdroid_vendor.img.
      ...
      with tempfile.NamedTemporaryFile() as image:
        image.write(data)
        image.flush()
        ReplaceKeyInAvbHashtreeFooter(image, vendor_key, vendor_algorithm, misc_info)
...
def ReplaceKeyInAvbHashtreeFooter(image, new_key, new_algorithm, misc_info):
  ...
  def GetAvbInfo(avbtool, image_name):
    ...
    for line_info_raw in info_raw.split('\n'):
      ...
      if value == "":
        ...
        if not prop_parsed:
          raise ValueError("Failed to parse prop while getting avb information.")
  ...
  def GetAvbHashtreeDescriptor(avb_info):
    hashtree_descriptors = tuple(filter(lambda x: "Hashtree descriptor" in x, info.get('Descriptors')))
  ...
  props = map(lambda x: x.get("Prop"), filter(lambda x: "Prop" in x, info.get('Descriptors')))
