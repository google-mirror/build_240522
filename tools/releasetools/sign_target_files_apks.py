...
def ReadApexKeysInfo(tf_zip):
  ...
  for line in tf_zip.read('META/apexkeys.txt').decode().split('\n'):
    ...
    matches = re.match(
        ...
        r'sepolicy_key="(?P<SEPOLICY_KEY>.*?)"'
        r'sepolicy_certificate="(?P<SEPOLICY_CERT>.*?)"'
        r'fsverity_tool="(?P<FSVERITY_TOOL>.*?)"'
    ...
    if name.contains(OPTIONS.sepolicy_name):
      ...
      # keys[name] = (payload_private_key, container_key, sign_tool, sepolicy_key, sepolicy_cert, fsverity_tool)
