...
class AddImageToTargetFilesTest(unittest.TestCase):
  ...
  def test_AddRadioImagesForAbOta_ImageExists(self):
    ...
    with open(boot_image_path, 'wb') as boot_image_file:
      boot_image_file.write(b'boot')
    system_image_path = os.path.join(image_path, 'system.img')
    with open(system_image_path, 'wb') as system_image_file:
      system_image_file.write(b'system')
