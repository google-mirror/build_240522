...
class AddImagesToTargetFilesTest(unittest.TestCase):
  ...
  def test_AddCareMapForAbOta(self):
    ...
    care_map_file = os.path.join(OPTIONS.input_tmp, 'META', 'care_map.proto')
  ...
  def test_AddCareMapForAbOta_verityNotEnabled(self):
    """No care_map.proto should be generated if verity not enabled."""
    image_paths = self._test_AddCareMapForAbOta()
    OPTIONS.info_dict = {}
    AddCareMapForAbOta(None, ['system', 'vendor'], image_paths)
  ...
  def test_AddCareMapForAbOta_zipOutput(self):
    ...
    care_map_name = "META/care_map.proto"
  ...
  def test_AddCareMapForAbOta_zipOutput_careMapEntryExists(self):
    ...
    output_file = common.MakeTempFile(suffix='.zip')
    with zipfile.ZipFile(output_file, 'w') as output_zip:
      # Create an existing META/care_map.proto entry.
      common.ZipWriteStr(output_zip, 'META/care_map.proto',
                         'dummy care_map.proto')

      # Request to add META/care_map.proto again.
      AddCareMapForAbOta(output_zip, ['system', 'vendor'], image_paths)
    ...
    self.assertIn('META/care_map.proto', OPTIONS.replace_updated_files_list)
