...
class SparseImage(object):
  ...
  Wraps a sparse image file (and optional file map and copy_blocks) into an
  image object suitable for passing to BlockImageDiff. file_map contains
  the mapping between files and their blocks. copy_blocks contains the set
  of blocks that should be always written to the target regardless of the
  old contents (i.e. copying instead of patching). copy_blocks should be in
  the form of string like "0" or "0 1-5 8".
  """

  def __init__(self, simg_fn, file_map_fn=None, copy_blocks=None):
    ...
    self.copy_blocks = rangelib.RangeSet(data=copy_blocks)
    ...
    if file_map_fn:
      self.LoadFileBlockMap(file_map_fn, self.copy_blocks)
    else:
  ...
  def TotalSha1(self):
    """Return the SHA-1 hash of all data in the 'care' regions but not in copy_blocks
    of this image."""
    h = sha1()
    for d in self._GetRangeData(self.care_map.subtract(self.copy_blocks)):
      h.update(d)
    return h.hexdigest()
  ...
  def LoadFileBlockMap(self, fn, copy_blocks):
    ...
    with open(fn) as f:
      for line in f:
        ...
        # Currently we assume that blocks in copy_blocks are not part of
        # any file.
        assert not copy_blocks.overlaps(ranges)
        remaining = remaining.subtract(ranges)

    remaining = remaining.subtract(copy_blocks)

    # For all the remaining blocks in the care_map (ie, those that
    # aren't part of the data for any file nor part of the copy_blocks),
