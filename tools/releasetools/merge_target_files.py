...
def files_from_path(target_path, extra_args=None):
  ...
  find_command = ['find', target_path] + extra_args
...
def create_merged_package(temp_dir, framework_target_files, framework_item_list,
                          ...
                          framework_misc_info_keys,
                          rebuild_recovery):
  """Merge two target files package into one target files structure
...
def generate_images(output_tmp_dir, rebuild_recovery):
  """ Generate Images from given merged artifacts

  This function takes merged output temporary directory and create images
  from it.

  Args:
    output_tmp_dir: Path to merged temp directory
    rebuild_recovery: If true, rebuild the recovery patch used by non-A/B
      devices and write it to the system image.
  """
  ...
  add_img_args.append(output_tmp_dir)

  add_img_to_target_files.main(add_img_args)


def generate_super_image(target_dir, output_super_empty):
  """ Generate super image from target package

  Args:
    target_dir: Path to the target artifact which contains misc_info.txt for
...
def create_img_archive(source_path, target_path):
  """ Create IMG archive from merged package

  Args:
    source_path: Path of the artifact to be packed
    target_path: Create IMG package from the artifact in source_path
  """
...
def create_output_archive(output_target_files, output_target_files_list,
                          output_dir):
  """ Create archive from merged artifact
  ...
  output_target_files_meta_dir = os.path.join(output_dir,
                                              'META')

  meta_content = files_from_path(output_target_files_meta_dir)
  other_content = files_from_path(output_dir,
...
def merge_target_files(temp_dir, framework_target_files, framework_item_list,
  ...
  generate_super_image(output_target_files_temp_dir, output_super_empty)
  ...
  output_zip = create_output_archive(output_target_files,
                                     os.path.join(temp_dir, 'output.list'),
                                     output_target_files_temp_dir)
