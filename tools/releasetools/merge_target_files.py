...
PARTITION_TAG_PATTERN = re.compile(r'partition="_?(.*)"')
...
def extract_partition_tag_from_item(pattern, line, default):
  """Extracts the partition tag (if any) from line.

  Uses pattern to search line for a partition tag. The pattern is expected to be
  structured such that group 1 captures the partition tag. If the line matches,
  lowercases the match, and returns that. Otherwise, returns default.

  Args:
    pattern: A regex used to match and capture (using group 1) a partition tag
      from line.
    line: A string that may contain a partition tag that might be matched by
      pattern.
    default: What to return if line doesn't match the pattern.
  Returns
    The extracted partition tag, lowercased.
  """
...
def item_list_to_partition_set(item_list):
  ...
  for item in item_list:
    partition_tag = extract_partition_tag_from_item(
        PARTITION_ITEM_PATTERN, item.strip(), None)
    if partition_tag:
...
def process_apex_keys_apk_certs_common(framework_target_files_dir,
  ...
  for key in framework_dict:
    ...
    partition_tag = extract_partition_tag_from_item(
        PARTITION_TAG_PATTERN, value, "system")

    if partition_tag in framework_partition_set:
  ...
  for key in vendor_dict:
    ...
    if partition_tag in vendor_partition_set:
      add_to_merged_dict(key, value)
