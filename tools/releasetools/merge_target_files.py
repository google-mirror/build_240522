...
def compile_split_sepolicy(product_out, partition_map):
  ...
  logger.info('Compiling split sepolicy: %s', ' '.join(cmd))
...
def rebuild_image_with_sepolicy(target_files_dir,
  ...
  # Ensure misc_info.txt contains 'building_<partition>_image=true' as necessary.
  def append_misc_info(misc_info_path):
    misc_info_lines = []
    with open(misc_info_path, 'r') as misc_info:
      misc_info_lines = misc_info.readlines()
    if 'building_{}_image=true'.format(partition) not in misc_info_lines:
      with open(misc_info_path, 'a') as misc_info:
        misc_info.write('building_{}_image=true\n'.format(partition))
  ...
  if not vendor_otatools:
    ...
    append_misc_info(os.path.join(target_files_dir, 'META', 'misc_info.txt'))
  else:
    # TODO(b/192253131): Remove the need for vendor_target_files by fixing
    ...
    # Unzip vendor build's otatools.zip and target-files archive.
    ...
    append_misc_info(
        os.path.join(vendor_target_files_dir, 'META', 'misc_info.txt'))
    ...
    # Move the partition to the merged target files dir.
    shutil.move(
...
def merge_target_files(temp_dir, framework_target_files, framework_item_list,
  ...
  Args:
    ...
    rebuild_recovery: If true, rebuild the recovery patch used by non-A/B
      devices and write it to the system image.
    vendor_otatools: Path to an otatools zip or dir used for recompiling vendor
      images.
  ...
  compile_split_sepolicy(output_target_files_temp_dir, filtered_partitions)
  if rebuild_sepolicy:
