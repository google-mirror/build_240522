...
def merge_dynamic_partition_info_dicts(system_dict,
  ...
  Args:
    ...
    size_prefix: The prefix in partition group size variables that precedes the
      name of the partition group. For example, 'super_group_a_size' would have
      size_prefix 'super_'.
    size_suffix: Similar to size_prefix but for the variable's suffix. For
      example, 'super_group_a_size' would have size_suffix '_size'.
  ...
  # dynamic partitions info file because these values may vary for each board
  # that uses a shared system image.
  ...
  for partition_group in merged_dict['super_partition_groups'].split(' '):
    # Set the partition group's size using the value from the other dynamic
    # partitions info file.
    ...
    # Set the partition group's partition list using a concatination of the
...
def process_dynamic_partitions_info_txt(system_target_files_temp_dir,
                                        other_target_files_temp_dir,
                                        output_target_files_temp_dir):
  ...
  if not os.path.exists(
      os.path.join(other_target_files_temp_dir, 'META',
  ...
  del merged_dynamic_partitions_dict['dynamic_partition_list']

  output_dynamic_partitions_info_txt = os.path.join(
      output_target_files_temp_dir, 'META', 'dynamic_partitions_info.txt')
