...
def process_ab_partitions_txt(
  ...
  with open(system_ab_partitions_txt) as input:
    system_ab_partitions = input.read().splitlines()

  with open(other_ab_partitions_txt) as input:
    other_ab_partitions = input.read().splitlines()

  output_ab_partitions_set = set (system_ab_partitions + other_ab_partitions)

  output_ab_partitions_txt = os.path.join(
      output_target_files_temp_dir, 'META', 'ab_partitions.txt')

  output_ab_partitions = sorted(output_ab_partitions_set)
  print('BILL> output_ab_partitions_set', output_ab_partitions_set)
  print('BILL> output_ab_partitions', output_ab_partitions)
...
def process_misc_info_txt(
  ...
  def read_helper(dir):
    misc_info_txt = os.path.join(dir, 'META', 'misc_info.txt')
    with open(misc_info_txt) as input:
      return list(input.read().splitlines())
