...
class VerityUtilsTest(unittest.TestCase):
  ...
  def test_VerityHashTreeInfoGenerator_create(self):
    ...
    self.assertEqual(verity_utils.VerityHashtreeInfoGenerator, type(generator))
    self.assertEqual(self.partition_size, generator.partition_size)
    self.assertTrue(generator.fec_supported)

  def test_VerityHashTreeInfoGenerator_decomposeImage(self):
    image_file = sparse_img.SparseImage(self._generate_image())

    generator = verity_utils.VerityHashtreeInfoGenerator(
        self.partition_size, 4096, True)
    self.assertTrue(generator.DecomposeSparseImage(image_file))
    self.assertEqual(991232, generator.file_system_size)
    self.assertEqual(12288, generator.hashtree_size)
    self.assertEqual(32768, generator.metadata_size)

  def test_VerityHashTreeInfoGenerator_parseHashtreeMetadata(self):
    image_file = sparse_img.SparseImage(self._generate_image())
    generator = verity_utils.VerityHashtreeInfoGenerator(
        self.partition_size, 4096, True)
    self.assertTrue(generator.DecomposeSparseImage(image_file))

    generator._parse_hashtree_metadata()

    self.assertEqual(self.hash_algorithm, generator.hashtree_info.hash_algorithm)
  ...
  def test_VerityHashTreeInfoGenerator_validateHashTree_smoke(self):
    generator = verity_utils.VerityHashtreeInfoGenerator(
        self.partition_size, 4096, True)
  ...
  def test_VerityHashTreeInfoGenerator_validateHashTree_failure(self):
    generator = verity_utils.VerityHashtreeInfoGenerator(
        self.partition_size, 4096, True)
    generator.image = sparse_img.SparseImage(self._generate_image())
  ...
  def test_VerityHashTreeInfoGenerator_generate(self):
    image_file = sparse_img.SparseImage(self._generate_image())

    generator = verity_utils.CreateHashTreeInfoGenerator(
        'system', 4096, self.prop_dict)
    generator.Generate(image_file)

    info = generator.hashtree_info
