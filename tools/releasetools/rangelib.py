...
class RangeSet(object):
  ...
  def __iter__(self):
    if len(self.original_data) != 0:
      data = tuple(self._original_ordered_data())
    else:
      data = self.data
  ...
  def _original_ordered_data(self):
    """Return tuple of data in original unsorted order. But the block ranges
    should be identical to the one in self.data.

    For example, this function could return (258768,259211,196604,196605) when
    self.data equals (196604,196605,258768,259211)
    """
    if len(self.original_data) == 0:
      return self.data
    out = []
    # self.data should be a subset of self.original_data. Add the
    # intersection of these two lists but in the order of original data.
    for i in range(0, len(self.original_data), 2):
      range_pair = RangeSet(data=list(self.original_data[i:i+2]))
      range_pair = range_pair.intersect(self)
      if len(range_pair.data) != 0:
        out.extend(list(range_pair.data))
    sorted_output = list(out)
    sorted_output.sort()
