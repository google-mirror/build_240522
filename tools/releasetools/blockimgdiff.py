...
class BlockImageDiff(object):
  ...
  def FindSequenceForTransfers(self):
    """A wrapper function to find the sequence with minimum stashed blocks."""
  ...
  def ComputePatchesForInputList(self, diff_queue, compress_target):
    """Returns a list of patch information for the input list of transfers.
      ...
      Returns:
        A list of tuples of (transfer order, patch_info, compressed_size)
    """
  ...
  def SelectAndConvertDiffTransfersToNew(self, stash_to_remove):
    ...
    for xf_index, patch_info, compressed_size in result:
      ...
      if size_ratio > 99:
        ...
        # This heuristic estimates the size increase in the final package to
        # remove one block of stashed data.
        score = ((compressed_size - len(xf.patch_info.content)) * 100.0
                 / used_stash_blocks)
        conversion_candidate.append(TransferSizeScore(xf, used_stash_blocks,
                                                      score))
    # High score goes to the beginning of the list.
    ...
    while conversion_candidate:
      ...
      # conservative here to avoid slight over conversion.
