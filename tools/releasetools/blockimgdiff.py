...
class BlockImageDiff(object):
  ...
  def SelectAndConvertDiffTransfersToNew(self, stashed_size_violated):
    ...
    for xf_index, patch_info, compressed_size in result:
      ...
      if len(xf.patch_info.content) >= compressed_size:
        ...
        score = ((compressed_size - len(xf.patch_info.content)) * 100.0
                 / used_stash_blocks)
        conversion_candidate.append(TransferSizeScore(xf, used_stash_blocks,
                                                      score))
    # Transfers with high score (i.e. expensive to convert) goes to the
    # beginning of the list.
    conversion_candidate.sort(key=lambda x: x.score, reverse=True)

    # TODO(xunchang), improve the logic to find transfers to convert, e.g.
    # convert the ones that contribute to the max stash, run RevisingStashSize
    # multiple times etc.
    while conversion_candidate:
      xf, used_stash_blocks, _ = conversion_candidate.pop()
      # Stops the process if we have removed enough stashed blocks. We are
      # conservative to avoid over converting transfers.
      if removed_stashed_blocks + used_stash_blocks > stashed_size_violated:
        break
