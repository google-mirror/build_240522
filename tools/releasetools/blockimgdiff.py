...
class BlockImageDiff(object):
  ...
  def SelectDiffTransfers(self):
    print("selecting diff commands")

    diff_transfers = [xf for xf in self.transfers if xf.style == "diff" and
                      xf.src_sha1 != xf.tgt_sha1]

    diff_queue = copy.copy(diff_transfers)
    result_queue = []

    def ComputeDiffRatio():
      ...
      while True:
        ...
        with open(tgt_file, "wb") as tgt_fd:
          self.tgt.WriteRangeDataToFd(xf.tgt_ranges, tgt_fd)
        if xf.patch_info:
          pass
        else:
          imgdiff = self.CanUseImgdiff(xf.tgt_name, xf.tgt_ranges,
                                       xf.src_ranges)
          src_file = common.MakeTempFile(prefix="src-")
          with open(src_file, "wb") as src_fd:
            self.src.WriteRangeDataToFd(xf.src_ranges, src_fd)

          xf.patch_info = PatchInfo(imgdiff,
                                    compute_patch(src_file, tgt_file, imgdiff))

        tgt_data = self.tgt.ReadRangeSet(xf.tgt_ranges)
        compress_obj = zlib.compressobj(6, zlib.DEFLATED, -zlib.MAX_WBITS)
        compressed = compress_obj.compress("".join(tgt_data)) + compress_obj.flush()

        with diff_lock:
          result_queue.append((xf, len(compressed)))

    threads = [threading.Thread(target=ComputeDiffRatio)
               for _ in range(self.threads)]
    for th in threads:
      th.start()
    while threads:
    ...
    stashed_blocks = 0
    for xf, compressed_size in result_queue:
      print(xf.style, xf.tgt_name, len(xf.patch_info.content), compressed_size,
            xf.tgt_ranges.size(),
            len(xf.patch_info.content) * 100.0 / xf.tgt_ranges.size() / 4096,
            len(xf.patch_info.content) * 100.0 / compressed_size)
      if len(xf.patch_info.content) * 100.0 / compressed_size > 99:
        for _, sh in xf.use_stash:
          stashed_blocks += sh.size()
        xf.ConvertToNew()

    result_queue = [pair for pair in result_queue if pair[0].style == "diff"
                    and sum(sr.size() for _, sr in pair[0].use_stash) != 0]
    result_queue.sort(
        key=lambda pair: (pair[1] - len(pair[0].patch_info.content)) * 100.0 /
                         (sum(sr[1].size() for sr in pair[0].use_stash)), reverse=True)

    while stashed_blocks < 8000 and result_queue:
      xf, compressed_size = result_queue.pop()
      removed_size = sum(sr[1].size() for sr in xf.use_stash)
      print("pop: ", xf.tgt_name, len(xf.patch_info.content), compressed_size, removed_size)
      stashed_blocks += removed_size
      xf.ConvertToNew()
