...
class ImgdiffStats(object):
  ...
  It keeps track of the files that can or cannot be applied imgdiff. Only files
  with supported types (BlockImageDiff.FileTypeSupportedByImgdiff()) should be
  logged.
  """
...
class BlockImageDiff(object):
  ...
  def CanUseImgdiff(self, name, tgt_ranges, src_ranges, large_apk=False):
    ...
    Returns:
      A boolean result.
    """
    if not self.FileTypeSupportedByImgdiff(name):
      return False
          ...
          if xf.src_sha1 == xf.tgt_sha1:
            ...
            if xf.patch:
              ...
              imgdiff = (
                  ...
                  self.CanUseImgdiff(xf.tgt_name, xf.tgt_ranges, xf.src_ranges))
  ...
  def FindTransfers(self):
    ...
    def AddSplitTransfers(tgt_name, src_name, tgt_ranges, src_ranges, style,
      ...
      # calling the RangeSha1()s.
      if not self.disable_imgdiff and self.FileTypeSupportedByImgdiff(tgt_name):
        if self.tgt.RangeSha1(tgt_ranges) != self.src.RangeSha1(src_ranges):
