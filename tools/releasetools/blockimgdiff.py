...
class BlockImageDiff(object):
  ...
  def ReviseStashSize(self, ignore_stash_limit=False):
    ...
    Args:
      ...
      simultaneous stash blocks instead. No change will be made to the transfer
      list with this flag.

    Return:
      A tuple of (tgt blocks converted to new, max simultaneous stash blocks)
    """
    ...
    stashed_blocks = 0
    new_blocks = 0
    max_simultaneous_stash_blocks = 0
    ...
    for xf in self.transfers:
      ...
      for stash_raw_id, sr in xf.stash_before:
        ...
        if max_allowed and stashed_blocks_after > max_allowed:
          ...
          max_simultaneous_stash_blocks = max(max_simultaneous_stash_blocks,
                                              stashed_blocks)
    ...
    return new_blocks, max_simultaneous_stash_blocks
  ...
  def SelectAndConvertDiffTransfersToNew(self):
    ...
    for xf_index, patch_info, compressed_size in result:
      ...
      # Convert the transfer to new if the compressed size is smaller.
      if size_ratio > 99:
