...
class BlockImageDiff(object):
  ...
  def WriteTransfers(self, prefix):
    ...
    # be freed immediately after its use. So unlike 'stash_raw_id', 'stash_id'
    # may be reused during the life cycle of an update (maintained by
    # 'free_stash_ids' heap and 'next_stash_id').
    #
    # In BBOTA v3+, it uses the hash of the stashed blocks as the stash slot id
    # 'stashes' records the map from 'hash' to the ref count. The stash will be
    # freed only if the count decrements to zero.
    stashes = {}
    stashed_blocks = 0
    ...
    if self.version == 2:
      ...
      # version 3+: the number of stash slots is unused.
      out.insert(2, "0\n")
      out.insert(3, str(max_stashed_blocks) + "\n")
  ...
  def ReverseBackwardEdges(self):
    ...
    record the blocks to be stashed. 'stash_raw_id' is a unique id to identify
    each pair. Note that for the same range (e.g. RangeSet("1-5")), it is
    possible to have multiple pairs with different 'stash_raw_id's. Each
    'stash_raw_id' will be consumed by one transfer. In BBOTA v3+, they will be
    merged into the same stash in WriteTransfers().
    """
