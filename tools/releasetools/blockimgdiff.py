...
class BlockImageDiff(object):
  ...
  def ReviseStashSize(self, ignore_stash_limit=False):
    ...
    Args:
      ...
      simultaneous stash size instead. No change will be made to the transfer
      list with this flag.

    Return:
      A tuple of (tgt blocks converted to new, max simultaneous stash size)
    """
    ...
    for xf in self.transfers:
      ...
      for stash_raw_id, sr in xf.stash_before:
        ...
        if max_allowed and stashed_blocks_after > max_allowed:
          ...
          max_simultaneous_stash_size = max(max_simultaneous_stash_size,
                                            stashed_blocks)
      ...
      if xf.style == "diff":
        ...
        if xf.src_ranges.overlaps(xf.tgt_ranges):
          if (max_allowed and
              stashed_blocks + xf.src_ranges.size() > max_allowed):
            ...
            max_simultaneous_stash_size = max(
                max_simultaneous_stash_size,
                xf.src_ranges.size() * self.tgt.blocksize)
    ...
    return new_blocks, max_simultaneous_stash_size
  ...
  def SelectAndConvertDiffTransfersToNew(self):
    ...
    # Remove the 'move' transfers, and computes the patch & compressed size
    # for the remaining.
