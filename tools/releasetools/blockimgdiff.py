...
class BlockImageDiff(object):
  ...
  def FindTransfers(self):
    ...
    # Save a list of large apks to split separately.
    large_apks = []
    ...
    def ParseAndValidateSplitInfo(patch_size, tgt_ranges, src_ranges, lines):
      """Parse the split info file and return a list of info tuples.

      split-info is in format:
        version#
        count of pieces
      ...
      for line in lines[2:]:
        ...
        assert len(info) == 3
        patch_length = int(info[0].strip())

        split_tgt_size = int(info[1].strip())
        ...
        # position in file.
        split_src_index = RangeSet.parse_raw(info[2].strip())
        split_src_ranges = RangeSet()
        for r in split_src_index:
      ...
      # Check that the size of all split pieces add up to the final file size
      # for patch and target.
      assert tgt_remain.size() == 0
      assert patch_start == patch_size
      return split_info_list

    def AddSplitTransferForLargeApks():
      """Create split transfers for files in large_apks."""
      ...
      while True:
        ...
        split_info_list = ParseAndValidateSplitInfo(patch_size_total,
                                                    ...
                                                    lines)
        pieces = 0
        for patch_start, patch_length, split_tgt_ranges, split_src_ranges\
            in split_info_list:
          with open(patch_file) as f:
            f.seek(patch_start)
            patch_content = f.read(patch_length)

          split_src_name = "{}-{}".format(src_name, pieces)
          split_tgt_name = "{}-{}".format(tgt_name, pieces)
