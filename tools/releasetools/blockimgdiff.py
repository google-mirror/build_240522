...
class BlockImageDiff(object):
  ...
  def ReviseStashSize(self):
    ...
    for xf in self.transfers:
      ...
      for idx, sr in xf.stash_before:
        if stashed_blocks + sr.size() > max_allowed:
          ...
          print("  %10d %10s %s" % (sr.size(), "explicit", use_cmd))
        else:
          stashed_blocks += sr.size()
      ...
      if xf.style == "diff" and self.version >= 3:
        ...
        if xf.src_ranges.overlaps(xf.tgt_ranges):
          if stashed_blocks + xf.src_ranges.size() > max_allowed:
            ...
            print("  %10d %10s %s" % (xf.src_ranges.size(), "implicit", xf))
    ...
    print("  Total %d blocks are packed as new blocks due to insufficient cache "
          "size." % (new_blocks,))
  ...
  def FindTransfers(self):
    """Parse the file_map for generate all the transfers."""
    ...
    def AddTransfer(tgt_name, src_name, tgt_ranges, src_ranges, style, by_id,
      ...
      while (tgt_ranges.size() > MAX_BLOCKS_PER_DIFF_TRANSFER and
        ...
        tgt_first = tgt_ranges.first(1024)
        src_first = src_ranges.first(1024)
