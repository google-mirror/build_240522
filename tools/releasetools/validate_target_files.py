...
def ValidateInstallRecoveryScript(input_tmp, info_dict):
  ...
  check_cmd = re.search(r'if ! applypatch --check (\w+:.+:\w+:\w+);',
                        lines[1].strip())
  check_target = check_cmd.group(1)
  assert len(check_target.split(':')) == 4
  ...
  if full_recovery_image:
    ...
    boot_info = source.group(1).split(':')
    ...
    # --target boot_type:boot_device:boot_size:boot_sha1
    target = re.search(r'--target (\w+:.+:\w+:\w+) && \\', lines[5].strip())
    assert target is not None
    patch_target = target.group(1)

    # Check we have the same recovery target in the check and patch commands.
    assert check_target == patch_target

    recovery_info = patch_target.split(':')
