...
def _CalculateFileSha1(file_name, unpacked_name, round_up=False):
  """Calculate the SHA1 for a given file. Round up its size to 4K if needed."""
...
def ValidateFileAgainstSha1(input_tmp, file_name, file_path, expected_sha1):
  """Check if the file has the expected SHA1."""
...
def ValidateInstallRecoveryScript(input_tmp):
  """Validate the SHA1 embedded in install-recovery.sh.
  ...
  For full recovery, we want to calculate the SHA1 of /system/etc/recovery.img
  and compare it against the one embedded in the script. While for recovery
  from boot, we want to check the SHA1 for both recovery.img and boot.img
  under IMAGES/.
  """
  ...
  with open(os.path.join(input_tmp, script_path), 'r') as script:
    ...
    if len(applypatch_argv) == 5:
      ...
      # We're patching boot.img to get recovery.img. bonus_args is optional
      # and has an extra size of 2.
