...
def AddRadioImagesForAbOta(output_zip, ab_partitions):
  """Copies radio images from RADIO/ or VENDOR_IMAGES/ to IMAGES/.
  ...
  for partition in ab_partitions:
    ...
    # Assert that the image is available under IMAGES/ now.
    if output_zip:
...
def AddCareMapTxtForAbOta(output_zip, ab_partitions, image_paths):
  ...
  for partition in ab_partitions:
    ...
    if (partition == "system" and
      ...
      system_img_path = image_paths['system']
    ...
    if (partition == "vendor" and
      ...
      vendor_img_path = image_paths['vendor']
...
def AddImagesToTargetFiles(filename):
  ...
  # Handle the prebuilt bootloader/radio images for A/B devices.
