...
def GetCareMap(which, imgname):
  ...
  # resides, which is all that needs to be verified. The additional blocks in
  # the image file contain verity metadata, by reading which would trigger
  # invalid reads.
  as we need to exclude the verity metadata blocks from the given
  ...
  # For sparse images, we will only check the blocks that are listed on the care
