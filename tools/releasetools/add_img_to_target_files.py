...
def GetCareMap(which, imgname):
  ...
  try:
    if is_sparse_img:
      with tempfile.NamedTemporaryFile() as tmpfile:
        RunAndCheckOutput(["simg2img", imgname, tmpfile.name])
        avbfooter = ParseAvbFooter(tmpfile)
    else:
  ...
  assert image_size < os.path.getsize(
      imgname), f"AVB footer's original image size {image_size} is larger than "
  f"or equal to image size on disk {os.path.getsize(imgname)}, this can't "
  "happen because a verified image = original image + hash tree data + FEC data + avbfooter."

  if not image_size:
    logger.warning(
        f"Parsed AVB footer does not contain a valid original imag e size {image_size}")
    return None
  ...
  if "extfs_sparse_flag" in OPTIONS.info_dict and is_sparse_img:
