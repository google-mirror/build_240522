...
def AddProductServices(output_zip):
  """Turn the contents of ProductServices into a product-services image and store it in
  output_zip."""

  img = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "product-services.img")
  if os.path.exists(img.input_name):
...
def AddImagesToTargetFiles(filename):
  ...
  # {vendor,product,product-services}.img are unlike system.img or system_other.img.
  # Because it could be built from source, or dropped into target_files.zip as
  # a prebuilt blob. We consider either of them as {vendor,product,product-services}.img\
  # being available, which could be used when generating vbmeta.img for AVB.
  ...
  has_product = (os.path.isdir(os.path.join(OPTIONS.input_tmp, "PRODUCT")) or
                 os.path.exists(os.path.join(OPTIONS.input_tmp, "IMAGES",
                                             "product.img")))
  has_productservices = (os.path.isdir(os.path.join(OPTIONS.input_tmp, "PRODUCTSERVICES")) or
                         os.path.exists(os.path.join(OPTIONS.input_tmp, "IMAGES",
