...
def AddMainline(output_zip):
  """Turn the contents of Mainline into an mainline image and store it in output_zip."""

  img = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "mainline.img")
  if os.path.exists(img.name):
    logger.info("mainline.img already exists; no need to rebuild...")
    return img.name

  block_list = OutputFile(
      output_zip, OPTIONS.input_tmp, "IMAGES", "mainline.map")
  CreateImage(
      OPTIONS.input_tmp, OPTIONS.info_dict, "mainline", img,
      block_list=block_list)
  return img.name
...
def AddImagesToTargetFiles(filename):
  ...
  has_mainline = ((os.path.isdir(os.path.join(OPTIONS.input_tmp,
                                              "MAINLINE")) and
                   OPTIONS.info_dict.get("building_mainline_image")
                   == "true") or
                  os.path.exists(os.path.join(OPTIONS.input_tmp, "IMAGES",
                                              "mainline.img")))
  has_system_ext = (
  ...
  if has_mainline:
    banner("mainline")
    partitions['mainline'] = AddMainline(output_zip)
