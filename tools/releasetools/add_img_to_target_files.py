...
def AddSystem(output_zip, recovery_img=None, boot_img=None):
  ...
  CreateImage(OPTIONS.input_tmp, OPTIONS.info_dict, "system", img,
              block_list=block_list, output_props=True)
...
def AddOdm(output_zip):
  ...
  CreateImage(
      ...
      block_list=block_list, output_props=False)
  return img.name
...
def CreateImage(input_dir, info_dict, what, output_file,
    block_list=None, output_props=None):
  """
  Set output_props to True if it is a dynamic partition, so that its size can
  be used when building super images.
  """
...
def AddSuperSplit(output_zip):
  ...
  def TransformPartitionArg(arg):
    ...
    assert size is not None, \
        '{}_size is not found; is {} built?'.format(name)
    lst[2] = str(size)
    return ':'.join(lst)

  def GetLpmakeArgsWithSizes():
    lpmake_args = shlex.split(OPTIONS.info_dict.get('lpmake_args').strip())

    for i in range(len(lpmake_args)):
      if lpmake_args[i] == '--partition':
