...
def AppendVBMetaArgsForPartition(cmd, partition, img_path):
  """Appends the VBMeta arguments for partition.

  It either includes the partition descriptor directly into vbmeta image, or
  includes the public key into vbmeta image for a chained partition.

  Args:
    cmd: A command list that will be used to generate vbmeta image. May append
        additional args to the list.
    partition: The name of the partition (e.g. "system").
    img_path: The partition image.
  """
...
def AddVBMeta(output_zip, partitions):
  ...
  Args:
    ...
    partitions: A dict that's keyed by partition names with image paths as
        values. Only valid partition names as listed in common.AVB_PARTITIONS
        are accepted.
  """
