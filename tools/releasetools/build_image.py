...
def CleanInputDirectory(input_directory, filter_file):
  ...
  if removed_files:
    print("Removed the following files because they should no longer be installed. " +
      "They may have been leftover from an old build that did install them, " +
      "or created by manual builds of their modules:")
    for f in removed_files:
      print(f)


def copy_one_file(src, dst, line):
  line = line.strip()
  if not line:
    return
  if line != os.path.normpath(line):
  ...
  if line.startswith("../") or line.startswith('/'):
    sys.exit(f"{line}: escapes staging directory by starting with ../ or /")
  src = os.path.join(src, line)
  dst = os.path.join(dst, line)
...
def CopyInputDirectory(src, dst, filter_file):
  with open(filter_file, 'r') as f:
    for line in f:
      ...
      if line.startswith("../") or line.startswith('/'):
        sys.exit(f"{line}: escapes staging directory by starting with ../ or /")
      os.makedirs(os.path.dirname(os.path.join(dst, line)), exist_ok=True)
      shutil.copy(os.path.join(src, line), os.path.join(dst, line), follow_symlinks=False)
...
def main(argv):
  ...
  if args.input_directory_filter_file:
    with tempfile.TemporaryDirectory() as new_input_directory:
