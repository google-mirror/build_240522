...
def AVBCheckVbmetaVersion(image_path, avbtool, vbmeta_version):
  ...
  The vbmeta_version is specified by BOARD_AVB_VBMETA_VERSION.
  ...
  """
  cmdline = "%s info_image  --print_minimum_libavb_version --image %s" % (
      avbtool, image_path)
  output, exit_code = RunCommand(shlex.split(cmdline))
  ...
  expected_vbmeta_version = output.rstrip()
  if expected_vbmeta_version != vbmeta_version:
    print "Incorrect BOARD_AVB_VBMETA_VERSION: %r (expected: %r)" % (
        vbmeta_version, expected_vbmeta_version)
    return False
