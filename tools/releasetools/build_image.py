...
def ImagePropFromGlobalDict(glob_dict, mount_point):
  ...
  # First propagate system properties to system_other. They'll get overridden
  # after as needed.
  ...
  for readonly, src_prop, dest_prop in fmt_props:
    if readonly and mount_point not in ro_mount_points:
      continue
    copy_prop(src_prop.format(mount_point), dest_prop)

  # Set prefixed properties that need a default value.
  if mount_point in ro_mount_points:
    prop = "{}_journal_size".format(mount_point)
    if not copy_prop(prop, "journal_size"):
      d["journal_size"] = "0"

    prop = "{}_extfs_rsv_pct".format(mount_point)
