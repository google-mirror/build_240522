...
def AVBCalcMinPartitionSize(image_size, size_calculator):
  ...
  """
  footer_size = image_size - size_calculator(image_size)
  ...
  # by delta, 2 delta, 4 delta, 8 delta, etc. We start by * 2 because when
  # delta increases, the final footer_size is increased, too.
  delta = common.RoundUpTo4K(int(footer_size * footer_ratio)) * 2
  hi = lo + delta
  new_image_size = size_calculator(hi)

  # Ensure hi is large enough to accommodate image_size. In most cases,
  # it won't enter the loop because initial delta is large enough.
  ...
  while lo < hi:
    ...
    new_image_size = size_calculator(mid)
