...
class ApexApkSigner(object):
  ...
  def ProcessApexFile(self, apk_keys, payload_key, signing_args=None, is_sepolicy=False):
    ...
    for entry in sepolicy_entries:
      sepolicy_name = os.path.basename(entry)
      if sepolicy_name not in apk_keys:
        raise ApexSigningError('Failed to find signing keys for {} in'
                               ' apex {}.  Use "-e <name>=" to specify a key'
                               .format(entry, self.apex_path))
      if not 'etc' in entry:
        logger.warning('Sepolicy path does not contain the intended directory name:'
                       ' %s', entry)
