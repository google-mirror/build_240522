...
def ValidateConfigLists():
  ...
  if from_both:
    logger.error(
        ...
        'config item list (or inferred list) includes each partition: %s' %
        ','.join(from_both))
    has_error = True
...
def InferItemList(input_namelist, framework):
  ...
  # Some META items and images are always grabbed from partial builds directly.
  ...
  for namelist in input_namelist:
    ...
    # Grab all non-framework files in IMAGES
    if partition == 'images':
      image_partition = os.path.splitext(os.path.basename(namelist))[0]
      if not framework and image_partition not in _FRAMEWORK_PARTITIONS:
