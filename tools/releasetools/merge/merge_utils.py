...
def GetTargetFilesItems(target_files_zipfile_or_dir):
  if zipfile.is_zipfile(target_files_zipfile_or_dir):
    ...
    item_list = []
    for _, dirs, files in os.walk(target_files_zipfile_or_dir):
      item_list.extend(dirs)
      item_list.extend(files)
    return item_list
  else:
...
def CollectTargetFiles(input_zipfile_or_dir, output_dir, item_list=None):
  """Extract input zipfile or copy input directory to output directory.

  Extract the input zipfile if `input_zip` is given; otherwise, copy
  `input_dir` to the `output_dir`. Throws an exception when both `input_zip`
  and `input_dir` are not set.

  Args:
    input_zip: The input zipfile to extract.
    input_dir: The input directory to copy.
    output_dir: The output directory that the input files are either extracted
      or copied.
    patterns: Files to be extracted or copied. Will extract or copy all files
      if omitted.
  """
