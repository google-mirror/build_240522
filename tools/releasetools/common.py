...
def CheckApkShareduidPartitionViolations(uid_dict, partition_groups):
  ...
  An error is defined as a sharedUserId that is found in partitions that
  span more than one partition group.

  Args:
    uid_dict: A dictionary created by using the standard json module to read a
      complete shareduid_violation_modules.json file.
    partition_groups: A list of groups, where each group is a list of partitions
      that belong to the same update group.
  ...
  for uid, partitions in uid_dict.items():
    found_in_groups = set()
    for partition, apks in partitions.items():
      for group in partition_groups:
        if partition in group:
          found_in_groups.add(",".join(sorted(group)))
