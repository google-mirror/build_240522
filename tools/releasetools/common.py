...
def _BuildVendorBootImage(sourcedir, avb_config, info_dict=None):
  ...
  fn = os.path.join(sourcedir, "dtb")
  if os.access(fn, os.F_OK):
    partition_name = avb_config["partition_name"]
    has_vendor_kernel_boot = (info_dict.get("vendor_kernel_boot", "") == "true")

    if (not has_vendor_kernel_boot) or \
        (has_vendor_kernel_boot and partition_name == "vendor_kernel_boot"):
      cmd.append("--dtb")
      cmd.append(fn)
  ...
  if info_dict.get("avb_enable") == "true":
    avbtool = info_dict["avb_avbtool"]
    part_size = info_dict[avb_config["part_size"]]
    cmd = [avbtool, "add_hash_footer", "--image", img.name,
           "--partition_size", str(part_size), "--partition_name", avb_config["partition_name"]]
    AppendAVBSigningArgs(cmd, avb_config["partition_name"])
    args = info_dict.get(avb_config['footer_args'])
    if args and args.strip():
      cmd.extend(shlex.split(args))
    RunAndCheckOutput(cmd)
...
def GetVendorBootImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  avb_config = {
      'part_size': 'vendor_boot_size',
      'partition_name': 'vendor_boot',
      'footer_args': 'avb_vendor_boot_add_hash_footer_args'
  }
  data = _BuildVendorBootImage(
      os.path.join(unpack_dir, tree_subdir), avb_config, info_dict)
  if data:
    return File(name, data)
  return None
...
def GetVendorKernelBootImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  avb_config = {
      'part_size': 'vendor_kernel_boot_size',
      'partition_name': 'vendor_kernel_boot',
      'footer_args': 'avb_vendor_kernel_boot_add_hash_footer_args'
  }
  data = _BuildVendorBootImage(
      os.path.join(unpack_dir, tree_subdir), avb_config, info_dict)
  if data:
