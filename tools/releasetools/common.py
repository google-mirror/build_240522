...
def ProcessABPartitionsTxt(target_files_temp_dirs, output_target_files_temp_dir):
  """Performs special processing for META/ab_partitions.txt.

  This function merges the contents of the META/ab_partitions.txt files from all of
  the target file archives, placing the merged result in the output directory. The
  precondition in that the files are already extracted. The post condition is that
  the output META/ab_partitions.txt contains the merged content. The format for each
  ab_partitions.txt a one partition name per line. The output file contains the union
  of the parition names.

  Args:
    target_files_temp_dirs: The names of the directories containing the
  ...
  for target_file in target_files_temp_dirs:
...
def ProcessDynamicPartitionsInfoTxt(target_files_dirs, output_target_files_dir):
  ...
  files from all directories, placing the merged result in the output directory.
...
def GenerateSuperEmpty(target_dir, output_super_empty=''):
  ...
  use_dynamic_partitions = LoadDictionaryFromFile(misc_info_txt).get(
...
def GetVBMetaPartitions(target_dir, info_dict):
  ...
  for partition in AVB_VBMETA_PARTITIONS:
    ...
    if chained_partitions:
      partitions[partition] = os.path.join(target_dir, 'IMAGES',
                                           "%s.img" % partition)
      vbmeta_partitions = [
  ...
  return partitions, vbmeta_partitions
