...
class Options(object):
  ...
  def __init__(self):
    # Set up search path, in order to find the path to framework/ and lib64/. At
    # the time of running this function, user-supplied search path (`--path`)
    # hasn't been available. So we set the path in relative to the path of the
    # current executable as the default, which will be overridden by commandline
    # flag.
