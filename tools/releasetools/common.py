...
class PartitionBuildProps(object):
    ...
    def _ReadPartitionPropFile(input_file, name):
        ...
        if data == '':
            logger.warning(
                "Failed to read build.prop for partition {}".format(name))
        return data
...
def MergeDynamicPartitionInfoDicts(framework_dict, vendor_dict):
    ...
    if "vabc_cow_version" not in vendor_dict or "vabc_cow_version" not in framework_dict:
        ...
        merged_dict["vabc_cow_version"] = min(
            vendor_dict["vabc_cow_version"], framework_dict["vabc_cow_version"])
...
def _SignBootableImage(image_path, prebuilt_name, partition_name,
    ...
    if info_dict.get("avb_enable") == "true":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            RunAndCheckOutput(
                ["unpack_bootimg", "--boot_img", image_path, "--out", tmpdir])
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
    ...
    data = _BuildBootableImage(prebuilt_name, os.path.join(unpack_dir, tree_subdir),
                               ...
                               os.path.join(
                                   unpack_dir, 'META/ramdisk_node_list')
        ...
        for entry in entries:
            if entry.header_offset == 0xFFFFFFFF:
                if len(entry.extra) % 8 == 0:
                    entry.header_offset = int.from_bytes(
                        entry.extra[-12:-4], "little")
                else:
                    entry.header_offset = int.from_bytes(
                        entry.extra[-8:], "little")
...
def GetMinSdkVersion(apk_name):
    ...
    for line in stdoutdata.split("\n"):
        ...
        if m:
            return m.group(1)
    raise ExternalError(
        "No minSdkVersion returned by aapt2 for apk: {}".format(apk_name))
...
def MakeRecoveryPatch(input_dir, output_sink, recovery_img, boot_img,
    ...
    if full_recovery_image:
        ...
        include_recovery_dtbo = info_dict.get(
            "include_recovery_dtbo") == "true"
        include_recovery_acpio = info_dict.get(
            "include_recovery_acpio") == "true"
