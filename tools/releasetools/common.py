...
def make_ramdisk(sourcedir, fs_config_file=None):
...
def _BuildBootableImage(sourcedir, fs_config_file, info_dict=None,
  ...
  if has_ramdisk:
    ramdisk_img = make_ramdisk(sourcedir, fs_config_file)
...
def _BuildVendorBootImage(sourcedir, info_dict=None):
  ...
  Take a ramdisk, dtb, and cmdline from the input (in 'sourcedir'), and turn
  them into a vendor boot image.
  ...
  ramdisk_img = make_ramdisk(sourcedir)
  ...
  if info_dict.get("vboot"):
    img_unsigned = tempfile.NamedTemporaryFile()
    cmd.extend(["--vendor_boot", img_unsigned.name])
  else:
