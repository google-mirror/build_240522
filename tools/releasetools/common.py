...
def _MakeRamdisk(sourcedir, fs_config_file=None, lz4_ramdisks=False):
  ...
  if lz4_ramdisks:
    p2 = Run(["lz4", "-l", "-12" , "--favor-decSpeed"], stdin=p1.stdout, stdout=ramdisk_img.file.fileno())
  else:
...
def _BuildBootableImage(image_name, sourcedir, fs_config_file, info_dict=None,
  ...
  if has_ramdisk:
    ramdisk_img = _MakeRamdisk(sourcedir, fs_config_file, lz4_ramdisks=info_dict.get("lz4_ramdisks")=='true')
...
def _BuildVendorBootImage(sourcedir, info_dict=None):
  ...
  ramdisk_img = _MakeRamdisk(sourcedir, lz4_ramdisks=info_dict.get("lz4_ramdisks")=='true')
