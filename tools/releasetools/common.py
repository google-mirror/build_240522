...
class DeviceSpecificParams(object):
  ...
  def FullOTA_GetBlockDifferences(self):
    """Called in the middle of full OTA installation, after FullOTA_InstallBegin
    and before FullOTA_InstallEnd.  Implementation should return a list of
    BlockDifference objects describing the update on each additional partitions.
    """
  ...
  def IncrementalOTA_GetBlockDifferences(self):
    """Called in the middle of incremental OTA installation, after
    IncrementalOTA_InstallBegin and before IncrementalOTA_InstallEnd.
    Implementation should return a list of BlockDifference objects describing
    the update on each additional partitions.
    """
    return self._DoCall("IncremetnalOTA_GetBlockDifferences")
...
class DynamicPartitionsDifference(object):
  ...
  def WriteScript(self, script, output_zip, write_verify_script=False):
    ...
    tmp_dir = MakeTempDir()
    op_list_path = os.path.join(tmp_dir, "op_list")
