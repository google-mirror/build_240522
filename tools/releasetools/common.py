...
def DealWithAVBSigningPathArgs(split_args):
  """ Deal with avb signing args which path relative to source code """
  # Check '--signing_helper' tool path exists.
  #The tool path maybe relative to the build environment, which may not
  #be available when without build environment and make some cmd like
  #'make_vbmeta_images' and 'add_hash_footer' excute failded. Try using
  # OPTIONS.search_path for this cases
...
def BuildVBMeta(image_path, partitions, name, needed_partitions):
  ...
  if args and args.strip():
    ...
    split_args = DealWithAVBSigningPathArgs(split_args)
...
def _BuildBootableImage(image_name, sourcedir, fs_config_file, info_dict=None,
  ...
  if info_dict.get("avb_enable") == "true":
    ...
    if args and args.strip():
      split_args = DealWithAVBSigningPathArgs(shlex.split(args))
      cmd.extend(split_args)
