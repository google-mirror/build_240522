...
AVB_PARTITIONS = ('boot', 'dtbo', 'odm', 'product', 'recovery', 'system',
                  'system_ext', 'vendor', 'vendor_boot', 'custom')
...
class BuildInfo(object):
  ...
  def __init__(self, info_dict, oem_dicts=None):
    ...
    # Allow oem_dicts to be None to use BuildInfo in sign_target_files_apks
    if self._oem_props and oem_dicts is None:
...
def GetAvbChainedCustomPartitionArg(partition, location, info_dict, key=None):
  ...
  """
  if key is None:
    key = info_dict["avb_custom_key_path"]
  pubkey_path = ExtractAvbPublicKey(info_dict["avb_avbtool"], key)
    ...
    for index, arg in enumerate(split_args[:-1]):
      ...
      if arg == '--include_descriptors_from_image':
        ...
        assert found, 'Failed to find {}'.format(chained_image)
            ...
            OPTIONS.info_dict.get("avb_custom_key_path")):
