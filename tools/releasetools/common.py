...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  if os.path.exists(prebuilt_path):
    ...
    signed_img = tempfile.NamedTemporaryFile(delete=False)
    signed_img.close()
    shutil.copy(prebuilt_path, signed_img.name)
    partition_name = tree_subdir.lower()
    _SignBootableImage(
        signed_img.name, prebuilt_name, partition_name, info_dict)
    signed_file = File.FromLocalFile(name, signed_img.name)
    os.unlink(signed_img.name)
    return signed_file
