...
def BuildSuperEmpty(target_files_dir):
  """Generates super_empty image from target package.

  Args:
    target_files_dir: Path to the target file package which contains misc_info.txt for
  ...
  if use_dynamic_partitions == 'true':
    super_empty_img = os.path.join(target_files_dir, 'IMAGES', 'super_empty.img')
