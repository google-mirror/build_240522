...
def _SignBootableImage(image_path, prebuilt_name, partition_name,
  ...
  Args:
    image_path: The full path of the image, e.g., unpack_dir/IMAGES/boot.img.
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  if os.path.exists(prebuilt_path):
    logger.info("Re-signing prebuilt %s from PREBUILT_IMAGES to IMAGES...",
                prebuilt_name)
    signed_path = os.path.join(unpack_dir, "IMAGES", prebuilt_name)
    shutil.copy(prebuilt_path, signed_path)
    partition_name = tree_subdir.lower()
    _SignBootableImage(signed_path, prebuilt_name, partition_name, info_dict)
    return File.FromLocalFile(name, signed_path)
