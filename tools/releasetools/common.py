...
def ResolveAVBSigningPathArgs(split_args):
  ...
  def get_search_path(path):
    if not path or os.path.exists(path) or not OPTIONS.search_path:
      return path
  ...
  if isinstance(split_args, list):
    for index, arg in enumerate(split_args[:-1]):
      if arg == '--signing_helper':
        ...
        split_args[index + 1] = get_search_path(signing_helper_path)
        break
  elif isinstance(split_args, str):
    split_args = get_search_path(split_args)
