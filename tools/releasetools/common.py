...
class BlockDifference(object):
  def __init__(self, partition, tgt, src=None, check_first_block=False,
    ...
    # On devices with dynamic partitions, for new partitions partitions,
...
class DynamicPartitionsDifference(object):
  def __init__(self, info_dict, block_diff_dict, progress_dict=None,
    ...
    assert set(block_diff_dict.keys()).issubset(set(
    ...
    if partition_updates:
      logger.info("Updating dynamic partitions %s", partition_updates.keys())
    if ignored_partitions:
      logger.info("Ignoring dynamic partitions %s because no BlockDifference "
                  "object is found", list(ignored_partitions))
