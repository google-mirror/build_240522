...
def CompileSplitSEPolicy(product_out, partition_map, output_policy):
  ...
  if not os.path.exists(vendor_plat_version_file):
  ...
  for policy in (map(lambda (partition, path): get_file(partition, path),
                     required_policy_files)):
