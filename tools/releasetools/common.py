...
class BuildInfo(object):
  ...
  def __init__(self, info_dict, oem_dicts=None):
    ...
    for partition in AVB_PARTITIONS:
      try:
        fingerprint = self.CalculateFingerprint(partition)
  ...
  def GetPartitionFingerprint(self, partition):
    for p in [partition] + BuildInfo._FINGERPRINT_DEFAULT_SOURCE_ORDER:
      if p in self._partition_fingerprints:
        return self._partition_fingerprints[p]
    return None

  def CalculateFingerprint(self, partition):
