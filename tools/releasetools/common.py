...
class BuildInfo(object):
  ...
  def _ResolveRoProductBuildProp(self, prop):
    ...
    for source in source_order:
      ...
      prop_val = self._GetRawBuildProp(source, source_prop)
      if prop_val:
        return prop_val
...
def LoadInfoDict(input_file, repacking=False):
  ...
  d["fstab"] = _FindAndLoadRecoveryFstab(d, input_file, ReadHelper)
...
def LoadRecoveryFSTab(read_helper, input_file, fstab_version,
  ...
  try:
    data = read_helper(input_file, recovery_fstab_path)
  except KeyError:
...
def _FindAndLoadRecoveryFstab(info_dict, input_file, read_helper):
  ...
  if info_dict.get('no_recovery') != 'true':
    ...
    return LoadRecoveryFSTab(
        read_helper, input_file, info_dict['fstab_version'],
  ...
  if info_dict.get('recovery_as_boot') == 'true':
    ...
    if isinstance(input_file, zipfile.ZipFile):
      ...
      if not os.path.exists(path):
        recovery_fstab_path = 'BOOT/RAMDISK/etc/recovery.fstab'
    return LoadRecoveryFSTab(
        read_helper, input_file, info_dict['fstab_version'], recovery_fstab_path,
        system_root_image)
