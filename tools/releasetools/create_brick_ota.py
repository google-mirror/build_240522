...
def CreateBrickOta(product_name: str, output_path: Path, extra_wipe_partitions: List[str]):
  partitions_to_wipe = PARTITIONS_TO_WIPE
  if extra_wipe_partitions is not None:
    partitions_to_wipe = PARTITIONS_TO_WIPE + extra_wipe_partitions
  ...
  with zipfile.ZipFile(output_path, "w") as zfp:
    ...
    zfp.writestr("META-INF/com/android/metadata", "\n".join(
        ["ota-type=BRICK", "post-timestamp=9999999999", "pre-device=" + product_name]))
...
def main(argv):
  ...
  parser.add_argument('--extra-wipe-partitions', type=str,
  ...
  CreateBrickOta(args.product, args.otafile, args.extra_wipe_partitions)
  if args.package_key:
