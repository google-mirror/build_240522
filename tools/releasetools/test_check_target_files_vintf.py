...
class CheckTargetFilesVintfTest(test_utils.ReleaseToolsTestCase):
  ...
  @test_utils.SkipIfExternalToolsUnavailable()
  def test_CheckVintf_optional_odm_compat(self):
    test_dir = self.prepare_test_dir('vintf/optional_odm_compat')
  ...
  @test_utils.SkipIfExternalToolsUnavailable()
  def test_CheckVintf_required_odm_incompat(self):
    test_dir = self.prepare_test_dir('vintf/required_odm_incompat')
    write_string_to_file('vintf_odm_manifest_skus=sku',
                         os.path.join(test_dir, 'META/misc_info.txt'), mode='a')
