...
class FSVeritySigner:
  def __init__(self, fsverity_path):
    ...
    self.setHashAlg("sha256")
    self.setBlockSize(4096)
    self.setRawSignature(False)
  ...
  def setHashAlg(self, hash_alg):
    self.hash_alg = hash_alg

  def setBlockSize(self, block_size):
    self.block_size = block_size

  def setRawSignature(self, raw_signature):
  ...
  def sign(self, input_file, output_file=None):
    ...
    if not output_file:
      output_file = input_file + '.fsverity_metadata'
...
if __name__ == '__main__':
  ...
  signer.setKey(args.key)
  signer.setCert(args.cert)
  signer.setHashAlg(args.hash_alg)
  signer.setBlockSize(args.block_size)
  signer.setRawSignature(args.raw_signature)
