  ...
  for input_file in input_files:
    if os.path.exists(os.path.join(
      ...
      if not 'vendor.build.prop' in merged_dict:
        ...
        raise ValueError(
            'Found two vendor build.prop files! Expected only on in combined input.'
        )
...
def CheckTargetFilesCompatibility(input_files, target_files_dir, info_dict):
  ...
  with open(shareduid_violation_modules, 'w') as f:
    ...
    for input_file in input_files:
      image_glob = [
          os.path.basename(g)
          for g in glob.glob(os.path.join(input_file, 'IMAGES', '*.img'))
      ]
...
def MergeTargetFilesForReleaseTools(input_files, temp_dir):
  ...
  partitions, vbmeta_partitions = common.GetVBMetaPartitions(
      temp_target_files_dir, merged_info_dict)
