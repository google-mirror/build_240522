...
def MergeTargetFilesForReleaseTools(input_files, temp_dir):
  ...
  if merged_info_dict['use_dynamic_partitions'] == 'true':
    super_empty_img = os.path.join(temp_target_files_dir, 'IMAGES', 'super_empty.img')
    build_super_image.BuildSuperImage(merged_info_dict, super_empty_img)
  else:
    raise ValueError(
