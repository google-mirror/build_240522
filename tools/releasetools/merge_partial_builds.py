...
"""
Given a set of input target-files, produces any of an image zipfile suitable for use with
'fastboot update', an OTA package, or a super.img

Usage:  merge_partial_builds --input_file partial_target_file --input_file partial_target_file  --output_img output_image_zip --output_ota output_ota_zip --output_super output_super_image

Flags:
  --input_file partial_target_file
      The path to a partial build target files archive that is to be merged. This argument can be repeated.
...
def MergeInfoDictsForReleaseTools(input_files, output_file):
  ...
  for input_file in input_files:
      info_dicts.append(common.LoadDictionaryFromFile(os.path.join(input_file, 'META', 'misc_info.txt')))

  merged_dict = common.MergeDynamicPartitionInfoDicts(info_dicts)

  ab_partitions = common.UniqConcat([d.get('ab_partitions', '') for d in info_dicts])
  if ab_partitions:
    merged_dict['ab_partitions'] = ab_partitions

  avb_custom_images_partition_list = common.UniqConcat([d.get('avb_custom_images_partition_list', '') for d in info_dicts])
  if avb_custom_images_partition_list:
    merged_dict['avb_custom_images_partition_list'] = avb_custom_images_partition_list

for partition_name in common.AVB_VBMETA_PARTITIONS:
    concat = common.UniqConcat([d.get('avb_{}'.format(partition_name), '') for d in info_dicts])
    if concat:
  ...
  for key in ('avb_avbtool', 'super_image_in_update_package', 'bootloader_in_update_package',
              'dynamic_partition_retrofit', 'build_super_partition', 'extfs_sparse_flag', 'vendor.build.prop', 'vintf_enforce',
              'vintf_odm_manifest_skus', 'vintf_vendor_manifest_skus', 'vintf_include_empty_odm_sku', 'vintf_include_empty_vendor_sku', 'recovery_api_version', 'fstab_version', 'allow_non_ab', 'ab_update', 'cache_size', 'default_system_dev_certificate', 'no_recovery', 'tool_extension', 'board_uses_vendorimage'):
    is_consistent, value = common.IsValueConsistent(info_dicts, key)
    if is_consistent:
      merged_dict[key] = value

  os.mkdir(os.path.join(output_file, 'META'))
  common.WriteSortedData(merged_dict, os.path.join(output_file, 'META', 'misc_info.txt'))

  # CheckVintf requires the vendor build props
  for input_file in input_files:
    if os.path.exists(os.path.join(input_file, 'VENDOR', 'build.prop')) or os.path.exists(os.path.join(input_file, 'VENDOR', 'etc', 'build.prop')):
      if not 'vendor.build.prop' in merged_dict:
        merged_dict['vendor.build.prop'] = common.PartitionBuildProps.FromInputFile(input_file, 'vendor')
        logger.warning(merged_dict['vendor.build.prop'])
      else:
        raise ValueError('Found two vendor build.prop files! Expected only on in combined input.')
...
def CheckTargetFilesCompatibility(input_files, target_files_dir, info_dict):
  ...
  with open(shareduid_violation_modules, 'w') as f:
    partition_groups = []
    for input_file in input_files:
      image_glob = [os.path.basename(g) for g in glob.glob(os.path.join(input_file, 'IMAGES', '*.img'))]
      partition_groups.append(image_glob)

    partition_map = {}
    for partition in list(itertools.chain.from_iterable(partition_groups)):
...
def MergeTargetFilesForReleaseTools(input_files, temp_dir):
  ...
  This includes merging any information (e.g. misc_info.txt, dynamic_partitions.txt,
  and ab_partitions.txt) needed for the release tools to produce the needed build
  artifacts. The merged target files archive will only include the minimum information
  needed to allow the release tools to operate. All other information will be excluded.
  ...
  Raises;
    ValueError: If the union of the contents of the set of target files archives is not empty.
  """
  ...
  # The rough huerstic used here is that if the archive has built <partition>.img
  # then we assume it authoritative for that partition and copy both the image itself
  # and the partition directory to the temporary merged target files
  os.mkdir(os.path.join(temp_target_files_dir, 'IMAGES'))
  for input_dir in input_files:
    image_glob = [os.path.basename(g) for g in glob.glob(os.path.join(input_dir, 'IMAGES', '*.img'))]
    for directory in glob.glob(os.path.join(input_dir, '*')):
      ...
      if image in image_glob:
        ...
        shutil.copytree(directory, os.path.join(temp_target_files_dir, os.path.basename(directory)), symlinks=True)
        shutil.copyfile(os.path.join(input_dir, 'IMAGES', image), os.path.join(temp_target_files_dir, 'IMAGES', image))
        if image == 'vendor':
          # We assume that the partial build for the vendor image is
          # authortative for android-info.txt and kernel info
          shutil.copytree(os.path.join(input_dir, 'OTA'), os.path.join(temp_target_files_dir, 'OTA'))
          shutil.copyfile(os.path.join(input_dir, 'META', 'kernel_version.txt'), os.path.join(temp_target_files_dir, 'META', 'kernel_version.txt'))
          shutil.copyfile(os.path.join(input_dir, 'META', 'kernel_configs.txt'), os.path.join(temp_target_files_dir, 'META', 'kernel_configs.txt'))
    for image in ('vbmeta_system.img', 'userdata.img', 'dtbo.img', 'cache.img'):
      if image in image_glob:
        if os.path.exists(os.path.join(temp_target_files_dir, 'IMAGES', image)):
          raise ValueError('{} already exists in output!'.format(image))
        logger.warning('Copying {}'.format(image))
        shutil.copyfile(os.path.join(input_dir, 'IMAGES', image), os.path.join(temp_target_files_dir, 'IMAGES', image))

  merged_info_dict = MergeInfoDictsForReleaseTools(input_files, temp_target_files_dir)

  common.ProcessABPartitionsTxt(input_files, temp_target_files_dir)

  common.ProcessDynamicPartitionsInfoTxt(input_files, temp_target_files_dir)

  CheckTargetFilesCompatibility(input_files, temp_target_files_dir, merged_info_dict)

  partitions, vbmeta_partitions = common.GetVBMetaPartitions(temp_target_files_dir, merged_info_dict)
  OPTIONS.info_dict = merged_info_dict
  common.BuildVBMeta(os.path.join(temp_target_files_dir, 'IMAGES', 'vbmeta.img'), partitions, 'vbmeta', vbmeta_partitions)

  common.GenerateSuperEmpty(temp_target_files_dir)

  temp_target_files_zip = os.path.join(temp_dir, 'temp-target-files.zip')
  common.CreateTargetFilesArchive(temp_target_files_zip, temp_target_files_dir, temp_dir)
...
def MergePartialBuilds(input_files, output_img, output_ota, output_super):
  ...
  try:
    ...
    temp_target_files_zip = MergeTargetFilesForReleaseTools(extracted_input_files, temp_dir)
    ...
    if output_super:
      build_super_image.main([temp_target_files_dir, output_super])
...
def main():
  ...
  if (args or OPTIONS.input_files is None or (OPTIONS.output_img is None
      and OPTIONS.output_ota is None)):
    common.Usage(__doc__)
    sys.exit(1)

  MergePartialBuilds(OPTIONS.input_files, OPTIONS.output_img, OPTIONS.output_ota, OPTIONS.output_super)
