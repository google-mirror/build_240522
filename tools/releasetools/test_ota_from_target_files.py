...
class RuntimeFingerprintTest(test_utils.ReleaseToolsTestCase):
  ...
  def test_GetPackageMetadata_incremental_package(self):
    ...
    def by_name(state): return state.partition_name

    pre_partition_states = sorted(
        metadata_proto.precondition.partition_state, key=by_name)
    ...
    post_partition_states = sorted(
        metadata_proto.postcondition.partition_state, key=by_name)
