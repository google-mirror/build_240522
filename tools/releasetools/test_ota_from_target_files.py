...
class OtaFromTargetFilesTest(test_utils.ReleaseToolsTestCase):
  ...
  def test_ConstructOtaApexInfo_incremental_package(self):
    ...
    target_files = self.construct_tf_with_apex_info(infos)

    with zipfile.ZipFile(target_files) as target_zip:
      with zipfile.ZipFile(target_files) as source_zip:
        info_bytes = ConstructOtaApexInfo(target_zip, source_zip)
    apex_metadata_proto = ota_metadata_pb2.ApexMetadata()
