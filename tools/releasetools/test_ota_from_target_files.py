...
class PayloadSignerTest(unittest.TestCase):
  ...
  @staticmethod
  def GetDefaultSigner(testdata_dir):
    common.OPTIONS.package_key = os.path.join(testdata_dir, 'testkey')
    common.OPTIONS.key_passwords = {
        common.OPTIONS.package_key : None,
    }
  ...
  def test_init(self):
    payload_signer = self.GetDefaultSigner(self.testdata_dir)
...
class PayloadTest(unittest.TestCase):
  ...
  def _construct_target_files():
    ...
    with zipfile.ZipFile(target_files, 'w') as target_files_zip:
      ...
      for partition in ab_partitions:
        target_files_zip.writestr('IMAGES/' + partition + '.img', os.urandom(10))
  ...
  def test_SignPayload_full(self):
    payload = self._create_payload_full()
    payload.SignPayload(PayloadSignerTest.GetDefaultSigner(self.testdata_dir))
  ...
  def test_SignPayload_badSigner(self):
    ...
    payload = self._create_payload_full()
    payload_signer = PayloadSignerTest.GetDefaultSigner(self.testdata_dir)
