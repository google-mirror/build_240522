...
class MockScriptWriter(object):
  ...
  def Mount(self, a, b):
    self.script.append(('Mount', a, b))

  def AssertDevice(self, a):
    self.script.append(('AssertDevice', a))

  def AssertOemProperty(self, a, b):
    self.script.append(('AssertOemProperty', a, b))

  def AssertFingerprintOrThumbprint(self, a, b):
    self.script.append(('AssertFingerprintOrThumbprint', a, b))

  def AssertSomeFingerprint(self, a, b):
    self.script.append(('AssertSomeFingerprint', a, b))

  def AssertSomeThumbprint(self, a, b):
    self.script.append(('AssertSomeThumbprint', a, b))
...
class BuildInfoTest(unittest.TestCase):
  ...
  def test_init(self):
    ...
    self.assertEqual(
        ...
        target_info.GetVendorBuildProp('ro.vendor.build.fingerprint'))
  ...
  def test_WriteDeviceAssertion(self):
    target_info = BuildInfo(self.TEST_INFO_DICT, None)
    script_writer = MockScriptWriter()
    target_info.WriteDeviceAssertion(script_writer)
    self.assertEqual([('AssertDevice', 'product-device')], script_writer.script)

  def test_WriteDeviceAssertion_with_oem_props(self):
    target_info = BuildInfo(self.TEST_INFO_DICT_USES_OEM_PROPS,
                            self.TEST_OEM_DICTS)
    script_writer = MockScriptWriter()
    target_info.WriteDeviceAssertion(script_writer)
    self.assertEqual(
        [
            ('AssertOemProperty', 'ro.product.device', ['device1', 'device2',
                                                        'device3']),
            ('AssertOemProperty', 'ro.product.brand', ['brand1', 'brand2',
                                                       'brand3']),
        ],
  ...
  def test_WriteFingerprintAssertion_source_oem_props(self):
    target_info = BuildInfo(self.TEST_INFO_DICT, None)
  ...
  def test_WriteFingerprintAssertion_target_oem_props(self):
    target_info = BuildInfo(self.TEST_INFO_DICT_USES_OEM_PROPS,
                            self.TEST_OEM_DICTS)
  ...
  def test_WriteFingerprintAssertion_both_with_oem_props(self):
    target_info = BuildInfo(self.TEST_INFO_DICT_USES_OEM_PROPS,
                            self.TEST_OEM_DICTS)
...
class LoadOemDictsTest(unittest.TestCase):
  ...
  def test_SingleDict(self):
    dict_file = tempfile.NamedTemporaryFile(delete=False, dir=self.tempdir)
    with dict_file as dict_fp:
      dict_fp.write('abc=1\ndef=2\nxyz=foo\na.b.c=bar\n')

    oem_dicts = _LoadOemDicts([dict_file.name])
  ...
  def test_MultipleDicts(self):
    ...
    for i in range(3):
      ...
      with dict_file as dict_fp:
        dict_fp.write(
            'ro.build.index={}\ndef=2\nxyz=foo\na.b.c=bar\n'.format(i))
      oem_source.append(dict_file.name)
    oem_dicts = _LoadOemDicts(oem_source)
    self.assertEqual(3, len(oem_dicts))
    self.assertEqual('foo', oem_dicts[0]['xyz'])
    self.assertEqual('bar', oem_dicts[2]['a.b.c'])
    self.assertEqual('0', oem_dicts[0]['ro.build.index'])
    self.assertEqual('1', oem_dicts[1]['ro.build.index'])
    self.assertEqual('2', oem_dicts[2]['ro.build.index'])
