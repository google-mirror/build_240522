...
void usage(void)
{
    ...
    fprintf(stderr,
        "Usage: zipalign [-f] [-p] [-s <padTo>] [-v] [-z] <align> infile.zip outfile.zip\n"
        "       zipalign -c [-p] [-s <padTo>] [-v] <align> infile.zip\n\n" );
    fprintf(stderr,
        "  <align>: alignment in bytes, e.g. '4' provides 32-bit alignment\n");
    fprintf(stderr, "  -c: check alignment only (does not modify file)\n");
    fprintf(stderr, "  -f: overwrite existing outfile.zip\n");
    fprintf(stderr, "  -p: page-align uncompressed .so files\n");
    fprintf(stderr, "  -s padTo: pad file to be sized as mulitples of <padTo>\n");
