...
def mangle_build_prop(prop_list, kernel_version_file_for_uffd_gc):
  ...
  if prop_list.get_value("ro.dalvik.vm.enable_uffd_gc") == "default":
    ...
    prop_list.put("ro.dalvik.vm.enable_uffd_gc",
        "true" if should_enable_uffd_gc(kernel_version_file_for_uffd_gc)
        else "false")
...
def main(argv):
  ...
  parser.add_argument("--kernel-version-file-for-uffd-gc", type=str,
                      required=True, dest='kernel_version_file_for_uffd_gc')
