...
def override_optional_props(prop_list):
  ...
  for name in prop_list.get_all_names():
    ...
    if len(overriding_props) > 1:
      ...
      # Make the last optinal one as non-optional
      optional_props[-1].optinal = False
...
class PropList:
  ...
  def put(self, name, value):
    # Note: when there is an optinal prop for the name, its value isn't changed.
    # Instead a new non-optional prop is appended, which will override the
    # optional prop. Otherwise, the new value might be overriden by an existing
