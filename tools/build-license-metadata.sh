...
installMap["x"]=808    # set an arbitrary index
unset installMap["x"]  # delete it to make installMap empty again
...
moduleName="${license_package_name}"
case "${ofile}" in
  *.meta_lic)
    moduleName=$(expr "${ofile}" : '^.*[/]\([^/]*\)[.]meta_lic$')
    ;;
esac
...
if ${is_container} || [ -z "${targets}" ]; then
  for m in ${installmap}; do
    mod=$(expr "${m}" : '^\([^:]*\):.*$')
    val=$(expr "${m}" : '^[^:]*:\(.*\)$')
    if [ -n "${depModules[${mod}]+isset}" ]; then
      echo 'install_map: "'${mod}:${val}'"'
    elif [ -n "${depModules[${mod}.${moduleName}]+isset}" ]; then
      echo 'install_map: "'${mod}.${moduleName}:${val}'"'
    else
      echo 'install_map: "'${m}'"'
    fi
  done >>"${ofile}"
