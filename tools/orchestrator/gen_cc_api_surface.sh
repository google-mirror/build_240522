...
# This uses rsync --checksum so that copy operation is performed only if src != dest
...
function copy_files(){
  ...
  (cd $inner_tree && rsync --checksum $@ $out_location)
}
...
function generate_bp_from_template(){
  # TODO: Prevent name collision of the headers module in consuming inner trees
  header_module_name=$1_$2_headers
cat << EOF
...
for api_contribution in ${api_contributions}; do
  ...
  # Android.bp should be updated only if the contents have changed. Otherwise, the updated mtime will
  # cause Soong to recompile everything from scratch
  tmp=$(mktemp)
  generate_bp_from_template $api_library_name $api_surface_name $symbol_file $first_version > $tmp
  rsync --checksum $tmp $output_directory_for_contributions/Android.bp
done
