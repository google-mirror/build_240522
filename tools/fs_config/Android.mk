...
# List of supported vendor, oem, odm, product, system_ext and gms Partitions
fs_config_generate_extra_partition_list := $(strip \
  ...
  $(if $(BOARD_GMSIMAGE_FILE_SYSTEM_TYPE),gms) \
)
...
ifneq ($(filter gms,$(fs_config_generate_extra_partition_list)),)
##################################
# Generate the gms/etc/fs_config_dirs binary file for the target
# Add fs_config_dirs or fs_config_dirs_gms to PRODUCT_PACKAGES in
# the device make file to enable
include $(CLEAR_VARS)

LOCAL_MODULE := fs_config_dirs_gms
LOCAL_MODULE_CLASS := ETC
LOCAL_INSTALLED_MODULE_STEM := fs_config_dirs
LOCAL_MODULE_PATH := $(TARGET_OUT_GMS)/etc
include $(BUILD_SYSTEM)/base_rules.mk
$(LOCAL_BUILT_MODULE): PRIVATE_ANDROID_FS_HDR := $(system_android_filesystem_config)
$(LOCAL_BUILT_MODULE): PRIVATE_ANDROID_CAP_HDR := $(system_capability_header)
$(LOCAL_BUILT_MODULE): PRIVATE_TARGET_FS_CONFIG_GEN := $(TARGET_FS_CONFIG_GEN)
...
##################################
# Generate the gms/etc/fs_config_files binary file for the target
# Add fs_config_files of fs_config_files_gms to PRODUCT_PACKAGES in
# the device make file to enable
include $(CLEAR_VARS)

LOCAL_MODULE := fs_config_files_gms
LOCAL_MODULE_CLASS := ETC
LOCAL_INSTALLED_MODULE_STEM := fs_config_files
LOCAL_MODULE_PATH := $(TARGET_OUT_GMS)/etc
include $(BUILD_SYSTEM)/base_rules.mk
