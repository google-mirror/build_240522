...
fs_config_generate_extra_partition_list := $(if \
	$(filter-out $(TARGET_OUT), \
		$(TARGET_OUT_VENDOR)),$(if $(BOARD_USES_VENDORIMAGE) \
			$(BOARD_VENDORIMAGE_FILE_SYSTEM_TYPE),vendor)) $(if \
	$(filter-out $(TARGET_OUT) $(TARGET_OUT_VENDOR), \
		$(TARGET_OUT_OEM)),$(if $(BOARD_USES_OEMIMAGE) \
			$(BOARD_OEMIMAGE_FILE_SYSTEM_TYPE),oem)) $(if \
	$(filter-out $(TARGET_OUT) $(TARGET_OUT_VENDOR) $(TARGET_OUT_OEM), \
		$(TARGET_OUT_ODM)),$(if $(BOARD_USES_ODMIMAGE) \
			$(BOARD_ODMIMAGE_FILE_SYSTEM_TYPE),odm))
# Partitions we actually have build control over, and can supply fs_config_*
fs_config_generate_extra_built_partition_list := $(if \
	$(filter-out $(TARGET_OUT), \
		$(TARGET_OUT_VENDOR)),$(if \
			$(BOARD_VENDORIMAGE_FILE_SYSTEM_TYPE),vendor)) $(if \
	$(filter-out $(TARGET_OUT) $(TARGET_OUT_VENDOR), \
		$(TARGET_OUT_OEM)),$(if \
			$(BOARD_OEMIMAGE_FILE_SYSTEM_TYPE),oem)) $(if \
...
$(LOCAL_BUILT_MODULE): $(fs_config_generate_bin) $(foreach \
    t,$(fs_config_generate_extra_built_partition_list),fs_config_dirs_$(t))
	@mkdir -p $(dir $@)
	$< -D $(if $(fs_config_generate_extra_partition_list),-P $(subst \
		       $(space),$(comma),$(patsubst %,-%,$(strip \
		       $(fs_config_generate_extra_partition_list))))) -o $@
...
include $(BUILD_SYSTEM)/base_rules.mk
$(LOCAL_BUILT_MODULE): $(fs_config_generate_bin) $(foreach \
    t,$(fs_config_generate_extra_built_partition_list),fs_config_files_$(t))
	@mkdir -p $(dir $@)
	$< -F $(if $(fs_config_generate_extra_partition_list),-P $(subst \
		       $(space),$(comma),$(patsubst %,-%,$(strip \
		       $(fs_config_generate_extra_partition_list))))) -o $@
