...
class FSConfigFileParser():
    ...
    def __init__(self, config_files, oem_ranges=None):
 ...
 '''
    ...
    def _gen_inc():
        for i in FSConfigGen._INCLUDES:
            s = '#include %s' if i.startswith('<') else '#include "%s"'
            print  s%i
...
class AIDArrayGen(object):
    ...
    def __init__(self, opt_group):
        opt_group.add_argument('hdrfile', help='The android_filesystem_config.h file to parse')
    ...
    def __call__(self, args):
            ...
            for name, aid in hdr.get_aids().iteritems():
                print AIDArrayGen._ID_ENTRY % (name, aid.define)
...
class OEMAidGen(object):
    ...
    def _print_aid(self, aid):
        ...
        if found_file != self._old_file:
            print OEMAidGen._FILE_COMMENT % aid[0]
            self._old_file = found_file

        print OEMAidGen._GENERIC_DEFINE % (aid[1], aid[2])
