...
class CapabilityHeaderParser(object):
    ...
    _CAP_DEFINE = re.compile(r'\s*#define\s+CAP_.*')
    ...
    def _parse(self, capability_file):
        ...
        for line in capability_file:
            if CapabilityHeaderParser._CAP_DEFINE.match(line):
                chunks = line.split()
                cap = chunks[1]
                value = chunks[2]
                ...
                if not cap in self._SKIP_CAPS:
                    try:
                        self._caps[cap] = int(value, 0)
                    except ValueError:
                        sys.exit('Could not parse capability define "%s":"%s"' % (cap, value))

    @property
    def caps(self):
        """Retrieves the dictionary of capabilities.
...
class FSConfigGen(BaseGenerator):
    ...
    def add_opts(self, opt_group):
        ...
        opt_group.add_argument(
            '--all-partitions',
            required=False,
            help='Comma separated list of all possible partitions, used to ignore these partitions when generating the output for the system partition')

        opt_group.add_argument(
            '--files',
            required=False,
            action='store_true',
            help='Output fs_config_files')

        opt_group.add_argument(
            '--dirs',
            required=False,
            action='store_true',
            help='Output fs_config_dirs')
    ...
    def __call__(self, args):
        ...
        self._capability_parser = CapabilityHeaderParser(args['capability_header'])
        ...
        if self._partition == "system" and self._all_partitions is None:
            sys.exit('All other partitions must be provided if generating output for the system partition')
    ...
    def _to_fs_entry(self, fs_config, out_file):
        ...
        path_binary = ctypes.create_string_buffer(path, path_length_aligned_64).raw
    ...
    def _emit_entry(self, fs_config):
        ...
        if self._partition == "system":
            for skip_partition in self._all_partitions.split(','):
                if path.startswith(skip_partition) or path.startswith("system/" + skip_partition):
                    return False
            return True
        else:
            if path.startswith(self._partition) or path.startswith("system/" + self._partition):
                return True
            return False
    ...
    def _generate(self):
        ...
        if are_files and self._generate_files:
        ...
        if are_dirs and self._generate_dirs:
            with open(self._out_file, 'wb') as open_file:
