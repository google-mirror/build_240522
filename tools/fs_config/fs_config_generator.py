...
class AID(object):
    ...
    def __init__(self, identifier, value, found, is_oem):
        """
        Args:
            ...
            is_oem (Boolean): If the AID belongs to OEM.
        Raises:
...
class AIDHeaderParser(object):
    ...
    def _handle_aid(self, identifier, value):
        ...
        aid = AID(identifier, value, self._aid_header, False)
...
class FSConfigFileParser(object):
    ...
    def _handle_aid(self, file_name, section_name, config):
        ...
        try:
            aid = AID(section_name, value, file_name, True)
        except ValueError as exception:
...
class PasswdGen(BaseGenerator):
    ...
    def _print_formatted_line(self, aid):
        ...
        if aid.is_oem:
            shell_cmd = '/vendor/bin/sh'
        else:
            shell_cmd = '/system/bin/sh'

        print "%s::%s:%s::/:%s" % (logon, uid, uid, shell_cmd)
