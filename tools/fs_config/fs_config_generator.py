...
class FSConfigFileParser(object):
    ...
    _AID_OEM_RESERVED_RANGES = [(2900, 2999), (5000, 5999),]
    ...
    def _handle_aid(self, file_name, section_name, config):
        ...
        normalized_value = FSConfigFileParser._convert_int(value)
        ...
        if not any(lower <= normalized_value <= upper
                   for (lower, upper) in
                   FSConfigFileParser._AID_OEM_RESERVED_RANGES):
    ...
    def _handle_path(self, file_name, section_name, config):
        ...
        tmp = []
        for cap in caps:
            if FSConfigFileParser._convert_int(cap):
                tmp.append('(' + cap + ')')
            else:
