...
class AIDHeaderParser(object):
        ...
        for lineno, line in enumerate(aid_file):
            ...
            if AIDHeaderParser._AID_DEFINE.match(line):
                ...
                try:
                    ...
                    sys.exit(errmsg % (str(exception) + ' for "%s"' % identifier))
    ...
    def _handle_oem_range(self, identifier, value):
        ...
        try:
            ...
            raise ValueError('Could not convert "%s" to integer value, got: "%s"' % (identifier, value))
    ...
    def _is_overlap(range_a, range_b):
        """
        Calculates the overlap of two range tuples. A range tuple is a closed
        range. A closed range includes it's endpoints. Note that python tuples
        use () notation which collides with the mathematical notation for open
        ranges.
