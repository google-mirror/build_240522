...
int main(int argc, char** argv) {
  ...
  for (; (pc < end) && pc->prefix; pc++) {
    ...
    if (partitions) {
      ...
      submit = false;
      /*
       * To deal with partitions_copy == NULL, we skip the loop, and submit
       * and pass the contents unfiltered.  Better to let unrelated content
       * go, than to kill the build processes.  The unrelated content will
       * not affect the platform negatively except to introduce some
       * harmless non-deterministic build product.
       */
