...
class FileLister:
    def __init__(self, args) -> None:
        ...
        self.base = args.base
        self.rel_path = False if args.base else args.rel_path
    ...
    def get_files(self) -> None:
        ...
        for file in sorted(glob(self.folder_dir, recursive=True)):
            if Path(file).is_file():
                if self.base or self.rel_path:
                    file = relpath(file, self.folder_dir[:-2])
                if self.base:
                    file = join(self.base, file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base', dest='base',
                        action='store', default="", type=str,
                        help="optional directory to replace the base directories of output. Cannot be passed with --rel-path.")
    ...
    parser.add_argument('--rel-path', dest='rel_path',
                        ...
                        help="optional flag. If passed, outputs output directory of the files relative to input dir")
