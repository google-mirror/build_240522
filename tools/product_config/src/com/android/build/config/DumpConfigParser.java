...
public class DumpConfigParser {
    ...
    private void parseImpl() throws CsvParser.ParseException, IOException {
        ...
        for (; index < lineCount; index++) {
            ...
            if (matchLineType(line, "phase", 2)) {
                ...
                final Position pos = parsePosition(line, fields.get(5));
    ...
    // TODO: Implement me
    private Position parsePosition(CsvParser.Line line, String str) {
    ...
    private static List<String> splitList(String text) {
        final String[] split = LIST_SEPARATOR.split(text.trim());
        final ArrayList<String> result = new ArrayList(split.length);
        for (String s: split) {
            if (s.length() > 0) {
                result.add(s);
            }
