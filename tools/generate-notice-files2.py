#!/usr/bin/env python
#
# Copyright (C) 2012-2020 The Android Open Source Project
#
...
Usage: generate-notice-files --text-output [plain text output file] \
               ...
               -t [file title] -n [directory of notices] \
               -r [directory of targets] T [T...]

Generate the Android notice files for the given build targets T,
including both text and html files.
...
def combine_notice_files_html(file_hash, input_dir, output_filename):
    ...
    SRC_DIR_STRIP_RE = re.compile(r'^' + re.escape(input_dir) + r'(/.*).txt')
...
def combine_notice_files_xml(files_with_same_hash, input_dir, output_filename):
    ...
    for file_key in files_with_same_hash.keys():
        for filename in files_with_same_hash[file_key]:
             if filename not in id_table:
                 id_table[filename] = set()
             id_table[filename].add(file_key)
    ...
    for filename in sorted_filenames:
        stripped_filename = SRC_DIR_STRIP_RE.sub(r"\1", filename)
        for contentid in id_table.get(filename):
            print >> output_file, '<file-name contentId="%s">%s</file-name>' % (contentid, stripped_filename)
    ...
    # Output the individual notice file lists
    for filename in sorted_filenames:
        for file_key in id_table.get(filename):
...
def get_args():
    ...
    parser.add_argument(
        'targets', metavar='T', nargs='+',
        help='The build targets for which to generate notices.')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='The file title.')
    parser.add_argument(
        '-n', '--notice-dir', required=True,
        help='The directory containing notices.')
    parser.add_argument(
        '-r', '--target-root-dir', required=True,
        help='The directory containing build artifact subdirectories.')
...
def main(argv):
    ...
    input_dir = args.notice_dir

    ROOT_DIR_STRIP_RE = re.compile(r'^' + re.escape(args.target_root_dir) + r'/(.*)$')
    targets = []
    for target in args.targets:
        t = ROOT_DIR_STRIP_RE.sub(r'\1', os.path.normpath(target))
        if t is not None and len(t) > 0:
            targets.append(t)

    files_with_same_hash = defaultdict(list)
    for target in targets:
        filename = os.path.join(input_dir, target + '.txt')
