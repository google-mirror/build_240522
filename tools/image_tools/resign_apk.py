...
def make_sign_apk_cmds(file, key, cert):
    return build_top + "/prebuilts/jdk/jdk9/linux-x86/bin/java -Djava.library.path=" + build_top + "/out/host/linux-x86/lib64 -jar " + build_top + "/out/host/linux-x86/framework/signapk.jar " + cert + ' ' + key + ' ' + file + ' ' + 'temp.signed'
...
execute(make_mount_cmd(image_path + '.bak', mount_dir))
...
#resign_single_apk(res)

if os.path.exists(build_top + key_path_prefix + "/releasekey.x509.pem"):
    tag = "user"
else:
    tag = "userdebug"
print("build varient is " + tag)
execute('DEFAULT_SYSTEM_DEV_CERTIFICATE=' + build_top + key_path_prefix + ' MAINLINE_SEPOLICY_DEV_CERTIFICATES=' + build_top + key_path_prefix + ' \\system/sepolicy/tools/insertkeys.py -c . -o ' + mount_dir + 'system/etc/selinux/plat_mac_permissions.xml system/sepolicy/private/keys.conf system/sepolicy/private/mac_permissions.xml -t ' + tag)

zip_xttr_result, zip_out, zip_error = execute("xattr -p security.selinux " + mount_dir + "system/etc/security/otacerts.zip")
...
execute(make_xattr_cmd(zip_out.rstrip(' \t\r\n\0'), mount_dir + 'system/etc/security/otacerts.zip'))
