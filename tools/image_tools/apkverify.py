...
class ApkSignature:
    def __init__(self, apkpath='test.apk', fd=None):
        ...
        self.zip_file = None
        if fd:
            self.zip_file = ZipFile(fd, 'r')
        else:
            if is_zipfile(self.apkpath):
                self.zip_file = ZipFile(self.apkpath, 'r')
                ...
                if filename.startswith(b'META-INF/'):
                    if filename.endswith(b'.SF'):
                        for sf_line in self.zip_file.read(zipInfo).split(b'\n'):
                            if sf_line.startswith(b'Signature-Version:'):  #sf_line.startswith(b'X-Android-APK-Signed:'):
                                sf_line = sf_line.split(b':')[1].strip()
                                if sf_line == b'1.0':    #sf_line == b'2' or sf_line == b'2, 3':
                                    v2 = False
                            if sf_line.startswith(b'X-Android-APK-Signed:'):
                                sf_line = sf_line.split(b':')[1].strip()
                                if sf_line == b'2' or sf_line == b'2, 3':
                                    v2 = True
    ...
    def __verify_sigv1(self):
        ...
        if self.__v1_jarverifymanifest():
            ...
            if len(sigv1verifys) == 0:
                ...
                return self.sigv1
        return self.sigv1, sigv1sfs

    def __verify_sigv2(self):
        if self.sigv2 is not None:
            return self.sigv2
        self.sigv2 = False
        sig, sig_start, sig_end, cd_end, filesize, _ = self.__v2_zipfindsig()
        # print(sig)
                            ...
                            for _hash_type_tuple_, hash_value in algs_for_zip:
                                ...
                                if hash_type not in algs_for_zip_dict:
                                    ...
                                    if algs_for_zip_dict[hash_type] != hash_value:
                                        ...
                                        return self.sigv2
                            print(sig_start, sig_end, cd_end, filesize, algs_for_zip_dict)
        ...
        return self.sigv2, algs_for_zip_dict
    ...
    def v2_zipfindsig(self):
        return self.__v2_zipfindsig()
    ...
    def __v2_zipfindsig(self):
        ...
        while break_goto_fail:
            ...
            while idx + 12 <= sigin_head - 24:
                ...
                ret_v2sigs[index] = s
                if index == 0x42726577:
                    ret_offset_vp = ret_sig_start + idx + 12
                #print("%d *****%s ****** %d " % (index, length, s))
                idx += 12
                idx += length - 4
                # buff2 = buf[idx:]
                # print(repr(buff2))
            # print(v2sig)
        return ret_v2sigs, ret_sig_start, ret_offset_cd, ret_cd_end, filesize, ret_offset_vp
