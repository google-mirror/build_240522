...
func setup(env []string) {
	builtins = starlark.StringDict{
		...
		"psetdefault":      starlark.NewBuiltin("psetdefault", propsetSetDefault),
		"queue":            starlark.NewBuiltin("queue", MakeQueue),
	...
	// TODO(asmundak): on Windows, it's rather COMSPEC than SHELL, but
	// check the other differences before supporting it on Windows
	const shellKey = "SHELL="
	for _, e := range os.Environ() {
		if strings.HasPrefix(e, shellKey) {
			shellPath = e[len(shellKey):]
		}
	}
}
