...
// If 'only_files' is non-zero, only the paths to flat files are returned.
// NOTE that the returned paths are relative to 'top'.
func find(_ *starlark.Thread, b *starlark.Builtin, args starlark.Tuple,
	kwargs []starlark.Tuple) (starlark.Value, error) {
	var top, pattern string
	var onlyFiles int
	if err := starlark.UnpackPositionalArgs(b.Name(), args, kwargs, 1, &top, &pattern, &onlyFiles); err != nil {
		return starlark.None, err
	}
