...
html_head_scripts = """\
  ...
  function showBySeverity(show) {
    var e = document.getElementById("by_severity");
    e.style.display = (show ? 'block' : 'none');
    e = document.getElementById("by_project");
...
def emit_stats_by_project():
  ...
  totalbyseverity = {s: sum(warnings[p][s] for p in project_names)
  ...
  for s in severity.range:
    if totalbyseverity[s]:
      one_row.append(totalbyseverity[s])
      total_all_severities += totalbyseverity[s]
