...
# dump some number of warnings per project
def dumpstatsbyproject():
    ...
    for p in projectnames:
        ...
        for s in severity.kinds:
            warningsbyproject[p][s] = 0
    for i in warnpatterns:
        s = i['severity']
        if not s in severities:
            continue
        for p in i['projects']:
            warningsbyproject[p][s] += i['projects'][p]
    totalbyseverity = {}
    totalallprojects = 0
    ...
    for p in projectnames:
        ...
        for s in severity.kinds:
            output('<td align="right">{}</td>'.format(warningsbyproject[p][s]))
            totalbyproject += warningsbyproject[p][s]
            totalbyseverity[s] += warningsbyproject[p][s]
...
def dumphtml():
    ...
    # If severity.SKIP warnings are not dumpped,
    # the project anchors should be dumped through dumpskippedanchors.
    dumpseverity(severity.SKIP)
    dumpfixed()
    dumphtmlepilogue()
