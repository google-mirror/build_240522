...
def combine_notice_files_xml(files_with_same_hash, input_dir, output_filename):
    ...
    for filename in sorted_filenames:
        stripped_filename = SRC_DIR_STRIP_RE.sub(r"\1", filename)
        print >> output_file, '<file-name contentKey="%s">%s</file-name>' % (id_table.get(filename), stripped_filename)
    ...
    for filename in sorted_filenames:
        ...
        print >> output_file, '<file-content contentKey="%s">' % file_key
        print >> output_file, '<![CDATA[' + html_escape(open(filename).read()) + ']]>'
        print >> output_file, "</file-content>"
        print >> output_file
