...
class TagValueWriter:
  ...
  @staticmethod
  def marshal_product_package(sbom_doc):
    if sbom_doc.describes:
      product_package_id = sbom_doc.describes
      product_package = [p for p in sbom_doc.packages if p.id == product_package_id][0]
      tagvalues = TagValueWriter.marshal_package(product_package)
      tagvalues.append(
        f'{Tags.RELATIONSHIP}: {sbom_doc.id} {sbom_data.RelationshipType.DESCRIBES} {product_package_id}')
  ...
  def write(sbom_doc, file):
    ...
    content += TagValueWriter.marshal_product_package(sbom_doc)
