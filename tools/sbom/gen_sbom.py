...
Usage example:
  gen-sbom.py --output_file out/soong/sbom/aosp_cf_x86_64_phone/sbom.spdx \
...
def main():
  ...
  # Scan the metadata in sqlite3 and create the corresponding package and file records in SPDX
  conn = sqlite3.connect(':memory:')
  with sqlite3.connect(args.metadata) as c:
    c.backup(conn)

  conn.row_factory = sqlite3.Row
  cursor = conn.cursor()
  make_metadata = cursor.execute(
    'select installed_file, module_path, is_prebuilt_make_module, product_copy_files, kernel_module_copy_files, is_platform_generated from make_metadata')
  rows = make_metadata.fetchall()
  installed_files = []
  for row in rows:
    installed_file = dict(zip(row.keys(), row))
    installed_files.append(installed_file)

  soong_metadata = cursor.execute(
      'select name, package, module_type as soong_module_type, installed_files from modules')
  rows = soong_metadata.fetchall()
  soong_modules = []
  for row in rows:
    module = dict(zip(row.keys(), row))
    module['installed_file_list'] = module['installed_files'].split(' ')
    soong_modules.append(module)

  # Find which Soong module an installed file is from
  for installed_file in installed_files:
    for module in soong_modules:
      if installed_file['installed_file'] in module['installed_file_list']:
        installed_file.update(module)
        #installed_file['module_path'] = module['package']
        break
      else:
        installed_file['soong_module_type'] = ''

  # Scan the metadata in CSV file and create the corresponding package and file records in SPDX
  for installed_file_metadata in installed_files:
    installed_file = installed_file_metadata['installed_file']
    module_path = installed_file_metadata['module_path']
    product_copy_files = installed_file_metadata['product_copy_files']
    kernel_module_copy_files = installed_file_metadata['kernel_module_copy_files']
