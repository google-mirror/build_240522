...
# If a file is from a source fork or prebuilt fork packakge, add its package information to SBOM
def add_package_of_file(file_id, file_metadata, doc, report):
  ...
  external_doc_ref, pkgs, rels = get_sbom_fragments(file_metadata, metadata_file_path)
  if len(pkgs) > 0:
    if external_doc_ref:
...
def main():
  ...
  with sqlite3.connect(':memory:') as conn:
    ...
    make_metadata = cursor.fetchall()
    cursor.close()

    # Get all records from table modules, which contains metadata of all soong modules
    cursor = conn.execute('select name, package, package as module_path, module_type as soong_module_type, built_files, installed_files, static_dep_files from modules')
    soong_metadata = cursor.fetchall()
    cursor.close()
  ...
  for row in make_metadata:
    installed_file_metadata = dict(zip(row.keys(), row))
    installed_files_metadata.append(installed_file_metadata)
  ...
  for installed_file_metadata in installed_files_metadata:
    ...
    if installed_file_metadata['static_dep_files']:
      for dep_file in installed_file_metadata['static_dep_files'].split(' '):
        if dep_file not in built_file2soong_module:
        ...
        doc.add_relationship(sbom_data.Relationship(id1=file_id,
                                                    ...
                                                    id2=new_file_id(dep_file.removeprefix(args.soong_out+'/.intermediates/'))))
