...
PLACEHOLDER_MAINLINE_MODULES = '{MAINLINE_MODULES}'
...
def main(argv):
  ...
  if len(argv) != 4 and len(argv) != 6:
    sys.stderr.write(
        ...
        'and 2 extra optional arguments for replacing mainline modules: '
        '--mainlineModules \'MAINLINE_MODULES\'.\n')
    return 1
  ...
  with open(instrumentation_test_config_template) as template:
    ...
    config = config.replace(PLACEHOLDER_MAINLINE_MODULES, mainline_modules)
