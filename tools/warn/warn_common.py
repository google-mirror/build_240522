...
def find_android_root(buildlog):
  ...
  for line in buildlog:
    ...
    if warning_pattern.match(line) and (
        line.find('/b/f/w/') < 0 and line.find('/tmp/') != 0):
...
def parse_input_file_chrome(infile, flags):
  ...
  # TODO(chh): This might need change to handle RBE output.
...
def parse_input_file_android(infile, flags):
  ...
  # Here we can only be sure to recover paths starting with "/b/f/w/".
