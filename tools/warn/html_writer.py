...
def write_severity(csvwriter, sev, kind, warn_patterns, add_description=False):
...
def dump_csv(csvwriter, warn_patterns, add_description=False):
  """Dump number of warnings [plus warning descriptions] in CSV format to writer."""
  sort_warnings(warn_patterns)
  total = 0
  for s in Severity.levels:
    total += write_severity(csvwriter, s, s.column_header, warn_patterns, add_description)
