...
def emit_buttons(writer):
  ...
  writer('<div id="all_warnings_section" style="display:none;">\n'
         '<p><button class="button" onclick="expandCollapse(1);">'
...
function computeTopDirsFiles() {
  ...
  function countWarnings(minWarnings, warningsOf, isDir) {
    ...
    rows.sort(function(a,b) {return b[1].v-a[1].v;});
...
function dressUp(warning) {
  if (FlagPlatform == "chrome") {
    return warning;
  } else {
    return addURL(warning);
  }
}
function selectDirFile(idx, rows, dirFile) {
  ...
  for (var i = 0; i < WarningMessages.length; i++) {
    if (WarningMessages[i].startsWith(prefix)) {
      data.addRow([dressUp(WarningMessages[i])]);
    }
  }
  var table = new google.visualization.Table(
      document.getElementById(divName));
...
def dump_subsection_header(writer, table_name, section_title):
...
def dump_section_header(writer, table_name, section_title):
  writer('<hr>')
  dump_subsection_header(writer, table_name, section_title)
...
def dump_dir_file_section(writer, dir_file, table_name, section_title):
  ...
  writer('<blockquote>')
  dump_subsection_header(writer, subsection_name, subsection_title)
