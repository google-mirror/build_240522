 ...
 * byte of the APK, as opposed to the JAR Signature Scheme which protects only the names and
...
public abstract class ApkSignerV2 {
     ...
     *    only a minimal amount of APK parsing before the signature is verified, completely
    ...
    public static final int SIGNATURE_RSAPSS_WITH_SHA256 = 0x0101;
    public static final int SIGNATURE_RSAPSS_WITH_SHA512 = 0x0102;
    ...
    public static final String SF_ATTRIBUTE_ANDROID_APK_SIGNED_NAME = "X-Android-APK-Signed-TBD1";
    public static final String SF_ATTRIBUTE_ANDROID_APK_SIGNED_VALUE = "true";
     ...
     * @throws InvalidKeyException if a signing key is in the wrong format or is not suitable for
    ...
    public static ByteBuffer[] sign(
        ...
        // Locate ZIP End of Central Directory, Central Directory, and check that Central Directory
        // is immediately followed by the ZIP End of Central Directory.
        inputApk.order(ByteOrder.LITTLE_ENDIAN);
        int endOfCentralDirOffset = ZipUtils.findZipEndOfCentralDirectoryRecord(inputApk);
        if (endOfCentralDirOffset == -1) {
            throw new ApkParseException("Failed to locate ZIP End of Central Directory");
        }
        if (ZipUtils.isZip64EndOfCentralDirectoryLocatorPresent(
                inputApk, endOfCentralDirOffset)) {
            throw new ApkParseException("ZIP64 format not supported");
        }
        inputApk.position(endOfCentralDirOffset);
        ...
        int expectedEndOfCentralDirOffset = centralDirOffset + centralDirSize;
        if (expectedEndOfCentralDirOffset < centralDirOffset) {
            throw new ApkParseException(
                    "ZIP Central Directory extent too large. Offset: " + centralDirOffset
                            + ", size: " + centralDirSize);
        }
        if (endOfCentralDirOffset != expectedEndOfCentralDirOffset) {
            throw new ApkParseException(
                    "ZIP Central Directory not immeiately followed by ZIP End of"
                            + " Central Directory. CD end: "
                            + expectedEndOfCentralDirOffset + ", EoCD start: "
        ...
        inputApk.clear();
        ByteBuffer beforeCentralDir = sliceAndAdvancePosition(inputApk, centralDirOffset);
        ByteBuffer centralDir =
                sliceAndAdvancePosition(inputApk, endOfCentralDirOffset - centralDirOffset);
        // Create a copy of End of Central Directory because we'll need modify its contents later.
        byte[] endOfCentralDirBytes = new byte[inputApk.remaining()];
        inputApk.get(endOfCentralDirBytes);
        ByteBuffer endOfCentralDir = ByteBuffer.wrap(endOfCentralDirBytes);
        endOfCentralDir.order(inputApk.order());
        ...
        endOfCentralDir.clear();
        ZipUtils.setZipEocdCentralDirectoryOffset(endOfCentralDir, centralDirOffset);
    ...
    private static Map<Integer, byte[]> computeContentDigests(
        ...
        for (ByteBuffer input : contents) {
            while (input.hasRemaining()) {
                ...
                final ByteBuffer chunk = sliceAndAdvancePosition(input, chunkSize);
                for (int digestAlgorithm : digestAlgorithms) {
    ...
    private static byte[] generateApkSigningBlock(byte[] apkSignatureSchemeV2Block) {
        ...
        result.putLong(blockSizeFieldValue);
        result.put(APK_SIG_BLOCK_MAGIC);
    ...
    private static byte[] generateSignerBlock(
        ...
        for (int signatureAlgorithm : signerConfig.signatureAlgorithms) {
            ...
            AlgorithmParameterSpec signatureParameterSpec = signatureParams.getSecond();
            byte[] signatureBytes;
            try {
                Signature signature = Signature.getInstance(jcaSignatureAlgorithm);
                signature.initSign(signerConfig.privateKey);
                signature.setParameter(signatureParameterSpec);
    ...
    private static ByteBuffer sliceAndAdvancePosition(ByteBuffer source, int sliceSize) {
        if (sliceSize < 0) {
            throw new IllegalArgumentException("sliceSize: " + sliceSize);
        }
        int originalLimit = source.limit();
        int position = source.position();
        int limit = position + sliceSize;
    ...
    private static Pair<String, ? extends AlgorithmParameterSpec>
        ...
        switch (sigAlgorithm) {
            case SIGNATURE_RSAPSS_WITH_SHA256:
                return Pair.create(
                        "SHA256withRSA/PSS",
                        new PSSParameterSpec(
                                "SHA-256", "MGF1", MGF1ParameterSpec.SHA256, 256 / 8, 1));
            case SIGNATURE_RSAPSS_WITH_SHA512:
                return Pair.create(
                        "SHA512withRSA/PSS",
                        new PSSParameterSpec(
