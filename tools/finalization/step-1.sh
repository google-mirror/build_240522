...
function commit_step_1_changes() {
    ...
    repo forall -c '\
        if [[ $(git status --short) ]]; then
            ...
            git add -A . ;
            git commit \
              -m "$PLATFORM_CODENAME is now $PLATFORM_SDK_VERSION" \
              -m " "
              -m "Ignore-AOSP-First: $PLATFORM_CODENAME Finalization" \
              -m "Bug: $BUG_ID" \
              -m "Test: build" \
            ;
...
function finalize_step_1_main() {
    ...
    # move all changes to fina-step1 branch and upload to gerrit
