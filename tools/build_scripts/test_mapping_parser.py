"""
TODO(lucafarsi): DO NOT SUBMIT without a detailed description of parser.
"""
...
REQUIRED_MODULES = ['ats-tradefed-tests', 'DsuGsiIntegrationTest', 'adb-remount-sh', 'mediaroutertest', 'CompanionDeviceManagerMultiDeviceTestCases', 'soong_zip']
...
def parseChangedFiles(element, changed_files, project_path):
  if isinstance(element, list):
    for i in element:
      parseChangedFiles(i, changed_files, project_path)
    return
  if not isinstance(element, dict):
    return

  if 'path' in element.keys():
    changed_files.append(project_path + element.get('path'))
    return
  for i in element.values():
    parseChangedFiles(i, changed_files, project_path)

def parseModulesFromTestMappings(test_mappings, module_list):
  if isinstance(test_mappings, list):
    for i in test_mappings:
      parseModulesFromTestMappings(i, module_list)
    return
  if not isinstance(test_mappings, dict):
    return

  if 'name' in test_mappings.keys() and 'host' in test_mappings.keys():
    module_list.append(test_mappings.get('name'))
  for element in test_mappings.values():
    parseModulesFromTestMappings(element, module_list)

def getModulesToBuild(change_info: Text):
  change_info_file = open(change_info)
  change_info_contents = json.load(change_info_file)
  change_info_file.close()

  changed_files = []
  for element in change_info_contents['changes']:
    parseChangedFiles(element, changed_files, element.get('projectPath'))

  test_mappings = GetTestMappings(changed_files, set())

  modules_to_build = REQUIRED_MODULES.copy()

  parseModulesFromTestMappings(test_mappings, modules_to_build)
