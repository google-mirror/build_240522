...
def get_build_var_values(build_var):
  ...
  bash_cmd = "bash ../../soong/soong_ui.bash --dumpvar-mode " + build_var
  build_var_vals = subprocess.run(bash_cmd, capture_output=True, shell=True, text=True)
  # print (build_var_vals.stdout)
  return build_var_vals.stdout

# get a full set of modules in $(PRODUCT_PACKAGE) for a specific lunch target
def get_modules_product_packages():
  result = set()
  modules_product_packages = get_build_var_values("PRODUCT_PACKAGES").split()
  for module in modules_product_packages:
    if module in result:
      continue
    result.add(module)
  return result
...
def main():
  ...
  initial_modules = get_modules_product_packages()
  get_module_product_packages_plus_deps(initial_modules, product_package_modules_plus_deps, soong)
  print (product_package_modules_plus_deps)
