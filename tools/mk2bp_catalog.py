...
ANALYZERS = (
  ...
  Analyzer("jni .so", lambda line: "LOCAL_JNI_SHARED_LIBRARIES" in line),
...
def get_partition_from_installed(PRODUCT_OUT, filename):
  host_prefix = "out/host/linux-x86/"
...
def main():
  ...
  PRODUCT_OUT = "out/target/product/%s" % TARGET_DEVICE
  ...
  for installed, module in soong.installed.items():
    partition = get_partition_from_installed(PRODUCT_OUT, installed)
      ...
      <div id="container">
        ...
        <div class="Help">
          ...
          <table class="HelpColumns">
            ...
            <tr>
              <th class="Warning">jni .so</th>
              <td>Makefiles using <code>LOCAL_JNI_SHARED_LIBRARIES</code>.</td>
            </tr>
            <tr>
  ...
  print("""
    ...
    function update_details(id) {
      ...
      for (var i=0; i<analyses.length; i++) {
        ...
        fileA.href = 'https://source.corp.google.com/android/' + analysis.filename;
        ...
        if (analysis.line_matches.length > 0) {
          ...
          for (var j=0; j<analysis.line_matches.length; j++) {
            ...
            linenoA.href = 'https://source.corp.google.com/android/' + analysis.filename
                + ';l=' + line_match.lineno;
