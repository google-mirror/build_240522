...
"""
Commandline for internal main
-----------------------------
  build/make/tools/mk2bp_catalog.py \
    e-device=$(get_build_var TARGET_DEVICE) \
    --title="Remaining Android.mk files for Google $(get_build_var TARGET_DEVICE)-$TARGET_BUILD_VARIANT (go/remaining-android-mk)" \
    --codesearch="https://source.corp.google.com/android/" \
    > remaining-android-mk.html

Commandline for aosp main
-------------------------
  build/make/tools/mk2bp_catalog.py \
    --device=$(get_build_var TARGET_DEVICE) \
    --title="Remaining Android.mk files for AOSP $(get_build_var TARGET_DEVICE)-$TARGET_BUILD_VARIANT (go/remaining-android-mk)" \
    > remaining-android-mk.html
"""
