...
use aconfig_protos::ProtoParsedFlag as ProtoParsedFlag;
...
fn convert_parsed_flag(flag: &ProtoParsedFlag) -> Flag {
    ...
    let value = "blah".to_string();
    Flag { namespace, name, value, definitions: vec![] }
}
...
impl FlagSource for DeviceConfigSource {
    fn list_flags(namespace: Option<String>) -> Result<Vec<Flag>> {
        ...
        let flags = match namespace {
            Some(n) => reconcile(&pb_flags, dc_flags)
                .iter()
                .filter(|f| f.namespace == n)
                .cloned()
                .collect(),
            None => reconcile(&pb_flags, dc_flags),
        };
        Ok(flags)
    }
}
