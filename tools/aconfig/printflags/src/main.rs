...
fn main() -> Result<()> {
    // read device_config
    let output = Command::new("/system/bin/device_config")
        .arg("list")
        .output()?;
...
"#;
        let expected = HashMap::from([
            ("namespace_one/com.foo.bar.flag_one".to_string(), State::ENABLED),
            ("namespace_one/com.foo.bar.flag_two".to_string(), State::DISABLED),
            ("namespace_two/android.flag_one".to_string(), State::ENABLED),
        ]);
