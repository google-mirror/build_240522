...
class AconfigStorageTest : public ::testing::Test {
 protected:
  std::string copy_to_ro_temp_file(std::string const& source_file) {
    auto temp_file = std::string(std::tmpnam(nullptr));
    auto content = std::string();
    ReadFileToString(source_file, &content);
    WriteStringToFile(content, temp_file);
    chmod(temp_file.c_str(), S_IRUSR | S_IRGRP);
    return temp_file;
  }

  std::string write_storage_location_pb_file(std::string const& package_map,
