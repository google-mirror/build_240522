...
pub fn create_java_lib(mut input: Input, codegen_mode: CodegenMode) -> Result<Vec<OutputFile>> {
    ...
    let _flag_ids = assign_flag_ids(&package, &modified_parsed_flags)?;
...
fn assign_flag_ids(
    package: &str,
    parsed_flags: &[ProtoParsedFlag],
) -> Result<HashMap<String, u32>> {
    assert!(parsed_flags.windows(2).all(|pfs| pfs[0].name() <= pfs[1].name()));
...
mod tests {
    ...
    fn test_assign_flag_ids() {
        ...
        let flag_ids = assign_flag_ids(&package, &parsed_flags.parsed_flag).unwrap();
