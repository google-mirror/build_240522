...
pub fn parse_flags(
    ...
    for mut input in declarations {
        ...
        match container {
            None => ensure!(
                !flag_declarations.has_container(),
                "failed to parse {}: expected no container, got {}",
                input.source,
...
mod tests {
    ...
    #[test]
    fn test_parse_flags_container_mismatch_between_declaration_and_none_command_line() {
        let first_flag = r#"
        package: "com.first"
        container: "declaration.container"
        flag {
        ...
        assert_eq!(
            format!("{:?}", error),
            "failed to parse memory: expected no container, got declaration.container"
        );
    }
