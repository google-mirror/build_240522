...
pub fn generate_code(cache: &Cache, lang: &str) -> Result<GeneratedFile> {
    match lang {
        "java" => generate_java_code(cache),
        "c++" => generate_cpp_code(cache),
        _ => panic!("code gen for {} is not yet supported", lang),
    }
}
