...
pub fn generate_rust_code(cache: &Cache) -> Result<OutputFile> {
    ...
    let path = ["src", "mod.rs"].iter().collect();
...
struct TemplateParsedFlag {
    ...
    // TinyTemplate's conditionals are limited to single if <bool> expressions
...
mod tests {
    ...
    fn test_generate_rust_code() {
        ...
        assert_eq!("src/mod.rs", format!("{}", generated.path.display()));
        let expected = r#"
pub fn test_disabled_ro() -> bool {
    false
}
pub fn test_disabled_rw() -> bool {
    profcollect_libflags_rust::GetServerConfigurableFlag("test", "disabled-rw", "false") == "true"
}
pub fn test_enabled_ro() -> bool {
    true
}
pub fn test_enabled_rw() -> bool {
