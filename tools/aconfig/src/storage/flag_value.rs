...
impl FlagValueList {
        ...
        for pkg in packages.iter() {
            ...
            for pf in pkg.boolean_flags.iter() {
                ...
                list.booleans[start_offset + (*fid as usize)] =
                    if pf.state() == ProtoFlagState::ENABLED { true } else { false };
            }
        }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        [
            ...
            self.booleans
                .iter()
                .map(|&v| if v { 1u8.to_le_bytes() } else { 0u8.to_le_bytes() })
                .collect::<Vec<_>>()
                .concat(),
        ]
        .concat()
...
mod tests {
    ...
    use crate::storage::{
        ...
        tests::read_u8_from_bytes, tests::read_u32_from_bytes,
    };
    ...
    impl FlagValueList {
        ...
        fn from_bytes(bytes: &[u8]) -> Result<Self> {
            ...
            let booleans = (0..num_flags)
                .map(|_| match read_u8_from_bytes(bytes, &mut head).unwrap() {
                    0 => false,
                    _ => true,
                })
                .collect();
    ...
    fn test_serialization() {
        let flag_value_list = create_test_flag_value_list();
        assert!(flag_value_list.is_ok());
        let flag_value_list = flag_value_list.unwrap();
