...
mod tests {
    use super::*;
    use crate::storage::{
        group_flags_by_package, tests::parse_all_test_flags, tests::str_from_bytes,
        tests::u32_from_bytes,
    };
    ...
    impl PackageTableHeader {
        ...
        fn from_bytes(bytes: &[u8]) -> Result<Self> {
            let mut head = 0;
            Ok(PackageTableHeader {
                version: u32_from_bytes(bytes, &mut head)?,
                container: str_from_bytes(bytes, &mut head)?,
                file_size: u32_from_bytes(bytes, &mut head)?,
                num_packages: u32_from_bytes(bytes, &mut head)?,
                bucket_offset: u32_from_bytes(bytes, &mut head)?,
                node_offset: u32_from_bytes(bytes, &mut head)?,
            })
        }
    }

    impl PackageTableNode {
        // test only method to deserialize back into the node strcut
        fn from_bytes(bytes: &[u8], num_buckets: u32) -> Result<Self> {
            let mut head = 0;
            let mut node = PackageTableNode {
                package_name: str_from_bytes(bytes, &mut head)?,
                package_id: u32_from_bytes(bytes, &mut head)?,
                boolean_offset: u32_from_bytes(bytes, &mut head)?,
                next_offset: match u32_from_bytes(bytes, &mut head)? {
                    0 => None,
                    val => Some(val),
                },
                bucket_index: 0,
            };
            node.bucket_index = storage::get_bucket_index(&node.package_name, num_buckets) as u32;
            Ok(node)
        }
    }
