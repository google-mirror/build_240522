    ...
    I: Iterator<Item = &'a ProtoParsedFlag>,
{
    ...
    let files = [
        ...
        FileSpec {
            ...
            template: if codegen_mode == CodegenMode::Production {
                include_str!("../templates/cpp_prod_flag_provider.template")
            } else {
                include_str!("../templates/cpp_test_flag_provider.template")
            },
            dir: "",
        },
    ];
...
"#;
    ...
    fn test_generate_cpp_code_for_prod() {
        ...
        let generated =
            generate_cpp_code(crate::test::TEST_PACKAGE, parsed_flags.parsed_flag.iter(), CodegenMode::Production)
                .unwrap();
    ...
    fn test_generate_cpp_code_for_test() {
        let parsed_flags = crate::test::parse_test_flags();
        let generated =
            generate_cpp_code(crate::test::TEST_PACKAGE, parsed_flags.parsed_flag.iter(), CodegenMode::Test)
                .unwrap();
        let mut generated_files_map = HashMap::new();
        for file in generated {
            generated_files_map.insert(
