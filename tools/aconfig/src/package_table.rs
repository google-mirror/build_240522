...
impl<'a> PackageTableNode<'a> {
    fn new(package: &'a FlagPackage, num_buckets: u32) -> Self {
        let hash_val = create_storage::get_bucket_index(&package.package_name.to_string());
        PackageTableNode {
            ...
            next_offset: 0,
            start_offset: 0,
            bucket_index: (hash_val % num_buckets as u64) as u32,
        }
    }
...
impl<'a> PackageTable<'a> {
    pub fn new(container: &'a str, packages: Vec<FlagPackage>) -> Self {
        ...
        // sort nodes by bucket index efficiency
        table.nodes.sort_by(|a, b| a.bucket_index.cmp(&b.bucket_index));

        // fill all the offsets
        let offset = 0;
        for (i, node) in table.nodes.iter().enumerate() {
            ...
            if table.buckets[node.bucket_index as usize] == u32::MAX {
                table.buckets[node.bucket_index as usize] = node.start_offset;
            }
        }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        vec![self.header.as_bytes(),
             self.buckets.iter().map(|v| v.to_be_bytes()).collect::<Vec<_>>().concat(),
