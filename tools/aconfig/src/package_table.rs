...
impl<'a> PackageTable<'a> {
    pub fn new(container: &'a str, packages: Vec<FlagPackage<'a>>) -> Self {
        // create table
        ...
        for i in 0..table.nodes.len() {
            ...
            let next_node_bucket_idx = table.nodes[i].bucket_index;

            table.nodes[i].start_offset = offset;
            ...
            if i + 1 < table.nodes.len() && node_bucket_idx == next_node_bucket_idx {
                table.nodes[i].next_offset = offset;
            }
        }
