...
mod aconfig_storage_rust_test {
    ...
    fn write_storage_location_file() -> Result<NamedTempFile> {
        let text_proto = r#"
files {
    version: 0
    container: "system"
...
"#;
        ...
        storage_files.write_to_vec(&mut binary_proto_bytes)?;
        let mut file = NamedTempFile::new()?;
        let _ = file.write_all(&binary_proto_bytes);
        Ok(file)
    }

    #[test]
    fn test_package_offset_query() {
        let file = write_storage_location_file().expect("failed to write temp storage location file");
    ...
    fn test_invalid_flag_offset_query() {
        ...
        let err =
            get_flag_offset_impl(&file_full_path, "vendor", 0, "enabled_ro").unwrap_err();
        assert_eq!(
            format!("{:?}", err),
            "StorageFileNotFound(Storage file does not exist for vendor)"
        );
    ...
    fn test_invalid_boolean_flag_value_query() {
        ...
        let err =
            get_boolean_flag_value_impl(&file_full_path, "vendor", 0u32).unwrap_err();
        assert_eq!(
            format!("{:?}", err),
            "StorageFileNotFound(Storage file does not exist for vendor)"
        );
        ...
        let err =
            get_boolean_flag_value_impl(&file_full_path, "system", 8u32).unwrap_err();
        assert_eq!(
            format!("{:?}", err),
