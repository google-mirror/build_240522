...
mod tests {
    ...
    fn test_package_query() {
        ...
        let package_offset = find_package(&package_table[..], "com.android.aconfig.storage.test_1")
            .unwrap()
            .unwrap();
        let expected_package_offset = PackageOffset { package_id: 0, boolean_offset: 0 };
        assert_eq!(package_offset, expected_package_offset);
        let package_offset = find_package(&package_table[..], "com.android.aconfig.storage.test_2")
            .unwrap()
            .unwrap();
        let expected_package_offset = PackageOffset { package_id: 1, boolean_offset: 3 };
        assert_eq!(package_offset, expected_package_offset);
        let package_offset = find_package(&package_table[..], "com.android.aconfig.storage.test_4")
            .unwrap()
            .unwrap();
    ...
    fn test_not_existed_package_query() {
        ...
        let package_table = create_test_package_table().unwrap().as_bytes();
        let package_offset =
            find_package(&package_table[..], "com.android.aconfig.storage.test_3").unwrap();
        assert_eq!(package_offset, None);
        // this will land at the end of a linked list
        let package_offset =
            find_package(&package_table[..], "com.android.aconfig.storage.test_5").unwrap();
        assert_eq!(package_offset, None);
    }
    ...
    fn test_higher_version_storage_file() {
        ...
        let error =
            find_package(&package_table[..], "com.android.aconfig.storage.test_1").unwrap_err();
        assert_eq!(
            format!("{:?}", error),
            format!(
                "Cannot read storage file with a higher version of {} with lib version {}",
