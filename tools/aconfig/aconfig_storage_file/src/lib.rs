...
pub use crate::package_table::{
    PackageOffset, PackageTable, PackageTableHeader, PackageTableNode,
};
...
pub(crate) fn read_u8_from_bytes(buf: &[u8], head: &mut usize) -> Result<u8> {
...
pub(crate) fn read_u16_from_bytes(buf: &[u8], head: &mut usize) -> Result<u16> {
...
pub(crate) fn read_u32_from_bytes(buf: &[u8], head: &mut usize) -> Result<u32> {
...
pub(crate) fn read_str_from_bytes(buf: &[u8], head: &mut usize) -> Result<String> {
...
pub fn get_flag_offset(container: &str, package_id: u32, flag: &str) -> Result<Option<FlagOffset>> {
