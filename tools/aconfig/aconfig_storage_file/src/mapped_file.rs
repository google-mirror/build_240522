...
mod tests {
    ...
    use crate::test_utils::{
        ...
        write_bytes_to_temp_file, set_temp_storage_files_to_read_only,
    };
...
"#;
        ...
        assert_eq!(
            format!("{:?}", error),
            "MapFileFail(fail to map non read only storage file ./tests/package.map)"
        );
...
"#;
        ...
        assert_eq!(
            format!("{:?}", error),
            "MapFileFail(fail to map non read only storage file ./tests/flag.map)"
        );
...
"#;
        ...
        assert_eq!(
            format!("{:?}", error),
            "MapFileFail(fail to map non read only storage file ./tests/flag.val)"
        );
    }
}
