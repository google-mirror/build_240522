...
fn map_container_storage_files(
    ...
    // is modified after mapped. We either have to make this api unsafe or ensure that
    // the file will not be modified which means it is read only. We should remove this
    // restriction if we need to support mmap non read only file in the future (by
    // making this api unsafe). But for now, all flags are boot stable, so the boot
    // flag file copy should be readonly.
    ensure!(
