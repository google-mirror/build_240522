...
static ALL_MAPPED_FILES: Lazy<Mutex<HashMap<&'static str, MappedStorageFileSet>>> =
    Lazy::new(|| {
...
fn find_container_storage_location(
    ...
    Err(anyhow!("Storage file does not exist for {}", container))
}
...
fn map_container_storage_files(
    ...
    let package_map = Arc::new(unsafe { MmapOptions::new().map(&package_map_file)? });
    let flag_map_file = File::open(files_location.flag_map())?;
    let flag_map = Arc::new(unsafe { MmapOptions::new().map(&flag_map_file)? });
    let flag_val_file = File::open(files_location.flag_val())?;
    let flag_val = Arc::new(unsafe { MmapOptions::new().map(&flag_val_file)? });
...
pub fn get_mapped_file(
    ...
    match all_mapped_files.get(container) {
        ...
        None => {
            ...
            all_mapped_files.insert(container, mapped_files);
            Ok(file_ptr)
        }
    }
}
...
"#;
        ...
        map_and_verify(location_file_full_path, StorageFileSelection::FlagMap, "./tests/flag.map");

        map_and_verify(location_file_full_path, StorageFileSelection::FlagVal, "./tests/flag.val");
    }
}
