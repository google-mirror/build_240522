...
private fun generateParsedFlags(fooState: Aconfig.flag_state): InputStream {
  val parsed_flag =
      Aconfig.parsed_flag
          .newBuilder()
...
class CheckFlaggedApisTest : BaseHostJUnit4Test() {
  ...
  fun testParseFlagValues() {
    ...
    val actual = parseFlagValues(generateParsedFlags(Aconfig.flag_state.ENABLED))
    assertEquals(expected, actual)
  }
  ...
  fun testFindErrorsNoErrors() {
    ...
    val actual =
        findErrors(
            ...
            parseFlagValues(generateParsedFlags(Aconfig.flag_state.ENABLED)),
  ...
  fun testFindErrorsDisabledFlaggedApiIsPresent() {
    ...
    val actual =
        findErrors(
            parseApiSignature("in-memory", API_SIGNATURE.byteInputStream()),
            parseFlagValues(generateParsedFlags(Aconfig.flag_state.DISABLED)),
            parseApiVersions(API_VERSIONS.byteInputStream()))
    assertEquals(expected, actual)
  }
}
