...
def dt_name(d_tag):
    g = globals()
    names = {value: name for name, value in g.items() if name.startswith('DT_')}
    return names.get(d_tag, str(d_tag))
...
class ELF(object):
    ...
    def parse(cls, buf, file_name):
                ...
                if sym.is_undef:
                    if not sym.is_weak:
                        try:
                            ...
                            imported[sym_name] = set([sym_ver])
                elif not sym.is_local:
                    try:
                        exported[sym_name].add(sym_ver)
                    except KeyError:
                        exported[sym_name] = set([sym_ver])
        ...
        return ParsedELF(dt_soname, dt_needed, imported, exported, header, first_version_name)
...
def find_symbol(lib, name, version):
    try:
...
def _parse_args():
    ...
    parser.add_argument('--disallow-unresolved-symbols', action='store_true')
...
def main():
    ...
    # Load main ELF file
    try:
        main_elf = ELF.open(args.filename)
        if args.skip_unknown_elf_machine and \
                main_elf.header.e_machine not in known_archs:
            return
    except ELFInvalidMagicError:
        if not args.skip_bad_elf_magic:
            print('error: Failed to read "{}"'.format(args.filename),
                  file=sys.stderr)
            sys.exit(2)
        return

    # Load shared lib ELF files
    shared_libs = []
    for name in args.shared_lib:
        try:
            shared_libs.append(ELF.open(name))
        except Exception:
            print(args.filename, 'error: Failed to read "{}"'.format(name),
                  file=sys.stderr)
            sys.exit(2)

    # Compare DT_NEEDED to shared libs specified in the build system
    has_error = False
    shared_libs_sonames = {lib.dt_soname for lib in shared_libs}
    for lib in main_elf.dt_needed:
        if lib not in shared_libs_sonames:
            print(args.filename,
                  'error: DT_NEEDED "{}" is not specified in shared_libs.'
                  .format(lib.decode('utf-8')), file=sys.stderr)
            has_error = True

    # dt_needed = set(main_elf.dt_needed)
    # for lib in shared_libs:
    #     if lib.dt_soname not in dt_needed:
    #         print('error: "{}" is not specified in DT_NEEDED.'
    #               .format(dt_soname.decode('utf-8')), file=sys.stderr)
    #         has_error = True

    if has_error:
        print(args.filename,
              'note: Suggested fix: LOCAL_SHARED_LIBRARIES := ' + ' '.join(
                  re.sub('\\.so$', '', lib) for lib in main_elf.dt_needed),
              file=sys.stderr)

    # Check symbols
    missing_symbols = []
    if args.disallow_unresolved_symbols and not has_error:
        for sym, imported_vers in main_elf.imported.iteritems():
            for imported_ver in imported_vers:
                found = False
                for lib in [main_elf] + shared_libs:
                    if find_symbol(lib, sym, imported_ver):
                        found = True
                        break
                if not found:
