    ...
    def parse(cls, buf, file_name):
        ...
        if dt_symtab is not None:
            ...
            for i in range(1, num_symbols):
                ...
                if sym.is_undef:
                    if not sym.is_weak:
                        imported.add(sym_name)
                elif not sym.is_local:
                    exported.add(sym_name)
        ...
        return (dt_soname, dt_needed, imported, exported)
...
def main():
    ...
    # Load main ELF file
    try:
        dt_soname, dt_needed, imported, exported = ELF.open(args.filename)
    except Exception:
    ...
    for name in args.shared_lib:
        try:
            ...
            print('error: Failed to read "{}"'.format(name), file=sys.stderr)
            sys.exit(1)

    # Compare DT_NEEDED shared libs
    has_error = False
    shared_libs_sonames = {ent[0] for ent in shared_libs}
    for lib in dt_needed:
        if lib not in shared_libs_sonames:
            print('error: DT_NEEDED "{}" is not specified in shared_libs.'
                  .format(lib.decode('utf-8')), file=sys.stderr)
            has_error = True

    #dt_needed = set(dt_needed)
    #for dt_soname, _, _, _ in shared_libs:
    #    if dt_soname not in dt_needed:
    #        print('error: "{}" is not specified in DT_NEEDED.'
    #              .format(dt_soname.decode('utf-8')), file=sys.stderr)
    #        has_error = True

    # Check symbols
    if args.disallow_unresolved_symbols:
        for sym in sorted(imported):
            if all(sym not in lib[3] for lib in shared_libs):
                sym_name = sym.decode('utf-8')
                print('error: Unresolved symbol: {}'.format(sym_name),
                      file=sys.stderr)
                has_error = True
