...
type licenseGraphImp struct {
	...
	// the universe of build graphs of a sub-graph rooted at rootFiles where all edges
	...
	// targets identifies and indexes the entire set of target node file names. (guarded by mu)
...
func (e targetEdgeImp) areIndependentModules() bool {
	return edgeNodesAreIndependentModules(e)
}
...
func (tn targetNodeImp) ModuleTypes() []string {
	return append([]string{}, tn.lg.targets[tn.file].moduleType...)
}
...
func (tn targetNodeImp) ModuleClasses() []string {
	return append([]string{}, tn.lg.targets[tn.file].moduleClass...)
}
...
func (tn targetNodeImp) LicenseConditions() LicenseConditionSet {
	return tn.lg.targets[tn.file].licenseConditions.Copy()
}
...
// InstalMap returns the list of path name transformations to make. (unordered)
func (tn targetNodeImp) InstallMap() []InstallMap {
	return append([]InstallMap{}, tn.lg.targets[tn.file].installMap...)
}

// Sources returns the list of files actually depended on by the target, which may be a proper subset of the files available from dependencies. (unordered)
