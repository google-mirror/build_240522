...
// getReachableNodes returns the set of reachable nodes in a license graph. (caches result)
func getReachableNodes(lg *licenseGraphImp) *targetNodeSet {
	lg.mu.Lock()
	...
	walk = func(f string) {
		if _, alreadyWalked := rmap[f]; alreadyWalked {
			return
		}
		rmap[f] = nil
