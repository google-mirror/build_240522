...
type ResolutionStep struct {
	// lg identifies the license graph to which the set applies.
	lg *LicenseGraph
	// index identifies the license condition from LicenseGraph.resolutions to use.
	index int
	// wg indicates when the resolution set is ready to use.
	wg sync.WaitGroup
}

// String returns a string representation of the set.
func (rs *ResolutionStep) String() string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "{")
	sep := ""
	for _, tn := range rs.lg.targets {
		cs := tn.resolutions[rs.index]
		if cs.IsEmpty() {
			continue
		}
		fmt.Fprintf(&sb, "%s%s%s", sep, tn.name, cs.String())
		sep = ", "
	}
	fmt.Fprintf(&sb, "}")
	return sb.String()
...
// compliance-only ResolutionStep methods

// newResolutionStep constructs a new, empty instance of resolutionStep for graph `lg`.
// (guarded by lg.mu must be locked)
func newResolutionStep(lg *LicenseGraph) (*ResolutionStep, error) {
	index, err := lg.allocateResolutionSet()
	if err != nil {
		return nil, err
	}
	rs := &ResolutionStep{lg, index, sync.WaitGroup{}}
	rs.wg.Add(1)
	return rs, nil
}
...
func asActionList(lg *LicenseGraph, stepIndex int) actionList {
	result := make(actionList, 0, len(lg.targets))
	for _, target := range lg.targets {
		cs := target.resolutions[stepIndex]
		if cs.IsEmpty() {
			continue
		}
