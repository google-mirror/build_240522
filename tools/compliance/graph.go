...
// InstallMap defines a simple path name transformation applied to Built or
// Installed paths to identify the file's location in the installed context.
//
// e.g. to replace the path to the output directory with the path relative
// to a container.
type InstallMap struct {
	// Prefix is a string that, when it appears at the start of a Built or
	// Installed path, gets replaced by Replacement to identify the file's
	// location in the installed context. e.g. path within a container
	Prefix string

	// Replacement is the string that replaces Prefix when Prefix appears
	// at the start of a Built or Installed path to identify the file's
	// location in the installed context. e.g. path within a container
	Replacement string
}
