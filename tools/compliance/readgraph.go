...
func readFile(recv *receiver, file string) {
	...
	if err != nil {
		...
		fmt.Fprintf(recv.stderr, "error opening license metadata %q: %w", file, err)
		return
	}
	...
	if err != nil {
		recv.results <- &result{file, nil, nil, err}
		fmt.Fprintf(recv.stderr, "error reading license metadata %q: %w", file, err)
		return
	}
	...
	if err != nil {
		recv.results <- &result{file, nil, nil, err}
		fmt.Fprintf(recv.stderr, "error license metadata %q: %w", file, err)
		return
	}
	...
	if err != nil {
		recv.results <- &result{file, nil, nil, err}
		fmt.Fprintf(recv.stderr, "error license metadata dependency %q: %w", file, err)
		return
	}
	tn.Deps = []*license_metadata_proto.AnnotatedDependency{}
