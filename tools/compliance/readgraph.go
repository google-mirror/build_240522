...
func ReadLicenseGraph(rootFS fs.FS, files []string) (LicenseGraph, error) {
	if len(files) == 0 {
...
func readFile(recv *receiver, file string) {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "error opening license metadata %q: %w", file, err)
		return
	}
	...
	if err != nil {
		recv.results <- &result{file, nil, nil, err}
		fmt.Fprintf(os.Stderr, "error license metadata %q: %w", file, err)
		return
	}
