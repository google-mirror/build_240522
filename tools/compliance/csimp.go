...
func (cs *licenseConditionSetImp) AddSet(conditions LicenseConditionSet) {
	cs.addSet(conditions.(*licenseConditionSetImp))
}

// ByName returns a list of the conditions in the set matching `names`.
func (cs *licenseConditionSetImp) ByName(names ...ConditionNames) LicenseConditionSet {
...
func (cs *licenseConditionSetImp) ByOrigin(origin TargetNode) LicenseConditionSet {
	other := &licenseConditionSetImp{cs.lg, make(map[string]map[string]interface{})}
	oimp := origin.(targetNodeImp)
	for name, origins := range cs.conditions {
		if _, ok := origins[oimp.file]; ok {
			other.conditions[name] = make(map[string]interface{})
			other.conditions[name][oimp.file] = nil
		}
	}
	return other
}
...
func newLicenseConditionSet(origin *targetNodeImp, conditions ...string) *licenseConditionSetImp {
	...
	if origin == nil {
		if len(conditions) > 0 {
			panic(fmt.Errorf("attempt to create nil condition set with conditions"))
		}
	} else {
		cs.lg = origin.lg
...
// remove changes the set to delete condition `lc`.
func (cs *licenseConditionSetImp) remove(lc LicenseCondition) {
	if _, isPresent := cs.conditions[lc.Name()]; !isPresent {
		panic(fmt.Errorf("attempt to remove non-existent condition: %q", lc.(licenseConditionImp).asString(":")))
	}
	if _, isPresent := cs.conditions[lc.Name()][lc.Origin().Name()]; !isPresent {
		panic(fmt.Errorf("attempt to remove non-existent origin: %q", lc.(licenseConditionImp).asString(":")))
	}
	delete(cs.conditions[lc.Name()], lc.Origin().Name())
}
...
func (cs *licenseConditionSetImp) asStringList(separator string) []string {
