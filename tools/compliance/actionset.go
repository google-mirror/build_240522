...
type actionSet map[string]*LicenseConditionSet
...
func (as actionSet) add(actsOn string, cs *LicenseConditionSet) {
...
func (as actionSet) addCondition(actsOn string, lc LicenseCondition) {
	if _, ok := as[actsOn]; !ok {
		as[actsOn] = newLicenseConditionSet(lc.origin.lg)
	}
	as[actsOn].Add(lc)
}
