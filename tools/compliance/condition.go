...
// asString returns a string representation of a license conditon.
func (lc LicenseCondition) asString(separator string) string {
	return lc.origin.file + separator + lc.name
}
...
func (l ConditionList) Less(i, j int) bool {
	if l[i].origin.file == l[j].origin.file {
		return l[i].name < l[j].name
	}
	return l[i].origin.file < l[j].origin.file
}
...
func (cl ConditionList) String() string {
	...
	sep := ""
	for _, lc := range cl {
		fmt.Fprintf(&sb, "%s%s:%s", sep, lc.origin.file, lc.name)
		sep = ", "
	}
	fmt.Fprintf(&sb, "]")
	return sb.String()
