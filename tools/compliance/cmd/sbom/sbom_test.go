...
func Test(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.condition+" "+tt.name, func(t *testing.T) {
			...
			ctx := context{stdout, stderr, compliance.GetFS(tt.outDir), "Android", []string{tt.stripPrefix}, fakeTime, &deps}

			err := sbomGenerator(&ctx, rootFiles...)
			if err != nil {
...
func (m packageName) isMatch(line string) bool {
	...
	return groups[1] == replaceChar(m.name)
}

func (m packageName) String() string {
	return "PackageName: " + replaceChar(m.name)
}
...
func (m spdxID) String() string {
	return "SPDXID: SPDXRef-DOCUMENT-" + replaceChar(m.name)
}
...
func (m spdxPkgID) String() string {
	return "SPDXID: SPDXRef-Package-" + replaceChar(m.name)
}
...
func (m spdxRelationship) isMatch(line string) bool {
	...
	return groups[1] == replaceChar(m.pkg1) && groups[2] == m.relation && groups[3] == replaceChar(m.pkg2)
}

func (m spdxRelationship) String() string {
	return "Relationship: SPDXRef-" + replaceChar(m.pkg1) + " " + m.relation + " SPDXRef-Package-" + replaceChar(m.pkg2)
}
