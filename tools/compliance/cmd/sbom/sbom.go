...
`, filepath.Base(os.Args[0]))
	...
	ctx := &context{ofile, os.Stderr, compliance.FS, *product, *stripPrefix, actualTime, &deps}
...
func getDocumentName(ctx *context, tn *compliance.TargetNode, pm *projectmetadata.ProjectMetadata) string {
	...
	if len(tn.ModuleName()) > 0 {
		if pm != nil {
			return replaceChar(pm.Name() + ":" + tn.ModuleName())
		}
...
// getDownloadUrl returns the download URL for pm or NOASSERTION
...
func getProjectMetadata(_ *context, pmix *projectmetadata.Index, tn *compliance.TargetNode) (*projectmetadata.ProjectMetadata, error) {
	pms, err := pmix.MetadataForProjects(tn.Projects()...)
	if err != nil {
		return nil, fmt.Errorf("Unable to read projects for %q: %v\n", tn, err)
	}
	if len(pms) == 0 {
		return nil, nil
	}
...
func sbomGenerator(ctx *context, files ...string) error {
	...
	compliance.WalkTopDownBreadthFirst(nil, lg, func(lg *compliance.LicenseGraph, tn *compliance.TargetNode, path compliance.TargetEdgePath) bool {
		if err != nil {
			return false
		}
