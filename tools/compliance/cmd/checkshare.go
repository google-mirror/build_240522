...
func main() {
	...
	recip := compliance.ResolveTopDownForCondition(licenseGraph, "reciprocal", *fullWalk)

	// proprietary is the set of resolutions for the Proprietary condition.
	proprietary := compliance.ResolveTopDownForCondition(licenseGraph, "proprietary", *fullWalk)

	// restrict is the set of resolutions for the Restricted condition.
	restrict := compliance.ResolveTopDownForCondition(licenseGraph, "restricted", *fullWalk)
	...
	status := 0
	for _, t := range shareSource.AppliesTo() {
		...
		if pconditions.HasByName(compliance.ImpliesProprietary) {
			// reciprocal only matters when the condition originates at the target, but restricted can originate anywhere.
			if allconditions.HasByName(compliance.ImpliesRestricted) || ownconditions.HasByName(compliance.ImpliesReciprocal) {
				// one or more conflicts detected
				status = 1
				// report all conflicting condition combinations
				for _, p := range pconditions.ByName(compliance.ImpliesProprietary) {
					for _, ss := range allconditions.ByName(compliance.ImpliesRestricted) {
						report(t, p, ss)
					}
					for _, ss := range ownconditions.ByName(compliance.ImpliesReciprocal) {
						report(t, p, ss)
					}
				}
			}
