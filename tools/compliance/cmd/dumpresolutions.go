...
func dumpResolutions(ctx *context, stdout, stderr io.Writer, files ...string) error {
	...
	if len(ctx.conditions) > 0 {
		...
		for _, c := range ctx.conditions {
			rlist = append(rlist, compliance.WalkResolutionsForCondition(resolutions, compliance.ConditionNames{c}))
		}
		if len(rlist) == 1 {
			resolutions = rlist[0]
		} else {
	...
	outputResolution := func(tname, aname, oname string, cnames []string) {
		if ctx.graphViz {
			...
			aNode := nodes[aname]
			oNode := nodes[oname]
			fmt.Fprintf(stdout, "\t%s -> %s -> %s [label=\"%s\"];\n", tNode, aNode, oNode, strings.Join(cnames, "\\n"))
		} else {
