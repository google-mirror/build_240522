...
func selfConditionsApplicableForConditionName(name string, selfConditions LicenseConditionSet, treatAsAggregate bool) *licenseConditionSetImp {
	...
	case "notice":
		return result.rename(treatAsAggregate, ImpliesNotice, "notice")
	case "reciprocal":
...
// share source and to keep the source private apply to the target.
func ConflictingSharedPrivateSource(licenseGraph LicenseGraph) []SourceSharePrivacyConflict {
	...
	for _, appliesTo := range combined.AppliesTo() {
		cs, _ := combined.Conditions(appliesTo)

		// conditions includes all of the conditions for the target
		conditions := ConditionList(cs.Conditions())

		if conditions.HasByName(ImpliesPrivate) {
			if conditions.HasByName(ImpliesShared) {
				size += conditions.CountByName(ImpliesShared)
			}
		}
	}
	...
	for _, appliesTo := range combined.AppliesTo() {
		...
		if conditions.HasByName(ImpliesPrivate) {
			if conditions.HasByName(ImpliesShared) {
				...
				for _, p := range conditions.ByName(ImpliesPrivate) {
					for _, ss := range conditions.ByName(ImpliesShared) {
...
//
// Reciprocal and Restricted top-down walks are joined except for restricted
// on the conditions where the AppliesTo and Origin are the same are preserved.
func ResolveSourceSharing(licenseGraph LicenseGraph) ResolutionSet {
	...
	for appliesTo, cs := range rsimp.resolutions {
		...
		for _, condition := range conditions {
			...
			corigin := condition.Origin().Name()
			if _, ok := shareSource[appliesTo].conditions[cname]; !ok {
				shareSource[appliesTo].conditions[cname] = make(map[string]interface{})
			}
			shareSource[appliesTo].conditions[cname][corigin] = rsimp.resolutions[appliesTo].conditions[cname][corigin]
		}
	}
...
func ResolveSourcePrivacy(licenseGraph LicenseGraph) ResolutionSet {
	...
	// affecting any contained private source.
