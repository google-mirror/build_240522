	...
	for _, tt := range tests {
		...
		testPrivateInterface := func(lg *LicenseGraph, cs *LicenseConditionSet, t *testing.T) {
			...
			for origin, expected := range tt.byOrigin {
				...
				if cs.hasByOrigin(origin) {
					if len(expected) == 0 {
						t.Errorf("unexpected LicenseConditionSet.hasByOrigin(%q): got true, want false", origin)
					}
				} else {
					if len(expected) != 0 {
						t.Errorf("unexpected LicenseConditionSet.hasByOrigin(%q): got false, want true", origin)
					}
				}
				if len(expected) != cs.countByOrigin(origin) {
					t.Errorf("unexpected LicenseConditionSet.countByOrigin(%q): got %d, want %d", origin, cs.countByOrigin(origin), len(expected))
				}
				byOrigin := cs.byOrigin(origin)
				if len(expected) != len(byOrigin) {
					t.Errorf("unexpected LicenseConditionSet.byOrigin(%q): got %v, want %v", origin, byOrigin, expected)
				} else {
					sort.Strings(expected)
		...
		t.Run(tt.name+"_private_set_interface", func(t *testing.T) {
			...
			for name, origins := range tt.conditions {
				for _, origin := range origins {
					cs.add(name, newTestNode(lg, origin))
				}
			}
			if tt.add != nil {
				other := newLicenseConditionSet(lg)
				for name, origins := range tt.add {
					for _, origin := range origins {
						other.add(name, newTestNode(lg, origin))
					}
				}
				cs.AddSet(other)
			}
