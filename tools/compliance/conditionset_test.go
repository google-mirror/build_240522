	...
	for _, tt := range tests {
		...
		checkMatching := func(cs LicenseConditionSet, t *testing.T) {
			for data, expectedNames := range tt.matchingAny {
				...
				t.Logf("MatchingAny(%s): actual set %04x, names={%s}", data, actual, strings.Join(actualNames, ", "))
				t.Logf("MatchingAny(%s): expected set %04x, names={%s}", data, expected, strings.Join(expectedNames, ", "))
		...
		checkMatchingSet := func(cs LicenseConditionSet, t *testing.T) {
			for data, expectedNames := range tt.matchingAny {
				...
				t.Logf("MatchingAnySet(%s): actual set %04x, names={%s}", data, actual, strings.Join(actualNames, ", "))
				t.Logf("MatchingAnySet(%s): expected set %04x, names={%s}", data, expected, strings.Join(expectedNames, ", "))
		...
		checkExpected := func(actual LicenseConditionSet, t *testing.T) bool {
			...
			t.Logf("actual license condition set: %04x {%s}", actual, strings.Join(actualNames, ", "))
			t.Logf("expected license condition set: %04x {%s}", expected, strings.Join(expected.Names(), ", "))
		...
		checkExpectedSet := func(actual LicenseConditionSet, t *testing.T) bool {
			...
			if actual.MatchesEverySet(expected, notExpected) {
				t.Errorf("actual.MatchesEverySet({expected}, {not expected}): want fales, got true")
			}
