...
func (ni *NoticeIndex) InputFiles(files []string) []string {
	if files == nil {
		files = make([]string, 0, len(ni.files) + len(ni.projectName) + len(ni.lg.targets))
	}
	for f := range ni.lg.targets {
		files = append(files, f)
	}
	files = ni.pmix.AllMetadataFiles(files)
