...
func checkSame(rsActual, rsExpected ResolutionSet, t *testing.T) {
	for _, target := range rsExpected.AttachesTo() {
	...
	for _, target := range rsActual.AttachesTo() {
		if !rsExpected.AttachesToTarget(target) {
			t.Errorf("unexpected target: %q in AttachesTo(), want not in AttachesTo()", target.Name())
		}
	}
}
