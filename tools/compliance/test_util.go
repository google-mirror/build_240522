...
func toConflictList(lg *licenseGraphImp, data []confl) []SourceSharePrivacyConflict {
	...
	for _, c := range data {
		...
		result = append(result, SourceSharePrivacyConflict{
				...
				licenseConditionImp{cshare, newTestNode(lg, oshare).(targetNodeImp)},
				licenseConditionImp{cprivacy, newTestNode(lg, oprivacy).(targetNodeImp)},
			})
	}
	return result
}
...
func checkSame(rsActual, rsExpected ResolutionSet, t *testing.T) {
	...
	for _, target := range expectedTargets {
		...
		if len(expectedRl) != len(actualRl) {
			t.Errorf("unexpected number of resolutions attach to %q: got %v with %d elements, want %v with %d elements",
		...
		for i := 0; i < len(expectedRl); i++ {
			...
			for i := 0; i < len(expectedConditions); i++ {
				if expectedConditions[i] != actualConditions[i] {
					t.Errorf("unexpected condition attached to %q acting on %q at index %d: got %s, want %s",
						target.Name(), expectedRl[i].ActsOn().Name(), i,
						actualConditions[i].(licenseConditionImp).asString(":"),
	...
	for i, target := range actualTargets {
		if !rsExpected.AttachesToTarget(target) {
			t.Errorf("unexpected target: got %q element %d in AttachesTo() %v with %d elements, want %v with %d elements",
				target.Name(), i, actualTargets, len(actualTargets), expectedTargets, len(expectedTargets))
		}
	}
}
