...
func ResolveBottomUpConditions(graph LicenseGraph) ResolutionSet {
	...
	walk = func(f string, treatAsAggregate bool) *licenseConditionSetImp {
		...
		for _, edge := range lg.index[f] {
			...
			// turn those into the conditons that apply to the target
		...
		// add the target's own conditions
		timp := targetNodeImp{lg, f}
		for _, lc := range lg.targets[f].LicenseConditions {
			result.add(lc, timp)
		}
	...
	for _, r := range lg.rootFiles {
		cs := walk(r, lg.targets[r].GetIsContainer())
		timp := targetNodeImp{lg, r}
		for _, lc := range lg.targets[r].LicenseConditions {
			cs.add(lc, timp)
		}
		rs.add(r, cs)
	}
...
func ResolveTopDownForCondition(graph LicenseGraph, condition string) ResolutionSet {
	...
	walk = func(f string, cs *licenseConditionSetImp, treatAsAggregate bool) {
		...
		if _, walkedBefore := rmap[f]; walkedBefore {
			rmap[f].addSet(cs)
		} else {
		...
		for _, edge := range lg.index[f] {
			...
			if pcs, alreadyWalked := rmap[edge.dependency]; alreadyWalked {
				if !treatAsAggregate {
					if _, asAggregate := cmap[edge.dependency]; asAggregate {
						// was previously walked as pure aggregate,
						// walk again in non-aggregate context
						delete(cmap, edge.dependency)
						walk(edge.dependency, dcs, false /* treat as aggregate */)
						continue
				        }
				}
				// make a new variable before altering value
				pcs = pcs.Copy().(*licenseConditionSetImp)
				pcs.removeSet(dcs)
				if pcs.IsEmpty() {
					// no new conditions -- all walked previously
					continue
				}
			}
