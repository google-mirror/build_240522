...
//
// At time of writing, policy implies that fullWalk will only affect
// "restricted" due to other target conditions not propagating to dependencies.
//
// e.g. If library L is linked into binary B and shared library S, recording
// "restricted" resolutions for both B and S on L and its transive dependencies
// requires walking the sub-tree rooted at L twice. In applications where only
// the fact/ that L has a source-sharing requirement matters, the "restricted"
// walk can complete faster by only walking the sub-tree rooted at L once:
// fullWalk == false.
func ResolveTopDownForCondition(graph LicenseGraph, condition string, fullWalk bool) ResolutionSet {
	lg := graph.(*licenseGraphImp)
	...
	walk = func(f string, cs *licenseConditionSetImp, treatAsAggregate bool) {
		...
		for _, e := range lg.index[f] {
			...
			dcs = selfConditionsApplicableForConditionName(condition, dcs)
			dcs = targetConditionsApplicableToDep(targetEdgeImp{lg, e}, rmap[f], treatAsAggregate)
			if dcs.IsEmpty() {
				continue
			}
	...
	for _, r := range lg.rootFiles {
		...
		cs = selfConditionsApplicableForConditionName(condition, cs)
		if cs.IsEmpty() {
			continue
		}
