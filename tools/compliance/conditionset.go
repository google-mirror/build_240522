...
type LicenseConditionSet struct {
	...
	// by mapping condition name -> origin target -> nil.
	conditions map[string]map[*TargetNode]interface{}
}
...
func (cs *LicenseConditionSet) Add(conditions ...LicenseCondition) {
	...
	for _, lc := range conditions {
		if _, ok := cs.conditions[lc.name]; !ok {
			cs.conditions[lc.name] = make(map[*TargetNode]interface{})
		}
		cs.conditions[lc.name][lc.origin] = nil
	}
}
...
func (cs *LicenseConditionSet) AddSet(other *LicenseConditionSet) {
	...
	for name, origins := range other.conditions {
		...
		if _, ok := cs.conditions[name]; !ok {
			cs.conditions[name] = make(map[*TargetNode]interface{})
		}
	...
	for _, cn := range names {
		for _, name := range cn {
			if origins, ok := cs.conditions[name]; ok {
				other.conditions[name] = make(map[*TargetNode]interface{})
				for origin := range origins {
...
func newLicenseConditionSet() *LicenseConditionSet {
	return &LicenseConditionSet{make(map[string]map[*TargetNode]interface{})}
}
