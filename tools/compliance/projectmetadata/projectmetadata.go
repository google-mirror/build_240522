...
func (ix *Index) readMetadataFile(pi *projectIndex, path string) {
	...
	if err != nil {
		pi.err = fmt.Errorf(`error in project %q METADATA %q: %w

METADATA and METADATA.android files must parse as text protobufs defined by
     build/soong/compliance/project_metadata_proto/project_metadata.proto

unknown fields don't matter. Check quoting, unescaped nested quotes, and
comment marker for protobuf is '#'

if importing a library that uses a different sort of METADATA file, add a
METADATA.android beside it to parse instead
