...
type Index struct {
	...
	// guards concurrent initialization of projects map and task channel above.
	mu sync.Mutex
...
func (ix *Index) MetadataForProjects(projects ...string) ([]*ProjectMetadata, error) {
	...
	for _, p := range projects {
		if _, ok := ix.projects.Load(p); !ok {
			projectsToRead = append(projectsToRead, p)
			ix.projects.Store(p, &projectIndex{done: make(chan struct{})})
		}
	}
	ix.mu.Unlock()
...
// getProjectIndex asserts `project` exists in the `projects` Map and returns the `projectIndex`.
func (ix *Index) getProjectIndex(project string) *projectIndex {
	pi, ok := ix.projects.Load(project)
	if !ok {
		panic(fmt.Errorf("%q does not exist in projects sync.Map", project))
	}
	return pi.(*projectIndex)
}
