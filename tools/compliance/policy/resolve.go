...
func ResolveBottomUpConditions(lg *LicenseGraph) *ResolutionSet {
	...
	walk = func(f string, treatAsAggregate bool) actionSet {
		...
		result[f] = newLicenseConditionSet(lg)
		result[f].addAll(f, lg.targets[f].LicenseConditions...)
...
func ResolveTopDownConditions(lg *LicenseGraph) *ResolutionSet {
	...
	walk = func(f string, cs *LicenseConditionSet, treatAsAggregate bool) {
		...
		for _, edge := range lg.index[f] {
			...
			dcs := targetConditionsApplicableToDep(TargetEdge{lg, edge}, cs, treatAsAggregate)
			if dcs.IsEmpty() {
	...
	for _, r := range lg.rootFiles {
		...
		walk(r, newLicenseConditionSet(lg), lg.targets[r].GetIsContainer())
	}
...
func WalkResolutionsForCondition(rs *ResolutionSet, names ConditionNames) *ResolutionSet {
	...
	WalkTopDown(lg, func(lg *LicenseGraph, tn TargetNode, _ []TargetEdge) bool {
		if _, ok := rmap[tn.file]; ok {
			return false
		}
		if !reachable.Contains(tn) {
			return false
		}
		if as, ok := rs.resolutions[tn.file]; ok {
			fas := as.byActsOn(reachable).byName(names)
			if !fas.isEmpty() {
				rmap[tn.file] = fas
			}
		}
