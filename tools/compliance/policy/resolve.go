...
func ResolveTopDownConditions(graph LicenseGraph) ResolutionSet {
	...
	walk = func(f string, cs *licenseConditionSetImp, treatAsAggregate bool) {
		...
		for _, edge := range lg.index[f] {
			...
			dcs, ok := rs.resolutions[edge.dependency]
			if !ok {
...
// resolving all deriving nodes for `condition`.
func WalkResolutionsForCondition(rs ResolutionSet, condition ConditionNames) ResolutionSet {
	...
	var walk func(f string, cs *licenseConditionSetImp)

	walk = func(f string, cs *licenseConditionSetImp) {
		if _, ok := rmap[f]; ok {
			rmap[f].addSet(cs)
		} else {
			rmap[f] = cs.Copy().(*licenseConditionSetImp)
		...
		// for each dependency
		for _, edge := range lg.index[f] {
			// Only need to include what is shipped.
			...
			dcs = dcs.ByName(condition).(*licenseConditionSetImp)
			if dcs.IsEmpty() {
				continue
			}
	...
	for _, r := range lg.rootFiles {
		...
		cs = cs.ByName(condition).(*licenseConditionSetImp)
		if cs.IsEmpty() {
			continue
		}
