	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedDepActions != nil {
				...
				depActions[tt.edge.dep] = lg.TargetNode(tt.edge.dep).LicenseConditions()
				if otherTarget != "" {
					// simulate a sub-dependency's condition having already propagated up to dep and about to go to target
					otherCs := lg.TargetNode(otherTarget).LicenseConditions()
				...
				for _, triple := range tt.expectedDepActions {
					...
					actsOn := fields[0]
					origin := fields[1]
					expectedConditions := newLicenseConditionSet(lg)
					expectedConditions.addAll(origin, fields[2:]...)
