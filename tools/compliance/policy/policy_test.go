	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedDepActions != nil {
				...
				depActions[tt.edge.dep] = lg.TargetNode(tt.edge.dep).LicenseConditions().(*licenseConditionSetImp)
				if otherTarget != "" {
					// simulate a sub-dependency's condition having already propagated up to dep and about to go to target
					otherCs := lg.TargetNode(otherTarget).LicenseConditions().(*licenseConditionSetImp)
					depActions[tt.edge.dep].addSet(otherCs)
					depActions[otherTarget] = otherCs
				}
				asActual := depActionsApplicableToTarget(lg.Edges()[0].(targetEdgeImp), depActions, tt.treatAsAggregate)
				asExpected := make(actionSet)
				for _, triple := range tt.expectedDepActions {
					fields := strings.Split(triple, ":")
					actsOn := fields[0]
					origin := fields[1]
					expectedConditions := newLicenseConditionSet(&targetNodeImp{lg, origin}, fields[2:]...)
