...
func TestPolicy_selfConditions(t *testing.T) {
	tests := []struct {
		name                   string
		conditionType          string
		conditions             []string
		expectedSelfConditions []string
	}{
		{
			name:                   "noticenotice",
			conditionType:          "notice",
			conditions:             []string{"notice"},
			expectedSelfConditions: []string{"notice"},
		},
		{
			name:                   "noticerestricted",
			conditionType:          "restricted",
			conditions:             []string{"notice"},
			expectedSelfConditions: []string{},
		},
		{
			name:                   "noticeproprietary",
			conditionType:          "proprietary",
			conditions:             []string{"notice"},
			expectedSelfConditions: []string{},
		},
		{
			name:                   "restrictednotice",
			conditionType:          "notice",
			conditions:             []string{"restricted"},
			expectedSelfConditions: []string{"restricted"},
		},
		{
			name:                   "restrictedrestricted",
			conditionType:          "restricted",
			conditions:             []string{"restricted"},
			expectedSelfConditions: []string{"restricted"},
		},
		{
			name:                   "restrictedproprietary",
			conditionType:          "proprietary",
			conditions:             []string{"restricted"},
			expectedSelfConditions: []string{},
		},
		{
			...
			expectedSelfConditions: []string{"proprietary"},
		},
		{
			name:                   "proprietaryrestricted",
			conditionType:          "restricted",
			conditions:             []string{"proprietary"},
			expectedSelfConditions: []string{},
		},
		{
			name:                   "proprietaryproprietary",
			conditionType:          "proprietary",
			conditions:             []string{"proprietary"},
			expectedSelfConditions: []string{"proprietary"},
		},
		{
			name:                   "manynotnotice",
			conditionType:          "notice",
			conditions:             []string{"permissive", "reciprocal", "by_exception_only"},
			expectedSelfConditions: []string{"permissive", "reciprocal", "by_exception_only"},
		},
		{
			name:                   "manynotrestricted",
			conditionType:          "restricted",
			conditions:             []string{"permissive", "reciprocal", "by_exception_only"},
			expectedSelfConditions: []string{},
		},
		{
			name:                   "manynotproprietary",
			conditionType:          "proprietary",
			conditions:             []string{"permissive", "reciprocal", "by_exception_only"},
			expectedSelfConditions: []string{},
		},
		{
			name:                   "manynotice",
			conditionType:          "notice",
			conditions:             []string{"notice", "reciprocal", "by_exception_only"},
			expectedSelfConditions: []string{"notice", "reciprocal", "by_exception_only"},
		},
		{
			name:                   "manyrestricted",
			conditionType:          "restricted",
			conditions:             []string{"permissive", "restricted", "by_exception_only"},
			expectedSelfConditions: []string{"restricted"},
		},
		{
			name:                   "manyproprietary",
			conditionType:          "proprietary",
			conditions:             []string{"permissive", "proprietary", "by_exception_only"},
			expectedSelfConditions: []string{"proprietary"},
		},
		{
			name:                   "manyall",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_ = newTestNode(lg, "target")
			lg.targets["target"].LicenseConditions = append(lg.targets["target"].LicenseConditions, tt.conditions...)
			selfConditions := selfConditionsApplicableForConditionName(
				tt.conditionType, lg.TargetNode("target").LicenseConditions(), false)
			actual := make([]string, 0, len(selfConditions.conditions))
			for name, origins := range selfConditions.conditions {
				actual = append(actual, name)
				for _, payload := range origins {
					if false != payload {
						t.Errorf("unexpected payload: got %v, want false", payload)
					}
				}
			}
			sort.Strings(actual)
			sort.Strings(tt.expectedSelfConditions)
			if len(actual) != len(tt.expectedSelfConditions) {
				t.Errorf("unexpected number of self conditions: got %v with %d conditions, want %v with %d conditions",
					actual, len(actual), tt.expectedSelfConditions, len(tt.expectedSelfConditions))
			} else {
				for i := 0; i < len(actual); i++ {
					if actual[i] != tt.expectedSelfConditions[i] {
						t.Errorf("unexpected self condition at element %d: got %q, want %q",
							i, actual[i], tt.expectedSelfConditions[i])
					}
				}
			}
			selfConditions = selfConditionsApplicableForConditionName(
				tt.conditionType, lg.TargetNode("target").LicenseConditions(), true)
			actual = actual[:0]
			for name, origins := range selfConditions.conditions {
				actual = append(actual, name)
				for _, payload := range origins {
					if true != payload {
						t.Errorf("unexpected payload: got %v, want true", payload)
					}
				}
			}
			sort.Strings(actual)
...
func TestPolicy_unknownSelfConditionType(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Errorf("expected error: want panic, got no panic")
		}
	}()

	lg := newLicenseGraphImp()
	_ = newTestNode(lg, "target")
	lg.targets["target"].LicenseConditions = append(lg.targets["target"].LicenseConditions, "notice", "restricted")
	_ = selfConditionsApplicableForConditionName("unknown", lg.TargetNode("target").LicenseConditions(), false)
}
