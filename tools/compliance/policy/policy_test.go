	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.otherCondition) > 0 {
				...
				otn.resolutions[0] = LicenseConditionSet(RecognizedConditionNames[otherCondition])
			}
			...
			if tt.expectedDepActions != nil {
				t.Run("depActionsApplicableToTarget", func(t *testing.T) {
					...
					csActual := depConditionsApplicableToTarget(lg, edge, depConditions, tt.treatAsAggregate)
			...
			if tt.expectedTargetConditions != nil {
				t.Run("targetConditionsApplicableToDep", func(t *testing.T) {
					...
					if otherTarget != "" {
						targetConditions = targetConditions.Union(otn.resolutions[0])
					}
