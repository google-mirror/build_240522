...
type EdgeContext interface {
	Context(path TargetEdgePath, edge *TargetEdge) interface{}
}

type NoEdgeContext struct{}

func (ctx NoEdgeContext) Context(path TargetEdgePath, edge *TargetEdge) interface{} {
	return nil
}
...
func (ctx ApplicableConditionsContext) Context(path TargetEdgePath, edge *TargetEdge) interface{} {
	universe := ctx.universe
	if len(path) > 0 {
		universe = path[len(path)-1].ctx.(LicenseConditionSet)
	}
	return applicableConditions(ctx.lg, edge, universe)
}
	...
	walk = func(fnode *TargetNode) {
		...
		for _, edge := range fnode.edges {
			...
			if ctx == nil {
				...
				edgeContext = ctx.Context(*path, edge)
			}
...
func WalkResolutionsForCondition(lg *LicenseGraph, rs *ResolutionStep, conditions LicenseConditionSet) ResolutionSet {
	...
	WalkTopDown(ApplicableConditionsContext{lg, conditions}, lg, func(lg *LicenseGraph, tn *TargetNode, path TargetEdgePath) bool {
		universe := conditions
		if len(path) > 0 {
			universe = path[len(path)-1].ctx.(LicenseConditionSet)
		}
		...
		treatAsAggregate := tn.IsContainer()
		if treatAsAggregate {
			for _, p := range path {
				if !p.Target().IsContainer() {
		...
		rmap[tn][tn] = tn.resolutions[rs.index]
		cmap[tn] = universe
		cs := tn.resolutions[rs.index]
		if !cs.IsEmpty() {
			cs = cs.Intersection(universe)
			pure := true
			for _, p := range path {
				target := p.Target()
				rmap[target][tn] = tn.resolutions[rs.index]
				if pure && !cs.IsEmpty() {
					if _, ok := result[target]; !ok {
						result[target] = make(ActionSet)
					}
...
// resolving all distributed works for `conditions`.
func WalkActionsForCondition(lg *LicenseGraph, rs *ResolutionStep, conditions LicenseConditionSet) ActionSet {
	shipped := ShippedNodes(lg)
	...
	WalkTopDown(ApplicableConditionsContext{lg, conditions}, lg, func(lg *LicenseGraph, tn *TargetNode, path TargetEdgePath) bool {
		...
		cs := universe.Intersection(tn.resolutions[rs.index])
		if !cs.IsEmpty() {
			amap[tn] = cs
		}
		return true
