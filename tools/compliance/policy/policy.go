...
func depActionsApplicableToTarget(e targetEdgeImp, depActions actionSet, treatAsAggregate bool) actionSet {
	result := make(actionSet)
	if e.isDerivation() {
		result.addSet(depActions)
		for _, cs := range depActions.byName(ImpliesRestricted) {
			result.add(e.e.target, cs)
		}
		return result
	}
	if !e.isDynamicLink() {
	...
	for actsOn, cs := range restricted {
		for _, lc := range cs.AsList() {
			...
			if hasOther || hasGpl {
				result.addCondition(actsOn, lc.(licenseConditionImp))
				result.addCondition(e.e.target, lc.(licenseConditionImp))
				continue
			}
