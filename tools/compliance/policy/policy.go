	...
	for actsOn, cs := range restricted {
		for _, lc := range cs.AsList() {
			...
			if hasOther || hasGpl {
				...
				result.addCondition(e.e.target, lc)
				continue
			}
...
func edgeIsDynamicLink(e *dependencyEdge) bool {
	_, isPresent := e.annotations.annotations["dynamic"]
	return isPresent
}
...
func edgeIsDerivation(e *dependencyEdge) bool {
	_, isDynamic := e.annotations.annotations["dynamic"]
	_, isToolchain := e.annotations.annotations["toolchain"]
