...
func rblfError(th *starlark.Thread, b *starlark.Builtin, args starlark.Tuple,
	kwargs []starlark.Tuple) (starlark.Value, error) {
	var message string
	if err := starlark.UnpackPositionalArgs(b.Name(), args, kwargs, 1, &message); err != nil {
		return starlark.None, err
	}
	return starlark.None, &starlark.EvalError{Msg: message}
}
...
func Run(filename string, src interface{}, env []string) error {
	...
	builtins = starlark.StringDict{
		"appendTo":         starlark.NewBuiltin("appendTo", appendTo), // TODO(asmundak): remove
		...
		"rblf_error":       starlark.NewBuiltin("rblf_error", rblfError),
		"rblf_file_exists": starlark.NewBuiltin("rblf_file_exists", fileExists),
		"rblf_wildcard":    starlark.NewBuiltin("rblf_wildcard", wildcard),
		"setFinal":         starlark.NewBuiltin("setFinal", setFinal), // TODO(asmundak): remove
		"set":              starlark.NewBuiltin("set", set),           // TODO(asmundak): remove
	}
