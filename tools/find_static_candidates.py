  ...
  def flattenDeps(self, module, module_info):
    ...
    for lib in static:
      if not lib or lib not in module_info:
        continue
      if lib in self.visited:
        self.extendUnique(module["static_libs"], self.visited[lib].static)
      else:
        res = self.flattenDeps(module_info[lib], module_info)
...
def main():
  ...
  includedStatically = defaultdict(IncludedLib)
  includedSharedly = defaultdict(IncludedLib)
  ...
  for name, module in module_info.items():
    ...
    for lib in module["shared_libs"]:
      includedSharedly[lib].count += 1
      includedSharedly[lib].parents.add(name)
    for lib in module["static_libs"]:
      includedStatically[lib].count += 1
      includedStatically[lib].parents.add(name)
  ...
  if args.print_shared:
    ...
    for name, lib in includedSharedly.items():
      if lib.count < MAX_SHARED_INCLUSIONS:
        print("{}: {} included by: {}".format(name, lib.count, lib.parents))
  ...
  if args.print_static:
    ...
    for name, lib in includedStatically.items():
      if lib.count > MIN_STATIC_INCLUSIONS:
        print("{}: {} included by: {}".format(name, lib.count, lib.parents))
  ...
  if args.module:
    ...
    print(
        "{} is included in shared_libs {} times by these modules: {}".format(
            args.module, includedSharedly[args.module].count,
            includedSharedly[args.module].parents
        )
    )
    print(
        "{} is included in static_libs {} times by these modules: {}".format(
            args.module, includedStatically[args.module].count,
            includedStatically[args.module].parents
        )
    )
