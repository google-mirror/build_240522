...
func TestMetadataNegativeCase(t *testing.T) {
	...
	expectedError := "error: Conflicting TrendyTeamIds and Locations for Test target: java-test-module-name-one. Team ID 1: 12346, Location 1: Android.bp. Team ID 2: 12345, Location 2: Android.bp"
	if !strings.Contains(strings.TrimSpace(string(stderr)), strings.TrimSpace(expectedError)) {
		t.Errorf("Unexpected error message. Expected to contain: %s, Got: %s", expectedError, stderr)
	}
}
