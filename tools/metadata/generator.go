		...
		if loaded {
			...
			for _, existing := range existingMetadata {
				if metadata.GetTrendyTeamId() != existing.GetTrendyTeamId() {
					errCh <- fmt.Errorf("error: Conflicting TrendyTeamIds and Locations for Test target: %s. Team ID 1: %s, Location 1: %s. Team ID 2: %s, Location 2: %s", key, metadata.GetTrendyTeamId(), metadata.GetPath(), existing.GetTrendyTeamId(), existing.GetPath())
					lock.Unlock()
					return
				}
