...
// A tool which dumps the interfaces of .dex files.
//
// This only dumps information which can be used by compilation of
// other .java files. When interfaces dumped by this tool was not
// changed, you don't need to re-compile .java files which depend on
// the .dex file.
//
// This is similar to "javap -constants" for .jar files. An important
// difference is this tool doesn't output values of non-static
// final fields. This is OK because jack doesn't expand non-static
// final values unlike javac.
...
void DumpField(const DexFile& dex_file, uint32_t idx, uint32_t flags,
  ...
  // !HasNext() means remaining field values are zero.
  if (static_field_values && static_field_values->HasNext()) {
    ...
    switch (static_field_values->GetValueType()) {
      ...
      case art::EncodedStaticFieldValueIterator::kString:
        printf(" = \"%s\"", dex_file.GetStringData(dex_file.GetStringId(v.i)));
        break;
      ...
      default:
        LOG_ALWAYS_FATAL("Unexpected static field type");
    }
...
void DumpClassData(const DexFile& dex_file,
  ...
  LOG_ALWAYS_FATAL_IF(static_field_values.HasNext());
