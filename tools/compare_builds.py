...
"""
  ...
  for files that have changed or have improperly have been regenerated, updating
...
# Which directories to use if no --dirs argument is supplied
DEFAULT_DIRS = (
    "apex",
    "data",
    "product",
...
DIFF_MODE = DiffType("DIFF_MODE", "Other mode bits differ")
...
def main():
  ...
  argparser.add_argument("--touch", nargs="+",
...
def AssertAtTop():
  """If the current directory not the top of an android source tree, print an error
     message and exit."""
...
def RunSoong(out_dir, args, capture_output=False):
  env = dict(os.environ)
  env["OUT_DIR"] = out_dir
  return subprocess.run([SOONG_UI,] + args, env=env, capture_output=capture_output)

def GetBuildVar(out_dir, name):
  """Gets a variable from the build system."""
  result = RunSoong(out_dir, ["--dumpvar-mode", name], capture_output=True)
...
def RequireBuildVar(out_dir, name):
  """Gets a variable from the builds system. If that fails, then print an error
     message and exit."""
  result = GetBuildVar(out_dir, name)
  if not result:
    sys.stderr.write("error: Can't determine %s. Please run lunch first.\n" % name)
    sys.exit(1)
  return result
...
def Clean(directory):
  """Runs a build, and returns the files that were requested."""
  try:
...
def FindOutFilesTouchedAfter(files, timestamp):
  """Find files in the out directory that were touched after timestamp.
  Skips files that are known to """
  result = []
