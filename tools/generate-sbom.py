...
def new_package_record(id, name, download_location, version, supplier, files_analyzed='false'):
  return {
      PACKAGE_NAME: name,
      SPDXID: id,
      PACKAGE_DOWNLOAD_LOCATION: download_location,
...
def get_sbom_fragments(module_path):
  ...
  if module_path.startswith('external/') and module_path.find('chromium-webview') == -1:
    ...
    source_package = new_package_record(source_package_id,
                                        name,
                                        'Not provided',
                                        args.build_version,
                                        args.product_mfr)
    ...
    upstream_package = new_package_record(upstream_package_id,
                                          name,
                                          'Not provided',
                                          args.build_version,
                                          args.product_mfr)
    ...
    prebuilt_package = new_package_record(prebuilt_package_id,
                                          name,
                                          'Not provided',
                                          args.build_version,
                                          args.product_mfr)
...
def main():
  ...
  product_package = new_package_record(product_package_id,
                                       'PRODUCT',
                                       '<Not provided>',
                                       args.build_version,
                                       args.product_mfr,
  ...
  platform_package = new_package_record(platform_package_id,
                                        'PLATFORM',
                                        '<Not provided>',
                                        args.build_version,
                                        args.product_mfr)
