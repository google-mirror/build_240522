...
def is_soong_prebuilt_module(file_metadata):
  return file_metadata['soong_module_type'] and file_metadata['soong_module_type'] in [
      'android_app_import', 'android_library_import', 'cc_prebuilt_binary', 'cc_prebuilt_library', 'cc_prebuilt_library_headers',
      'cc_prebuilt_library_shared', 'cc_prebuilt_library_static', 'cc_prebuilt_object', 'dex_import', 'java_import',
      'java_sdk_library_import', 'java_system_modules_import', 'libclang_rt_prebuilt_library_static', 'libclang_rt_prebuilt_library_shared',
      'llvm_prebuilt_library_static', 'ndk_prebuilt_object', 'ndk_prebuilt_shared_stl', 'nkd_prebuilt_static_stl',
      'prebuilt_apex', 'prebuilt_bootclasspath_fragment', 'prebuilt_dsp', 'prebuilt_firmware', 'prebuilt_kernel_modules',
      'prebuilt_rfsa', 'prebuilt_root', 'rust_prebuilt_dylib', 'rust_prebuilt_library', 'rust_prebuilt_rlib', 'vndk_prebuilt_shared',
...
def is_prebuilt_package(file_metadata):
  module_path = file_metadata['module_path']
  if module_path:
    return module_path.startswith('prebuilts/') or is_soong_prebuilt_module(file_metadata)
...
def installed_file_has_metadata(installed_file_metadata, report):
  ...
  is_build_prop = installed_file_metadata['is_build_prop']
  is_notice_file = installed_file_metadata['is_notice_file']

  if (not module_path and
      not product_copy_files and
      not kernel_module_copy_files and
      not is_build_prop and
      not is_notice_file and
...
def main():
  ...
  with open(args.metadata, newline='') as sbom_metadata_file:
    ...
    for installed_file_metadata in reader:
      ...
      elif module_path or installed_file_metadata['is_build_prop'] or installed_file_metadata['is_notice_file']:
