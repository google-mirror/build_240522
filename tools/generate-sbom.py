...
def new_doc_header(doc_id):
  return {
      ...
      DOCUMENT_NAMESPACE: '<document namespace here>',
...
def new_package_record(id, name, version, supplier, download_location=None, files_analyzed='false'):
  package = {
      PACKAGE_NAME: name,
      SPDXID: id,
...
def get_source_package_name(file_metadata, metadata_file_path):
  if not metadata_file_path:
    return file_metadata['module_path']

  metadata_proto = metadata_file_protos[metadata_file_path]
  if metadata_proto.name:
    return metadata_proto.name
  else:
    return os.path.basename(metadata_file_path)  # return the directory name only
...
def get_sbom_fragments(installed_file_metadata, metadata_file_path):
  ...
  if is_source_package(installed_file_metadata):
    ...
    name = get_source_package_name(installed_file_metadata, metadata_file_path)
    ...
    if r.__class__.__name__ == 'dict':
      if DOCUMENT_NAME in r:  # Doc header
        ...
        for ref in r[EXTERNAL_DOCUMENT_REF]:
          ...
          doc['externalDocumentRefs'].append(docRef)
