...
def new_doc_ref(package_name, sbom_url, sbom_checksum):
  return 'DocumentRef-{}-{} {} {}'.format(PKG_UPSTREAM, encode_for_spdxid(package_name), sbom_url, sbom_checksum)
...
def get_package_homepage(metadata_proto):
...
def get_sbom_fragments(installed_file_metadata, metadata_file_path):
  ...
  if metadata_file_path:
    ...
    name_in_metadata_file = metadata_proto.name
    homepage = get_package_homepage(metadata_proto)
    version = metadata_proto.third_party.version
  ...
  if is_source_package(installed_file_metadata):
    ...
    upstream_package = new_package_record(upstream_package_id, name, version, homepage)
    ...
    if metadata_file_path:
      ...
      if metadata_proto.third_party.WhichOneof('sbom') == 'sbom_ref':
        ...
        if sbom_url and sbom_checksum and upstream_element_id:
          external_doc_ref = new_doc_ref(name, sbom_url, sbom_checksum)
          relationships.append(new_relationship_record(prebuilt_package_id, REL_VARIANT_OF, external_doc_ref+':'+upstream_element_id))
...
def report_metadata_file(metadata_file_path, installed_file_metadata, report):
  if metadata_file_path:
    ...
    if not metadata_file_path in metadata_file_protos:
      ...
      # if package_metadata.version and not package_metadata.third_party.version:
      #   report[ISSUE_METADATA_FILE_INCOMPLETE].append(
      #       '{} should use "third_party.version" instead of "version"'.format(metadata_file_path + '/METADATA'))
  else:
