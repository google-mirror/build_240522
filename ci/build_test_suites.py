...
def find_changed_files(change) -> Set[Text]:
...
def read_modules_from_test_mappings(test_mappings) -> Set[Text]:
  module_list = set()
  ...
  for test_mapping in test_mappings.values():
    for suite in test_mapping.values():
      for element in suite:
        if 'name' in element.keys() and 'host' in element.keys():
          module_list.add(element.get('name'))

  return module_list

def get_modules_to_build(change_info: Text) -> Set[Text]:
  ...
  for change in change_info_contents['changes']:
    changed_files.update(find_changed_files(change))

  test_mappings = GetTestMappings(changed_files, set())

  modules_to_build = set(REQUIRED_MODULES)

  modules_to_build.update(read_modules_from_test_mappings(test_mappings))
...
def build_test_suites(argv):
  ...
  modules_to_build = get_modules_to_build(args.change_info)
  ...
  build_command = ['time', os.path.join(host_out, 'soong_zip')]
