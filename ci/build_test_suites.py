...
REQUIRED_MODULES = frozenset(['cts-tradefed', 'vts-tradefed', 'compatibility-host-util', 'soong_zip'])
...
def build_affected_modules(args: argparse.Namespace):
  modules_to_build = find_modules_to_build(pathlib.Path(args.change_info), args.extra_required_modules)
...
def find_modules_to_build(change_info: pathlib.Path, extra_required_modules: list[Text]) -> Set[Text]:
...
def find_affected_modules(test_mappings: Dict[str, Any], changed_files: Set[Text]) -> Set[Text]:
  modules = set()
...
def matches_file_patterns(file_patterns: list[Text], changed_files: Set[Text]) -> bool:
...
def get_soong_var(var: str) -> str:
  value = run_command(['./build/soong/soong_ui.bash', '--dumpvar-mode', '--abs', var]).strip()
  if not value:
