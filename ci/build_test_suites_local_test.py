...
class BuildTestSuitesLocalTest(ci_test_lib.TestCase):
  ...
  def test_can_interrupt_build(self):
    ...
    p = self._managed_subprocess(*args, **kwargs)
  ...
  def _managed_subprocess(self, *args, **kwargs) -> subprocess.Popen:
    # We start a new session so that we can kill the entire process group on
    # tear down. Otherwise, we'd create orphans that would hang the test
    # execution.
  ...
  def _terminate_managed_processes(self):
    for p in self._managed_processes:
      if not ci_test_lib.process_alive(p.pid):
        continue
      os.killpg(os.getpgid(p.pid), signal.SIGKILL)
