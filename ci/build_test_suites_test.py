...
class RunCommandIntegrationTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    base_name = 'py3-cmd'
    dest_file = pathlib.Path(self.temp_dir.name, base_name)
    with resources.as_file(resources.files('testdata').joinpath(base_name)) as p:
  ...
  def test_streams_stdout(self):
      marker = 'Spinach'
      stdout_file = pathlib.Path(self.temp_dir.name, 'stdout.txt')

      def run(stdout_file):
          with open(stdout_file, 'w') as f:
            build_test_suites.run_command([
                self.python_executable,
                '-u',  # Force stdout and stdderr streams to be unbuffered.
                '-c',
                textwrap.dedent(f'''\
                import time
      ...
      wait_until(lambda: stdout_file.is_file() and stdout_file.stat().st_size > 0)

      self.assertEquals(marker, read_file_contents(stdout_file))
      p.terminate()  # TODO: Terminate/Finally.

  # @timeout_decorator.timeout(seconds=DEFAULT_TEST_TIMEOUT_SECS)
  def test_propagates_interrupts(self):
      pid_file = pathlib.Path(self.temp_dir.name, 'pid.txt')
  ...
  polling_interval_secs: float = 0.1
    ...
    while not condition_function():
        if time.time() - start_time_secs > timeout_secs:
            raise TimeoutError(f"Condition not met within timeout: {timeout_secs} seconds")
