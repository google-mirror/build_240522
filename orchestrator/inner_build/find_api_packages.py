...
# Default name of the api contribution Bazel target. Can be overriden by module authors in api_packages.json
...
INNER_TREE_SEARCH_DIRS = [("frameworks", "base"), ("packages", "modules")]
...
class ApiPackageNotFoundException(Exception):
    pass
...
def read(filepath: str) -> ContributionData:
    ...
    def _deserialize(filepath, json_contents) ->ContributionData:
        ...
        if not package:
             raise ApiPackageDecodeException(filepath, "api_pacakge is a required field in api_packages.json")
...
class ApiPackageFinder:
    ...
    def _find_api_label(self, api_domain: str) -> BazelLabel:
        ...
        raise ApiPackageNotFoundException(f"Could not find API contributions for API domain: {api_domain}.")
    ...
    def find_api_label_string(self, api_domain: str) -> str:
        ...
        Raises:
            ApiPackageNotFoundException: If api_domain_name could not be found in api_pacakges.json file
        ...
        """
        return self._find_api_label(api_domain).to_string()
    ...
    def find_api_package(self, api_domain_name: str) -> str:
        ...
        Raises:
            ApiPackageNotFoundException: If api_domain_name could not be found in any api_pacakges.json file
            ApiPackageDecodeException: If a malformed api_packages.json is found during search

        Returns:
            Bazel label, e.g. //frameworks/base
        """
        return self._find_api_label(api_domain).pacakge_name
