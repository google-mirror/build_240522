...
NDKSTUBGEN="build/build/orchestrator/core/cc/ndkstubgen.py"
...
class GenCcStubsInput(NamedTuple):
    arch: str # target device arch (e.g. x86)
    version: int # numeric API level (e.g. 33)
    api: str # path to map.txt
...
class StubGenerator:
    ...
    def add_stub_gen_rule(self, ninja: Ninja):
        ...
        Running ndkstubgen create C stubs from API .map.txt files"""
        ...
        rule.add_variable("command", "${ndkstubgen} --arch ${arch} --api ${apiLevel} --api-map ${apiMap} ${in} ${out}")
        ninja.add_rule(rule)
    ...
    def add_stub_gen_action(self, ninja: Ninja, stub_input: GenCcStubsInput,
                            staging_dir: str) -> GenCcStubsOutput:
        ...
        outputs = GenCcStubsOutput(stub_src=os.path.join(staging_dir, "stub.c"),
                                   version_script=os.path.join(staging_dir, "stub.map"),
                                   symbol_list=os.path.join(staging_dir, "abi_symbol_list.txt"))
