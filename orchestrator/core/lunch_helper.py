...
def FindDirs(path, name, ttl=6):
    ...
    with os.scandir(path) as it:
        for dirent in sorted(it, key=lambda x: x.name):
            try:
                if dirent.is_dir():
                    if dirent.name == name:
                        yield os.path.sep.join((path, dirent.name))
                    elif ttl > 0:
    ...
    for subdir in subdirs:
        yield from FindDirs(os.path.sep.join((path, subdir)), name, ttl-1)
...
def WalkPaths(path, matcher, ttl=10):
    ...
    for subdir in sorted(subdirs):
        yield from WalkPaths(os.path.sep.join((path, subdir)), matcher, ttl-1)
...
def FindConfigDirs(workspace_root):
    ...
    yield os.path.sep.join((workspace_root, "build/make/orchestrator/multitree_combos"))

    dirs = ["vendor", "device"]
    for d in dirs:
        yield from FindDirs(os.path.sep.join((workspace_root, d)), "multitree_combos")
...
class ConfigException(Exception):
    ...
    def __init__(self, kind, message, locations, line=0):
        ...
        Args:
            message: Error message to dispaly to user
...
def Lunch(args):
    ...
    if not os.path.exists("build/make/orchestrator"):
        sys.stderr.write("ERROR: lunch_helper.py must be run from the root of a multi-tree"
                            + " workspace\n")
