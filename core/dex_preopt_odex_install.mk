...
$(warning $(LOCAL_DEX_PREOPT_FLAGS))
ifneq (,$(filter $(LOCAL_DEX_PREOPT_FLAGS),--compiler-filter=verify-dont-extract))
# In the case we are preopting the mode that doesn't extract, put the vdex in the a dm file instead
# of installing it normally.
my_generate_dm := true
endif

# No reason to use a dm file if the dex is already uncompressed.
# TODO: The app may get recompiled during boot from a compiler filter upgrade.
ifeq ($(LOCAL_UNCOMPRESS_DEX),true)
my_generate_dm := false
endif  # LOCAL_UNCOMPRESS_DEX
...
$(my_built_dm): $(my_copied_vdex)
	$(hide) mkdir -p $(dir $@)
	$(hide) rm -f $@
	$(hide) zip -qD -j -X -9 $@ $(my_built_dm) $(PRIVATE_INPUT_VDEX)
