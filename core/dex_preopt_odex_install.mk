...
built_odex := $(basename $(LOCAL_BUILT_MODULE))-$(DEX2OAT_TARGET_ARCH).odex
ifdef LOCAL_DEX_PREOPT_IMAGE
my_dex_preopt_image := $(LOCAL_DEX_PREOPT_IMAGE)
else
my_dex_preopt_image := $(DEFAULT_DEX_PREOPT_BUILT_IMAGE)
endif
$(built_odex): PRIVATE_2ND_ARCH_VAR_PREFIX :=
$(built_odex): PRIVATE_DEX_LOCATION := $(patsubst $(PRODUCT_OUT)%,%,$(LOCAL_INSTALLED_MODULE))
$(built_odex): PRIVATE_DEX_PREOPT_IMAGE := $(my_dex_preopt_image)
$(built_odex) : $(DEXPREOPT_ONE_FILE_DEPENDENCY_BUILT_BOOT_PREOPT) \
                $(DEXPREOPT_ONE_FILE_DEPENDENCY_TOOLS) \
                $(my_dex_preopt_image)
installed_odex := $(basename $(LOCAL_INSTALLED_MODULE))-$(DEX2OAT_TARGET_ARCH).odex

# #################################################
# Odex for the 2st arch
ifdef TARGET_2ND_ARCH
ifneq ($(DEX2OAT_TARGET_ARCH),$($(TARGET_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH))
built_odex2 := $(basename $(LOCAL_BUILT_MODULE))-$($(TARGET_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH).odex
ifdef LOCAL_DEX_PREOPT_IMAGE
my_dex_preopt_image := $(LOCAL_DEX_PREOPT_IMAGE)
else
my_dex_preopt_image := $($(TARGET_2ND_ARCH_VAR_PREFIX)DEFAULT_DEX_PREOPT_BUILT_IMAGE)
endif
...
$(built_odex2): PRIVATE_DEX_PREOPT_IMAGE := $(my_dex_preopt_image)
$(built_odex2) : $($(TARGET_2ND_ARCH_VAR_PREFIX)DEXPREOPT_ONE_FILE_DEPENDENCY_BUILT_BOOT_PREOPT) \
                 $(DEXPREOPT_ONE_FILE_DEPENDENCY_TOOLS) \
                 $(my_dex_preopt_image)

built_odex += $(built_odex2)
installed_odex += $(basename $(LOCAL_INSTALLED_MODULE))-$($(TARGET_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH).odex
endif  # DEX2OAT_TARGET_ARCH isn't the same as $(TARGET_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH
endif  # TARGET_2ND_ARCH
# #################################################
else  # must be APPS
# For an app, we build for the multilib arch it's targeted for.
built_odex := $(basename $(LOCAL_BUILT_MODULE))-$($(LOCAL_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH).odex
ifdef LOCAL_DEX_PREOPT_IMAGE
my_dex_preopt_image := $(LOCAL_DEX_PREOPT_IMAGE)
else
my_dex_preopt_image := $($(LOCAL_2ND_ARCH_VAR_PREFIX)DEFAULT_DEX_PREOPT_BUILT_IMAGE)
endif
...
$(LOCAL_BUILT_MODULE) $(built_odex): PRIVATE_DEX_PREOPT_IMAGE := $(my_dex_preopt_image)
$(LOCAL_BUILT_MODULE) $(built_odex) : $($(LOCAL_2ND_ARCH_VAR_PREFIX)DEXPREOPT_ONE_FILE_DEPENDENCY_BUILT_BOOT_PREOPT) \
                $(DEXPREOPT_ONE_FILE_DEPENDENCY_TOOLS) \
                $(my_dex_preopt_image)

installed_odex := $(basename $(LOCAL_INSTALLED_MODULE))-$($(LOCAL_2ND_ARCH_VAR_PREFIX)DEX2OAT_TARGET_ARCH).odex
...
$(installed_odex) : $(basename $(LOCAL_INSTALLED_MODULE))% : $(basename $(LOCAL_BUILT_MODULE))% \
