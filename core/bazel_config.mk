# BAZEL_PRODUCT_CONFIG_REPOSITORY_DIR is an environment variable passed in by product_config.bzl
BAZEL_VARIABLES := $(BAZEL_PRODUCT_CONFIG_REPOSITORY_DIR)/bazel.variables
...
# Create bazel.variables with copies of makefile settings.  Runs every build,
# but only updates bazel.variables if it changes
$(shell mkdir -p $(dir $(BAZEL_VARIABLES)))
$(call json_start)
...
$(file >$(BAZEL_VARIABLES).tmp,$(json_contents))

$(shell if ! cmp -s $(BAZEL_VARIABLES).tmp $(BAZEL_VARIABLES); then \
	  mv $(BAZEL_VARIABLES).tmp $(BAZEL_VARIABLES); \
	else \
	  rm $(BAZEL_VARIABLES).tmp; \
	fi)

endif # WRITE_BAZEL_VARIABLES
