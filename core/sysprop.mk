...
define generate-common-build-props
    ...
    $(if $(filter system,$(1)),\
      ...
      ,\
        ...
        $(if $(filter vendor,$(1)),\
            # Attestation specific properties for AOSP/GSI build running on device. Intentionally written in vendor
            # file because these variables do not overwrite after flashing GSI image.
            echo "ro.product.$(1).brand_for_attestation=$(PRODUCT_BRAND_FOR_ATTESTATION)" >> $(2);\
            echo "ro.product.$(1).device_for_attestation=$(PRODUCT_DEVICE_FOR_ATTESTATION)" >> $(2);\
            echo "ro.product.$(1).manufacturer_for_attestation=$(PRODUCT_MANUFACTURER_FOR_ATTESTATION)" >> $(2);\
            echo "ro.product.$(1).model_for_attestation=$(PRODUCT_MODEL_FOR_ATTESTATION)" >> $(2);\
            echo "ro.product.$(1).name_for_attestation=$(PRODUCT_NAME_FOR_ATTESTATION)" >> $(2);\
        )\
    )\
