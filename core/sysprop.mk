...
# $(5): list of prop names to force remove from the output. Properties from both
...
$(2): $(POST_PROCESS_PROPS) $(INTERNAL_BUILD_ID_MAKEFILE) $(API_FINGERPRINT) $(3)
	...
	$(hide) mkdir -p $$(dir $$@)
	$(hide) rm -f $$@ && touch $$@
	$(hide) $$(call generate-common-build-props,$(call to-lower,$(1)),$$@)
	$(hide) $(foreach file,$(3),\
	    if [ -f "$(file)" ]; then\
	        echo "" >> $$@;\
	        echo "####################################" >> $$@;\
...
$(eval $(call build-properties,system,$(TARGET_OUT)/build.prop,\
...
$(eval $(call build-properties,vendor,$(TARGET_OUT_VENDOR)/build.prop,\
$(_prop_files_),$(_prop_vars_),\
...
product_prop_file := $(TARGET_PRODUCT_PROP)
else
product_prop_file := $(wildcard $(TARGET_DEVICE_DIR)/product.prop)
endif

_prop_files_ := $(product_prop_file)
# Order matters here. When there are duplicates, the last one wins.
# TODO(b/117892318): don't allow duplicates so that the ordering doesn't matter
_prop_vars_ := \
...
$(eval $(call build-properties,product,$(TARGET_OUT_PRODUCT)/build.prop,\
$(_prop_files_),$(_prop_vars_),\
$(empty)))
...
odm_prop_file := $(TARGET_ODM_PROP)
else
odm_prop_file := $(wildcard $(TARGET_DEVICE_DIR)/odm.prop)
endif

_prop_files_ := $(odm_prop_file)
# Order matters here. When there are duplicates, the last one wins.
# TODO(b/117892318): don't allow duplicates so that the ordering doesn't matter
_prop_vars_ := \
...
$(eval $(call build-properties,odm,$(TARGET_OUT_ODM)/build.prop,\
$(_prop_files),$(_prop_vars_),\
$(empty)))
...
system_ext_prop_file := $(TARGET_SYSTEM_EXT_PROP)
else
system_ext_prop_file := $(wildcard $(TARGET_DEVICE_DIR)/system_ext.prop)
endif

_prop_files_ := $(system_ext_prop_file)
# Order matters here. When there are duplicates, the last one wins.
# TODO(b/117892318): don't allow duplicates so that the ordering doesn't matter
_prop_vars_ := PRODUCT_SYSTE_EXT_PROPERTIES

$(eval $(call build-properties,system_ext,$(TARGET_OUT_SYSTEM_EXT)/build.prop,\
$(_prop_files_),$(_prop_vars_),\
$(empty)))
