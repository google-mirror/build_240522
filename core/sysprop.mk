...
INSTALLED_BUILD_PROP_TARGET := $(TARGET_OUT)/etc/build.prop

$(eval $(call build-properties,system,$(INSTALLED_BUILD_PROP_TARGET),\
$(_prop_files_),$(_prop_vars_),\
$(_blacklist_names_)))

# TODO(jiyong): remove this. For now, system/build.prop exists because the path is used
# by some of the build tools and apps (although the file is not part of the formal API)
INSTALLED_BUILD_PROP_LEGACY_TARGET := $(TARGET_OUT)/build.prop
$(INSTALLED_BUILD_PROP_LEGACY_TARGET): $(INSTALLED_BUILD_PROP_TARGET)
	$(copy-file-to-target)
ALL_DEFAULT_INSTALLED_MODULES += $(INSTALLED_BUILD_PROP_LEGACY_TARGET)
...
INSTALLED_VENDOR_BUILD_PROP_TARGET := $(TARGET_OUT_VENDOR)/etc/build.prop
