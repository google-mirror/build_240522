...
###########################################################
## Generate colored warning or error message with the make
## file name and module name that caused the warning/error.
##
## $(1): message to print
## $(2): keep_going. if set to "true", build is not stopped.
##       default to true for log-warning, false for log-error
###########################################################
define log-warning
$(eval \
$(shell echo -ne "\e[1m"$(LOCAL_MODULE_MAKEFILE):$(LOCAL_MODULE):"\e[0m " >&2) \
$(shell echo -ne "\e[1;35mwarning:\e[0m " >&2) \
$(shell echo -e  "\e[1m"$(1)"\e[0m" >&2) \
$(if $(filter false,$(2)),$(error done),)\
)
endef
...
$(eval \
$(shell echo -ne "\e[1m"$(LOCAL_MODULE_MAKEFILE):$(LOCAL_MODULE):"\e[0m " >&2) \
$(shell echo -ne "\e[1;31merror:\e[0m " >&2) \
$(shell echo -e  "\e[1m"$(1)"\e[0m" >&2) \
$(if $(filter true,$(2)),,$(error done))\
)
endef
