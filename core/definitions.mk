...
$(strip $(foreach target, $(sort $(1)), \
  $(if $(strip $(ALL_MODULES.$(target).META_LIC)), \
    ...
    $(if $(strip $(ALL_TARGETS.$(target).META_LIC)), \
      ...
      $(call append-path,$(call license-metadata-dir),$(target).meta_lic)))))
endef
...
$(strip $(eval _meta := $(call append-path,$(_dir),$(_tgt).meta_lic)))
...
reportallnoticelibrarynames: PRIVATE_LIST_FILE := $(call append-path,$(call license-metadata-dir),filelist)
...
$(strip \
  ...
  $(foreach t,$(sort $(ALL_NON_MODULES)), \
    $(eval ALL_TARGETS.$(t).META_LIC := $(call append-path,$(_dir),$(t).meta_lic)) \
  ) \
