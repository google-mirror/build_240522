...
# Emit each installed notice file if it has not already been emitted.
$(if $(strip $(_infs)),$(foreach mif,$(_mifs),
$(if $(filter $(mif),$(ALL_MODULE_INSTALLED_FILENAMES)),

$(filter %/src/$(mif).txt,$(_infs)) : $(_dir)/$(1).meta_lic
$(filter %/src/$(mif).txt,$(_infs)): PRIVATE_INSTALLED_MODULE := $(mif)
$(filter %/src/$(mif).txt,$(_infs)) : PRIVATE_NOTICES := $(_notices)

$(filter %/src/$(mif).txt,$(_infs)): $(_notices)
	@echo Notice file: $$< -- $$@
	mkdir -p $$(dir $$@)
	awk 'FNR==1 && NR > 1 {print "\n"} {print}' $$(PRIVATE_NOTICES) > $$@

$(strip $(eval ALL_MODULE_INSTALLED_FILENAMES := $(filter-out $(mif),$(ALL_MODULE_INSTALLED_FILENAMES))))
),$(warning $(mif) redefined in $(1))))
