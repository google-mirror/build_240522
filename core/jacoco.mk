...
$(my_instrumented_timestamp_path): $(my_unzipped_timestamp_path) $(JACOCO_CLI_JAR)
  ...
  my_temp_jar_path_leaf := usable.jar
  my_temp_jar_path_stem := $(my_files)/work
  LOCAL_FULL_CLASSES_JACOCO_JAR := $(intermediates.COMMON)/classes-jacoco.jar
$(LOCAL_FULL_CLASSES_JACOCO_JAR): PRIVATE_TEMP_JAR_PATH_LEAF := $(my_temp_jar_path_leaf)
$(LOCAL_FULL_CLASSES_JACOCO_JAR): PRIVATE_TEMP_JAR_PATH_STEM := $(my_temp_jar_path_stem)
$(LOCAL_FULL_CLASSES_JACOCO_JAR): PRIVATE_TEMP_JAR_PATH := $(my_temp_jar_path_stem)/$(my_temp_jar_path_leaf)
$(LOCAL_FULL_CLASSES_JACOCO_JAR): PRIVATE_INSTRUMENTED_PATH := $(my_instrumented_path)
$(LOCAL_FULL_CLASSES_JACOCO_JAR): PRIVATE_INSTRUMENTED_TIMESTAMP_PATH := $(my_instrumented_timestamp_path)
...
$(LOCAL_FULL_CLASSES_JACOCO_JAR): $(my_instrumented_timestamp_path) $(LOCAL_FULL_CLASSES_PRE_JACOCO_JAR)
	...
	# copy instrumented files back into the jacoco jar
	cd $(PRIVATE_TEMP_JAR_PATH_STEM) \
	&& JarPath=`pwd`/$(PRIVATE_TEMP_JAR_PATH_LEAF) \
	&& cd - \
	&& cd $(PRIVATE_INSTRUMENTED_PATH) \
	&& zip -r -q $$JarPath .
