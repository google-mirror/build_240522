...
# Find duplicate shared libraries by md5 checksum.

OUT_DIR="$1"
TMP_MD5="${OUT_DIR}/_dup_md5"
TMP_SIZE="${OUT_DIR}/_dup_size"
TMP_CHECK="${OUT_DIR}/_dup_sanity_check"
TMP_SIZE_REAL="${OUT_DIR}/_dup_size_real"
TMP_FILE1="${OUT_DIR}/_dup_f1"
TMP_FILE2="${OUT_DIR}/_dup_f2"
MD5_DUPLICATES="${OUT_DIR}/duplicate-libs-md5.txt"
SIZE_DUPLICATES="${OUT_DIR}/duplicate-libs-size.txt"
...
find ./"${OUT_DIR}"/system/ -name "*.so" -type f -print0 | xargs -0 md5sum | sed -e "s# .*/# #" | sort | uniq -c | sort -g | sed "/^.*1 /d" | sed "s/^. *[0-9] //" > "${TMP_MD5}"
...
if [ -s "${TMP_MD5}" ]; then
	while read -r list; do
		...
		{
			echo "MD5: ${checksum}";											\
			find ./"${OUT_DIR}"/system/ -name "${filename}" -type f -print0 | xargs -0 md5sum | grep "${checksum}" | sed 's/^.* //';	\
			echo "";													\
		} >> "${MD5_DUPLICATES}"
	done <"${TMP_MD5}"
else
...
find ./"${OUT_DIR}"/system/ -name "*.so" -type f -print0 | xargs -0 stat --format="%s %n" | sed -e "s# .*/# #" | sort | uniq -c | sort -g | sed "/^.*1 /d" > "${TMP_SIZE}"
if [ -s "${TMP_SIZE}" ]; then
	while read -r list; do
		size=$(echo "${list}" | cut -f2 -d ' ')
		filename=$(echo "${list}" | cut -f3 -d ' ')
		# Sanity check - see if the files are not in the md5sum list and do nothing if that is the case.
		find ./"${OUT_DIR}"/system/ -name "${filename}" -type f -print0 | xargs -0 stat --format="%s %n" | grep "${size}" | sed "s/^.* //" | sort > "${TMP_CHECK}"
		...
		if [ -s "${TMP_SIZE_REAL}" ]; then
			...
			if [ "${count}" = 2 ]; then
				...
				# Check .note section
				${BINUTILS_PATH}/llvm-readelf --wide --notes "${file1}" > "${TMP_FILE1}"
				${BINUTILS_PATH}/llvm-readelf --wide --notes "${file2}" > "${TMP_FILE2}"
				{
					diff -u "${TMP_FILE1}" "${TMP_FILE2}" | sed "1d;2d;3d";	\
					echo "";
				} >> "${SIZE_DUPLICATES}"
				# Check .text section
				${BINUTILS_PATH}/llvm-objdump --line-numbers --disassemble --demangle --reloc --no-show-raw-insn --section=.text "${file1}" | sed "1d;2d"> "${TMP_FILE1}"
				${BINUTILS_PATH}/llvm-objdump --line-numbers --disassemble --demangle --reloc --no-show-raw-insn --section=.text "${file2}" | sed "1d;2d"> "${TMP_FILE2}"
				{
					diff -u "${TMP_FILE1}" "${TMP_FILE2}" | sed "1d;2d;3d";	\
					echo "";
				} >> "${SIZE_DUPLICATES}"
