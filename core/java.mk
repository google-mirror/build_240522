...
my_proguard_libraryjars :=
ifdef LOCAL_SDK_VERSION
ifdef TARGET_BUILD_APPS
# An app that doesn't target the "current" SDK may still contain references to
# the current SDK (references may be added by support library or by runtime
# classes added by desugar). So in that case we artifically raise the
# "SDK version" "linked" by ProGuard, to
# - suppress ProGuard warnings of referencing symbols unknown to the lower SDK version.
# - prevent ProGuard stripping subclass in the support library that extends class added in the higher SDK version.
# See b/20667396
ifeq (,$(filter current system_current test_current, $(LOCAL_SDK_VERSION)))
  my_proguard_libraryjars := $(call java-lib-header-files, sdk_vcurrent)
endif
else
  # A module in the platform build may also depend on the "current" SDK,
  # but we we can't just raise to the "current" SDK for platform modules,
  # because that would break apps that use APIs removed from the current SDK.
  # Instead we add some extra dependencies
  ...
  my_proguard_libraryjars := $(call java-lib-header-files,$(my_modules_to_raise))
endif
endif

legacy_proguard_flags := $(addprefix -libraryjars ,$(my_proguard_libraryjars) \
  $(filter-out $(my_proguard_libraryjars), \
    $(full_java_bootclasspath_libs) \
    $(full_shared_java_header_libs)))

legacy_proguard_lib_deps := $(my_proguard_libraryjars) \
  $(filter-out $(my_proguard_libraryjars),$(full_shared_java_header_libs))
...
$(full_classes_proguard_jar) : $(full_classes_pre_proguard_jar) $(extra_input_jar) $(my_proguard_libraryjars) $(common_proguard_flag_files) $(proguard_flag_files) $(legacy_proguard_lib_deps) | $(PROGUARD)
