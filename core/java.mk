...
ifneq ($(LOCAL_JAVAC_INPUT_SOURCES_NUM),)
ifneq ($(LOCAL_JAR_PROCESSOR),)
$(error "Javac sharding won't work with compilations that use code-generating annotation processors!")
endif
# 0 < $(LOCAL_JAVAC_INPUT_SOURCES_NUM) <= 8000
$(call int_check_valid,$(LOCAL_JAVAC_INPUT_SOURCES_NUM))
# 0 < $(java_sources) <= 8000
$(call int_check_valid,$(words $(java_sources)))

splits := $(call int_divide,$(call int_encode,$(words $(java_sources))),$(call int_encode,$(LOCAL_JAVAC_INPUT_SOURCES_NUM)))
ifneq ($(words $(java_sources)),$(words $(call int_multiply,$(call int_encode,$(LOCAL_JAVAC_INPUT_SOURCES_NUM)),$(splits))))
  splits += x
endif
...
ifneq ($(LOCAL_JAVAC_INPUT_SOURCES_NUM),)
ifneq ($(LOCAL_JAR_PROCESSOR),)
$(error "Javac sharding won't help with compilations that use code-generating annotation processors!")
endif
# 0 < $(LOCAL_JAVAC_INPUT_SOURCES_NUM) <= 8000
$(call int_check_valid,$(LOCAL_JAVAC_INPUT_SOURCES_NUM))
# 0 < $(java_sources) <= 8000
...
$(foreach x,$(shards),\
  $(eval $(call shard-java-source-list,$(x),\
    $(wordlist $(words $(call int_plus,$(call int_encode,1),\
      $(call int_multiply,$(call int_encode,$(LOCAL_JAVAC_INPUT_SOURCES_NUM)),\
        $(call int_subtract,$(call int_encode,$(x)),\
          $(call int_encode,1))))),\
            $(words $(call int_multiply,$(call int_encode,$(LOCAL_JAVAC_INPUT_SOURCES_NUM)),$(call int_encode,$(x)))),\
              $(sort $(java_sources))))))
...
$(foreach x,$(shards),\
  $(eval $(call create-classes-full-debug.jar,$(x),$(full_java_header_libs) $(full_classes_header_jar))))
