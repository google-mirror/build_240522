...
ifeq ($(LOCAL_PROTOC_OPTIMIZE_TYPE),micro)
    ifneq ($(filter libprotobuf-java-2.3.0-micro,$(LOCAL_JAVA_LIBRARIES)),)
        $(warning Stripping unneeded dependency on libprotobuf-java-2.3.0-micro in $(LOCAL_MODULE))
        my_static_libraries := $(filter-out libprotobuf-java-2.3.0-micro,$(my_static_libraries))
    endif
    LOCAL_STATIC_JAVA_LIBRARIES += libprotobuf-java-micro
else
  ifeq ($(LOCAL_PROTOC_OPTIMIZE_TYPE),nano)
    ifneq ($(filter libprotobuf-java-2.3.0-nano,$(LOCAL_JAVA_LIBRARIES)),)
        $(warning Stripping unneeded dependency on libprotobuf-java-2.3.0-nano in $(LOCAL_MODULE))
        my_static_libraries := $(filter-out libprotobuf-java-2.3.0-nano,$(my_static_libraries))
    endif
    LOCAL_STATIC_JAVA_LIBRARIES += libprotobuf-java-nano
  else
    ifneq ($(filter libprotobuf-java-2.3.0-lite,$(LOCAL_JAVA_LIBRARIES)),)
        $(warning Stripping unneeded dependency on libprotobuf-java-2.3.0-lite in $(LOCAL_MODULE))
        my_static_libraries := $(filter-out libprotobuf-java-2.3.0-lite,$(my_static_libraries))
    endif
