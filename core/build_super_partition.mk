...
$(INSTALLED_$(LOCAL_SUPER_PARTITION_NAME_UPPER)IMAGE_TARGET):
	$(call pretty,"Target $(LOCAL_SUPER_PARTITION_NAME) fs image: $@")
...
$(INSTALLED_$(LOCAL_SUPER_PARTITION_NAME_UPPER)IMAGE_EMPTY_TARGET):
	$(call pretty,"Target empty $(LOCAL_SUPER_PARTITION_NAME) fs image: $@")
...
$(LOCAL_CHECK_RULE_NAME): PRIVATE_NAME := $(LOCAL_SUPER_PARTITION_NAME_UPPER)

$(LOCAL_CHECK_RULE_NAME):
	partition_size_list="$(call read-size-of-partitions,$(BOARD_$(PRIVATE_NAME)_PARTITION_PARTITION_LIST))"; \
	sum_sizes_expr=$$(sed -e 's/ /+/g' <<< "$${partition_size_list}"); \
	max_size_tail=$(if $(filter true,$(AB_OTA_UPDATER))," / 2"); \
	max_size_expr=$(BOARD_$(PRIVATE_NAME)_PARTITION_SIZE)$${max_size_tail}; \
	if [ $$(( $${sum_sizes_expr} )) -gt $$(( $${max_size_expr} )) ]; then \
		echo "The sum of sizes of all logical partitions is larger than BOARD_$(PRIVATE_NAME)_PARTITION_SIZE$${max_size_tail}:"; \
		echo $${sum_sizes_expr} '==' $$(( $${sum_sizes_expr} )) '>' $${max_size_expr} '==' $$(( $${max_size_expr} )); \
		exit 1; \
	else \
		echo "The sum of sizes of all logical partitions is within BOARD_$(PRIVATE_NAME)_PARTITION_SIZE$${max_size_tail}:"; \
		echo $${sum_sizes_expr} '==' $$(( $${sum_sizes_expr} )) '<=' $${max_size_expr} '==' $$(( $${max_size_expr} )); \
	fi
