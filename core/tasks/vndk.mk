...
deps := $(call paths-of-intermediates,$(foreach lib,$(vndk_core_libs),$(lib):$(lib).so),SHARED_LIBRARIES)
...
$(vndk_snapshot_zip): $(deps)
deps :=

deps := $(call paths-of-intermediates,$(foreach lib,$(vndk_sp_libs),$(lib):$(lib).so),SHARED_LIBRARIES)
...
notices := $(call paths-of-notice-files,$(vndk_core_libs),vndk) \
...
ifdef TARGET_2ND_ARCH
deps := $(call paths-of-intermediates,$(foreach lib,$(vndk_core_libs),$(lib):$(lib).so),SHARED_LIBRARIES,true)
...
deps := $(call paths-of-intermediates,$(foreach lib,$(vndk_sp_libs),$(lib):$(lib).so),SHARED_LIBRARIES,true)
...
$(vndk_snapshot_zip): private-copy-vndk-intermediates = \
  $(if $(2),$(strip \
    @mkdir -p $(1) && \
    $(foreach file,$(2), \
      cp $(file) $(call append-path,$(1),$(subst .vendor,,$(notdir $(file)))) && \
    ) \
    true \
  ))
...
$(vndk_snapshot_zip): private-copy-notice-intermediates = \
  $(if $(2),$(strip \
    @mkdir -p $(1) && \
    $(foreach file,$(2), \
      cp $(call word-colon,1,$(file)) $(call append-path,$(1),$(subst .vendor,,$(call word-colon,2,$(file)))) && \
    ) \
    true \
  ))
...
$(vndk_snapshot_zip): $(SOONG_ZIP)
	...
	$(call private-copy-notice-intermediates, \
