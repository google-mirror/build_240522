...
general_tests_test_configs_zip := $(PRODUCT_OUT)/general-tests-test-configs.zip
# Create an artifact to include all jar files in general-tests.
general_tests_jars_zip := $(PRODUCT_OUT)/general-tests-jars.zip
# Create an artifact to include all host shared libraries in general-tests.
general_tests_host_shared_libs_zip := $(PRODUCT_OUT)/general-tests-host-shared-libs.zip

$(general_tests_zip) : PRIVATE_general_tests_list_zip := $(general_tests_list_zip)
$(general_tests_zip) : .KATI_IMPLICIT_OUTPUTS := $(general_tests_list_zip) $(general_tests_test_configs_zip) $(general_tests_jars_zip) $(general_tests_host_shared_libs_zip)
...
$(general_tests_zip) : PRIVATE_general_tests_test_configs_zip := $(general_tests_test_configs_zip)
$(general_tests_zip) : PRIVATE_general_tests_jars_zip := $(general_tests_jars_zip)
...
$(general_tests_zip) : $(COMPATIBILITY.general-tests.FILES) $(general_tests_tools) $(my_host_shared_lib_for_general_tests) $(SOONG_ZIP)
	...
	grep -e .*.jar$$ $(PRIVATE_INTERMEDIATES_DIR)/host.list > $(PRIVATE_INTERMEDIATES_DIR)/host-jars.list || true
	grep -e .*.jar$$ $(PRIVATE_INTERMEDIATES_DIR)/target.list > $(PRIVATE_INTERMEDIATES_DIR)/target-jars.list || true
	...
	grep $(TARGET_OUT_TESTCASES) $(PRIVATE_INTERMEDIATES_DIR)/shared-libs.list > $(PRIVATE_INTERMEDIATES_DIR)/target-shared-libs.list || true
	...
	$(SOONG_ZIP) -d -o $(PRIVATE_general_tests_test_configs_zip) \
	...
	$(SOONG_ZIP) -d -o $(PRIVATE_general_tests_jars_zip) \
	  -P host -C $(PRIVATE_INTERMEDIATES_DIR) -D $(PRIVATE_INTERMEDIATES_DIR)/tools \
	  -P host -C $(HOST_OUT) -l $(PRIVATE_INTERMEDIATES_DIR)/host-jars.list \
	  -P target -C $(PRODUCT_OUT) -l $(PRIVATE_INTERMEDIATES_DIR)/target-jars.list
	$(SOONG_ZIP) -d -o $(PRIVATE_general_host_shared_libs_zip) \
	  -P host -C $(HOST_OUT) -l $(PRIVATE_INTERMEDIATES_DIR)/host-shared-libs.list \
	  -P target -C $(PRODUCT_OUT) -l $(PRIVATE_INTERMEDIATES_DIR)/target-shared-libs.list
...
$(call dist-for-goals, general-tests, $(general_tests_zip) $(general_tests_list_zip) $(general_tests_test_configs_zip) $(general_tests_jars_zip) $(general_tests_host_shared_libs_zip))

intermediates_dir :=
general_tests_tools :=
general_tests_zip :=
general_tests_list_zip :=
general_tests_test_configs_zip :=
general_tests_jars_zip :=
