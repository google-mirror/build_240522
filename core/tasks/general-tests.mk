...
$(foreach m,$(COMPATIBILITY.general-tests.HOST_SHARED_LIBRARY.FILES),\
  $(if $(filter $(m),$(COMPATIBILITY.device-tests.HOST_SHARED_LIBRARY.FILES)),\
		$(eval existing_target := $(word 2,$(subst :, ,$(m))))\
		$(eval my_host_shared_lib_for_general_tests += $(existing_target)),\
    $(eval my_general_tests_shared_lib_files += $(m))))
