...
device-tests-test-configs-zip := $(PRODUCT_OUT)/device-tests-test-configs.zip
# Create an artifact to include all jar files in device-tests.
device-tests-jars-zip := $(PRODUCT_OUT)/device-tests-jars.zip
my_host_shared_lib_for_device_tests := $(call copy-many-files,$(COMPATIBILITY.device-tests.HOST_SHARED_LIBRARY.FILES))
$(device-tests-zip) : .KATI_IMPLICIT_OUTPUTS := $(device-tests-list-zip) $(device-tests-test-configs-zip) $(device-tests-jars-zip)
...
$(device-tests-zip) : $(COMPATIBILITY.device-tests.FILES) $(my_host_shared_lib_for_device_tests) $(SOONG_ZIP)
	...
	grep -e .*.jar$$ $@-host.list > $@-host-jars.list || true
	$(hide) for shared_lib in $(PRIVATE_HOST_SHARED_LIBS); do \
	  echo $$shared_lib >> $@-host.list; \
	done
	grep $(TARGET_OUT_TESTCASES) $@.list > $@-target.list || true
	grep -e .*.config$$ $@-target.list > $@-target-test-configs.list || true
	grep -e .*.jar$$ $@-target.list > $@-target-jars.list || true
	$(hide) $(SOONG_ZIP) -d -o $@ -P host -C $(HOST_OUT) -l $@-host.list -P target -C $(PRODUCT_OUT) -l $@-target.list
	$(hide) $(SOONG_ZIP) -d -o $(device-tests-test-configs-zip) \
	  -P host -C $(HOST_OUT) -l $@-host-test-configs.list \
	  -P target -C $(PRODUCT_OUT) -l $@-target-test-configs.list
	$(hide) $(SOONG_ZIP) -d -o $(device-tests-jars-zip) \
	  -P host -C $(HOST_OUT) -l $@-host-jars.list \
	  -P target -C $(PRODUCT_OUT) -l $@-target-jars.list
	...
	rm -f $@.list $@-host.list $@-target.list \
	  $@-host-test-configs.list $@-host-jars.list \
	  $@-target-test-configs.list $@-target-jars.list $(PRIVATE_device_tests_list)

device-tests: $(device-tests-zip)
$(call dist-for-goals, device-tests, $(device-tests-zip) $(device-tests-list-zip) $(device-tests-test-configs-zip) $(device-tests-jars-zip))
