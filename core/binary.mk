...
dbus_service_config_full_path := $(addprefix $(LOCAL_PATH)/,$(LOCAL_DBUS_BINDING_SERVICE_CONFIG))

dbus_header_prefix := $(generated_sources_dir)/dbus-bindings/$(strip $(LOCAL_DBUS_BINDING_HEADER_PREFIX))
dbus_adaptor_header := $(dbus_header_prefix)adaptors.h
dbus_proxy_header := $(dbus_header_prefix)proxies.h
dbus_method_name_header := $(dbus_header_prefix)method_names.h

# TODO(wiley) I'm sure I'm not supposed to touch LOCAL_REQUIRED_MODULES here
LOCAL_REQUIRED_MODULES += dbus-binding-generator
dbus_generator_path := $(HOST_OUT_EXECUTABLES)/dbus-binding-generator
...
$(dbus_adaptor_header) : $(dbus_generated_source_dependencies)
	echo "Generating DBus bindings"
	mkdir -p $(dir $(dbus_adaptor_header))
	$(dbus_generator_path) \
	    --service-config=$(dbus_service_config_full_path) \
	    --adaptor=$(dbus_adaptor_header) \
	    --proxy=$(dbus_proxy_header) \
	    --method-names=$(dbus_method_name_header) \
	    $(dbus_if_sources_full_path)
# These are actually generated by the above recipe, but we'd like to make the
# dependency chain complete.
$(dbus_proxy_header) : $(dbus_adaptor_header)
$(dbus_method_name_header) : $(dbus_adaptor_header)

my_generated_sources += $(dbus_adaptor_header) $(dbus_proxy_header) $(dbus_method_name_header)
