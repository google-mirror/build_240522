...
dbus_header_dependencies := \
    $(dbus_service_config_full_path) \
    $(DBUS_GENERATOR)

ifdef LOCAL_DBUS_PROXY_PREFIX
dbus_header_dir := $(dbus_header_prefix)/include/$(LOCAL_DBUS_PROXY_PREFIX)
dbus_headers := dbus-proxies.h
else
dbus_header_dir := $(dbus_header_prefix)
...
$(dbus_generated_headers) : PRIVATE_DBUS_MODULE := $(LOCAL_MODULE)
$(dbus_generated_headers) : PRIVATE_DBUS_SERVICE_CONFIG := $(dbus_service_config_full_path)

ifdef LOCAL_DBUS_PROXY_PREFIX
$(dbus_generated_headers) : PRIVATE_DBUS_INTERFACE_DEFINITIONS := $(dbus_if_sources_full_path)
$(dbus_generated_headers) : $(dbus_header_dependencies) $(dbus_if_sources_full_path)
...
# Each adaptor header depends on a corresponding .dbus.xml file at some
# arbitrary location we learn at build time.
define DBUS_ADAPTOR_BUILD_TEMPLATE
$(dbus_header_dir)/$(patsubst %.dbus.xml,%.h,$(notdir $(1))) : $(1) $(dbus_header_dependencies)
	$$(generate-dbus-adaptors)
endef
$(foreach input,$(dbus_if_sources_full_path),$(eval \
    $(call DBUS_ADAPTOR_BUILD_TEMPLATE,$(input))))
