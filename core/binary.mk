...
ifeq ($(strip $(LOCAL_CXX_STL)),default)
    ...
    my_cxx_stl := $(firstword $(subst _, ,$(strip $(LOCAL_CXX_STL))))
    my_stl_type := $(or $(word 2,$(subst _, ,$(strip $(LOCAL_CXX_STL)))),shared)
endif

ifeq ($(my_cxx_stl),libc++)
    my_cflags += -D_USING_LIBCXX
    my_c_includes += external/libcxx/include
    ifeq ($(my_stl_type),shared)
        my_shared_libraries += libc++
    else ifeq ($(my_stl_type),static)
        my_static_libraries += libc++
    else
        $(error my_stl_type must be "static" or "shared")
    endif
    ...
    ifeq ($(my_stl_type),shared)
        ...
        my_static_libraries += libstdc++ libstlport
    else
        $(error my_stl_type must be "static" or "shared")
    endif
