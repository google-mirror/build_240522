...
$(test_mappings_tarball) : PRIVATE_test_mapping_list := $(file <$(OUT_DIR)/.module_paths/TEST_MAPPING.list)

$(test_mappings_tarball) : $(intermediates)
	@echo "Building artifact to include TEST_MAPPING files."
	$(eval test_mappings_tarball := $(intermediates)/test_mapping.tar.bz2)
	$(eval test_mapping_dir := $(intermediates)/test_mappings)
	$(hide) rm -rf $(test_mapping_dir)
	$(hide) mkdir -p $(test_mapping_dir)
	$(foreach f, $(PRIVATE_test_mapping_list),\
	  $(eval target_dir := $(test_mapping_dir)/$(dir $(f))) \
		$(shell mkdir -p $(target_dir)) \
		$(shell cp $(f) $(target_dir)))
	$(hide) cd $(intermediates) && tar -jcvf test_mappings.tar.bz2 -C test_mappings .
