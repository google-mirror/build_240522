...
ifdef LOCAL_ENFORCE_USES_LIBRARIES
  ...
  $(my_enforced_uses_libraries): $(BUILD_SYSTEM)/verify_uses_libraries.sh $(SOONG_HOST_OUT_EXECUTABLES)/aapt
  $(my_enforced_uses_libraries): $(my_prebuilt_src_file)
	@echo Verifying uses-libraries: $<
	aapt_binary=$(SOONG_HOST_OUT_EXECUTABLES)/aapt \
	  uses_library_names="$(strip $(LOCAL_USES_LIBRARIES))" \
	  optional_uses_library_names="$(strip $(LOCAL_OPTIONAL_USES_LIBRARIES))" \
