...
ifeq ($(LOCAL_MODULE_MAKEFILE),$(SOONG_ANDROID_MK))
  ...
  my_visib_local_path := $(call clean-path,$(LOCAL_PATH))/
  $(foreach dep,$($(link_type).DEPS),\
    $(if $($(dep).VISIBILITY),\
      $(if $(filter $($(dep).VISIBILITY),$(my_visib_local_path)),,\
        $(eval _other := //$$($$(dep).MAKEFILE):$$(word 6,$$(subst :,$$(space),$$(dep))))\
        $(if $(filter false,$(BUILD_BROKEN_MAKE_VISIBILITY)),\
          $(call pretty-error,depends on $(_other) which is not visible to this module) \
