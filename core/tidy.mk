...
# Returns 2nd word of $(1) if $(LOCAL_PATH) contains 1st word of $(1).
define find_default_local_tidy_check2
$(if $(findstring //$(word 1,$(1)),//$(LOCAL_PATH)/),$(word 2,$(1)))
endef

# Returns 2nd part of $(1) if $(LOCAL_PATH) contains 1st part of $(1).
define find_default_local_tidy_check
$(call find_default_local_tidy_check2,$(subst :,$(space),$(1)))
endef

# Returns concatenated tidy check patterns from the
# DEFAULT_GLOBAL_TIDY_CHECKS and all matched patterns
# in DEFAULT_LOCAL_TIDY_CHECKS.
define default_global_tidy_checks
$(subst $(space),, \
  $(DEFAULT_GLOBAL_TIDY_CHECKS) \
  $(foreach pattern,$(DEFAULT_LOCAL_TIDY_CHECKS), \
    $(call find_default_local_tidy_check,$(pattern)) \
  ) \
)
endef
