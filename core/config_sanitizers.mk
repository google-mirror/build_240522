...
fsanitize := $(filter-out return_address,$(fsanitize))
fsanitize := $(filter-out indirect_branch,$(fsanitize))
...
ifneq ($(filter return_address,$(my_sanitize)),)
  ifneq ($(filter indirect_branch,$(my_sanitize)),)
    my_asflags += $(RETURN_AND_INDIRECT_SANITIZER_EXTRA_CFLAGS)
    my_cflags += $(RETURN_AND_INDIRECT_SANITIZER_EXTRA_CFLAGS)
    my_linker += $(INDIRECT_BRANCH_SANITIZER_EXTRA_LDFLAGS)
  else
    my_asflags += $(RETURN_ADDRESS_SANITIZER_EXTRA_CFLAGS)
    my_cflags += $(RETURN_ADDRESS_SANITIZER_EXTRA_CFLAGS)
  endif
else
  ifneq ($(filter indirect_branch,$(my_sanitize)),)
    my_asflags += $(INDIRECT_BRANCH_SANITIZER_EXTRA_CFLAGS)
    my_cflags += $(INDIRECT_BRANCH_SANITIZER_EXTRA_CFLAGS)
    my_linker += $(INDIRECT_BRANCH_SANITIZER_EXTRA_LDFLAGS)
  endif
endif
