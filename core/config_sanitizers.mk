...
ifneq ($(filter address,$(my_global_sanitize) $(my_sanitize)),)
  ...
  ifdef LOCAL_IS_HOST_MODULE
    ...
    my_new_shared_libraries := $($(LOCAL_2ND_ARCH_VAR_PREFIX)ADDRESS_SANITIZER_RUNTIME_LIBRARY) \
                               $(my_shared_libraries) \
                               $(ADDRESS_SANITIZER_CONFIG_EXTRA_SHARED_LIBRARIES)
    # Add asan libraries unless LOCAL_MODULE is the asan library.
    ifeq (,$(filter $(LOCAL_MODULE),$(my_new_shared_libraries)))
      my_shared_libraries := $(my_new_shared_libraries)
    endif
