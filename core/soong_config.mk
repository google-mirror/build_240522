...
add_json_val = $(eval _contents := $$(_contents)    "$$(strip $$(1))":$$(space)$$(strip $$(2))$(if $(3),,$(comma))$$(newline))
...
# Remove the trailing comma, but preserve spaces and newlines
_contents := $(subst $(newline),__SV_NL,$(subst $(space),__SV_SP,$(_contents)))
_contents := $(patsubst %$(comma)__SV_NL,%__SV_NL,$(_contents))
_contents := $(subst __SV_NL,$(newline),$(subst __SV_SP,$(space),$(_contents)))

$(file >$(SOONG_VARIABLES).tmp,$(_contents)}$(newline))
