...
# 1: Key name
# 2: Value
# 3: If empty, add a comma at the end
add_json_val = $(eval _contents := $$(_contents)$$(space)$$(space)"$$(strip $$(1))":$$(space)$$(strip $$(2))$(if $(3),,$(comma))$$(newline))
add_json_str = $(call add_json_val,$(1),"$(strip $(2))",$(3))
add_json_list = $(call add_json_val,$(1),$(call json_list,$(patsubst %,%,$(2))),$(3))
add_json_csv = $(call add_json_val,$(1),$(call csv_to_json_list,$(strip $(2))),$(3))
add_json_bool = $(call add_json_val,$(1),$(if $(strip $(2)),true,false),$(3))
...
# This must be last item! (since we need to omit the trailing comma)
$(call add_json_bool, UseGoma,                           $(filter-out false,$(USE_GOMA)),LAST_ITEM)
