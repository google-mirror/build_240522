...
# Creates a rule to symlink an architecture specfic vdex file to the shared
...
$(foreach v,$(1),$(call copy-vdex-file,$(call word-colon,2,$(v))))
endef
...
my_vdex_copy_pairs := $(call vdex-copy-many-files-shared-dest,\
  $(call prefix-copy-many-files-dest,$(DEXPREOPT_IMAGE_VDEX_BUILT_INSTALLED_$(my_suffix)),$(my_boot_image_out)))
ifeq ($(my_boot_image_arch),TARGET_ARCH)
  # Only install the vdex to the shared location for the primary architecture.
  my_copy_pairs += $(my_vdex_copy_pairs)
endif
...
ifneq (,$(my_copy_pairs))
  ...
  # Hack to let these psuedo-modules wrapped around Soong modules use LOCAL_SOONG_INSTALLED_MODULE.
  ...
  $(LOCAL_INSTALLED_MODULE): $(wordlist 2,$(words $(my_installed)),$(my_installed))
