...
ifndef LOCAL_IS_HOST_MODULE
  ...
  ifeq ($(USE_CORE_LIB_BOOTCLASSPATH),true)
    ...
    # bundling OpenJDK 8 and 9, so nothing < 8 is available. But
    # specifically, when building with OpenJDK 9 but targeting
    # LOCAL_JAVA_LANGUAGE_VERSION <= 1.8, we put OpenJDK 8's standard
    # libraries on the bootclasspath.
    # This means that such code can't (accidentally) refer to OpenJDK 9
    # specific APIs, and it means that references to existing APIs are
    # not reinterpreted in an OpenJDK 9-specific way, eg. calls to
    # subclasses of java.nio.Buffer as in http://b/70862583
    ifdef USE_OPENJDK9
      ifneq (,$(filter $(LOCAL_JAVA_LANGUAGE_VERSION), 1.6 1.7 1.8))
        ...
        full_java_bootclasspath_libs += ${ANDROID_JAVA8_HOME}/lib/tools.jar
      endif
    endif
