...
ifneq ($(LOCAL_JAVA_RESOURCE_FILES),)
  ...
  java_resource_file_groups += $(strip $(foreach res,$(LOCAL_JAVA_RESOURCE_FILES), \
    ...
    $(if $(filter /%,$(_base) $(_file)),$(error $(LOCAL_MODULE_MAKEFILE): LOCAL_JAVA_RESOURCE_FILES may not include absolute paths: $(_base) $(_file))) \
    $(patsubst %/,%,$(_base))::$(_file)))

#$(warning $(LOCAL_MODULE) LOCAL_JAVA_RESOURCE_FILES=$(LOCAL_JAVA_RESOURCE_FILES))
#$(warning $(LOCAL_MODULE) java_resource_file_groups=$(java_resource_file_groups))
