...
dependent module is processed. So it is necessary to communicate the information
from dependencies via dexpreopt.config files, and enforce topological order by
adding file-level dependencies from a module dexpreopt.config file to
dexpreopt.config files of its dependencies.
"""
  ...
  for sdk_ver in clc_map:
    ...
    for lib in clcs:
      ...
      if lib in uses_libs:
        ...
        if sdk_ver != 'any':
          raise SystemExit('missing dexpreopt.config for <uses-library> %s' % lib)
        clcs2[lib] = clc
