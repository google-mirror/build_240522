...
$(file >$(1),\
$(foreach v, $(shell grep -he "^[A-Z][A-Z0-9_]*$$" $(OUT_DIR)/dump-public-variables-temp.txt),\
$(v) := $(strip $($(v)))$(newline)))
endef
...
include $(board_config_mk)
  ...
  $(shell $(OUT_DIR)/soong/.bootstrap/bin/mkvars2rbc \
    <$(OUT_DIR)/rbc/make_vars_pre_board_config.mk \
    >$(OUT_DIR)/rbc/make_vars_pre_board_config.rbc)
  ifneq ($(.SHELLSTATUS),0)
    $(error Global variable converter failed: $(.SHELLSTATUS))
  endif

  $(shell $(OUT_DIR)/soong/.bootstrap/bin/mk2rbc \
  ...
  $(shell $(OUT_DIR)/soong/.bootstrap/bin/rbcrun \
    RBC_OUT="make,global" \
