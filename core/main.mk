...
# TODO(asmundak):
# `product-installed-files` and `host-installed-files` macros are called only once.
# When it happens, per-file configuration attributes (those named 
# PRODUCT.<product-makefile>.<attribute>) have been already moved to the global 
# <attribute> variable, so there is no need to use
# $(call get-product-var,$(1), FOO) because its value has been already
# hoisted to FOO. Fix the macro instead of having Starlark configuration
# separately.
