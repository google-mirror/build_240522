...
$(foreach m,$(ALL_MODULES), \
  ...
  $(if $(r), \
    $(if $(filter $(ALLOW_MISSING_DEPENDENCIES),true),,\
      $(foreach dep,$(r),\
        $(if $(ALL_MODULES.$(dep).INSTALLED)$(filter $(ALL_MODULES.$(dep).CLASS),PHONY),,\
          $(error $(ALL_MODULES.$(m).MAKEFILE): Module $(m) requires \
            $(if $(ALL_MODULES.$(dep).CLASS),uninstallable,missing) module $(dep))))) \
