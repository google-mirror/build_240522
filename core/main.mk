...
$(foreach suite,general-tests device-tests,\
  $(foreach m,$(COMPATIBILITY.$(suite).MODULES),\
    $(eval my_deps := $(call get-all-shared-libs-depes,$(m)))\
    $(foreach dep,$(my_deps),\
      $(foreach f,$(ALL_MODULES.$(dep).HOST_SHARED_LIBRARY_FILES),\
        $(if $(filter $(f),$(COMPATIBILITY.$(suite).HOST_SHARED_LIBRARY.DEPS)),,\
          ...
          $(eval COMPATIBILITY.$(suite).HOST_SHARED_LIBRARY.DEPS := \
            $$(COMPATIBILITY.$(suite).HOST_SHARED_LIBRARY.DEPS) $(f)))))))
endef
