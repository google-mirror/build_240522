...
# TODO(b/131587358): Fix dependencies on libicuuc.so and include it in the list.
APEX_MODULE_LIBS := \
  libadbconnection.so \
  libandroidicu.so \
  libandroidio.so \
...
apex_module_libs := \
  $(filter $(foreach lib,$(APEX_MODULE_LIBS),%/$(lib)), \
    $(filter-out $(foreach dir,$(APEX_LIBS_ABSENCE_CHECK_EXCLUDE), \
      ...
      $(filter $(TARGET_OUT)/lib/% $(TARGET_OUT)/lib64/%, $(product_target_FILES))))
ifdef apex_module_libs
$$(error APEX libraries found in system image (see comment for check-apex-libs-absence in \
...
$(hide) ( \
  ...
  findres=$$(find lib* \
    ...
    -print) && \
  if [ -n "$$findres" ]; then \
    echo "APEX libraries found in system image (see comment for check-apex-libs-absence in" 1>&2; \
