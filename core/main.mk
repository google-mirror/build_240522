...
$(PRODUCT_OUT)/sbom-metadata.csv: $(installed_files)
	...
	$(foreach f,$(installed_files),\
	  ...
	  $(foreach l,$(_static_libs),$(eval _all_static_libs += $l:$(strip $(sort $(ALL_MODULES.$l.PATH))):$(strip $(sort $(ALL_MODULES.$l.SOONG_MODULE_TYPE))))) \
	  $(foreach l,$(_whole_static_libs),$(eval _all_static_libs += $l:$(strip $(sort $(ALL_MODULES.$l.PATH))):$(strip $(sort $(ALL_MODULES.$l.SOONG_MODULE_TYPE))))) \
	  @echo /$(_path_on_device)$(comma)$(_module_path)$(comma)$(_soong_module_type)$(comma)$(_is_prebuilt_make_module)$(comma)$(_product_copy_files)$(comma)$(_kernel_module_copy_files)$(comma)$(_is_platform_generated)$(comma)$(_build_output_path)$(comma)$(_static_libs)$(comma)$(_whole_static_libs)$(comma) >> $@ $(newline) \
	  $(if $(_post_installed_dexpreopt_zip), \
	  for i in $$(zipinfo -1 $(_post_installed_dexpreopt_zip)); do echo /$$i$(comma)$(_module_path)$(comma)$(_soong_module_type)$(comma)$(_is_prebuilt_make_module)$(comma)$(_product_copy_files)$(comma)$(_kernel_module_copy_files)$(comma)$(_is_platform_generated)$(comma)$(PRODUCT_OUT)/$$i$(comma)$(_static_libs)$(comma)$(_whole_static_libs)$(comma) >> $@ ; done $(newline) \
	  ) \
	)
	$(foreach l,$(sort $(_all_static_libs)), \
	  ...
	  @echo $(_lib_stem).a$(comma)$(_module_path)$(comma)$(_soong_module_type)$(comma)$(comma)$(comma)$(comma)$(comma)$(comma)$(comma)$(comma)$(_is_static_lib) >> $@ $(newline) \
	)
