...
$(PRODUCT_OUT)/sbom-metadata.csv: $(installed_files)
	rm -f $@
	@echo installed_file$(comma)module_path$(comma)soong_module_type$(comma)product_copy_files$(comma)kernel_module_copy_files >> $@
	$(foreach f,$(installed_files),\
	  $(eval _path_on_device := $(patsubst $(PRODUCT_OUT)/%,%,$f)) \
	  @echo /$(_path_on_device)$(comma)$(strip $(sort $(ALL_MODULES.$(ALL_INSTALLED_FILES.$f).PATH)))$(comma)$(strip $(sort $(ALL_MODULES.$(ALL_INSTALLED_FILES.$f).SOONG_MODULE_TYPE)))$(comma)$(sort $(filter %$(_path_on_device),$(PRODUCT_COPY_FILES)))$(comma)$(sort $(filter %$(_path_on_device),$(KERNEL_MODULE_COPY_FILES))) >> $@ $(newline) \
	)
