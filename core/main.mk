...
$(eval $$(1).DEPS += $(filter-out $($(1).DEPS),$($(2).DEPS)))\
$(eval $$(1).ADD_STATIC_DEPS += $(filter-out $($(1).ADD_STATIC_DEPS),$($(2).STATIC_DEPS)))\
  ...
  $(if $(filter java:vendor,$($(lt).TYPE)),\
    $(foreach d,$($(lt).STATIC_DEPS),\
      $(if $(filter-out java:vendor,$($(d)).TYPE),\
        ...
        $(eval $$(lt).DEPS := $(filter-out $($(lt).STATIC_DEPS),$($(lt).DEPS)))\
        $(foreach d,$($(lt).DEPS),\
          $(if $($(d).UNINSTALLABLE),\
            $(call link-type-error,$(lt),$(d),$($(d).TYPE))))))
