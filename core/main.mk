...
# (b/129006418).
DISABLE_APEX_FILES_ABSENCE_CHECK ?=
...
# Used in `check-apex-files-absence` below.
# $(1): List of files installed on product.
# $(2): List of files whose absence is checked among installed files.
# $(3): List of path in the system image which are excluded from the checks.
# $(4): List of paths in the system image where to check for the absence of
#       files listed in $(2).
define generic-check-apex-files-absence
  $(call maybe-print-list-and-error, \
    ...
    APEX files found in $(1) (see comment for check-apex-files-absence in \
    build/make/core/main.mk for details))
endef

# Generic APEX file absence check on disk.
# Used in `check-apex-files-absence-on-disk` below.
# $(1): List of files whose absence is checked among installed files.
# $(2): List of path in the system image which are excluded from the checks.
# $(3): List of paths in the system image where to check for the absence of
#       files listed in $(1).
define generic-check-apex-files-absence-on-disk
  ( \
    cd $(TARGET_OUT) && \
    findres=$$(find $(3) \
    ...
    if [ -n "$$findres" ]; then \
      ...
      echo "check-apex-files-absence and check-apex-files-absence-on-disk in" 1>&2; \
...
ifdef DISABLE_APEX_FILES_ABSENCE_CHECK
  check-apex-files-absence :=
  check-apex-files-absence-on-disk :=
else
  ...
  define check-apex-files-absence
    $(call generic-check-apex-files-absence, \
  ...
  # DISABLE_APEX_FILES_ABSENCE_CHECK environment variable.
  #
  # Likewise, we check for the absence of APEX Java libraries (JARs).
  define check-apex-files-absence-on-disk
    $(hide) ( \
      $(call generic-check-apex-files-absence-on-disk, \
...
$(PRODUCT_OUT)/offending_artifacts.txt:
  ...
  $(call check-apex-files-absence,$(product_target_FILES))
else
