...
installed_files := $(sort $(filter-out $(PRODUCT_OUT)/apex/% $(PRODUCT_OUT)/fake_packages/%,$(filter $(PRODUCT_OUT)/%,$(modules_to_install))))
$(PRODUCT_OUT)/sbom-metadata.csv: FORCE $(installed_files)
	rm -f $@
	...
	$(foreach f,$(installed_files),\
	  ...
	  $(eval _is_system_linker_config := $(if $(findstring $f,$(SYSTEM_LINKER_CONFIG)),Y)) \
	  $(eval _is_platform_generated := $(_is_build_prop)$(_is_notice_file)$(_is_dexpreopt_image_profile)$(_is_product_system_other_avbkey)$(_is_event_log_tags_file)$(_is_system_other_odex_marker)$(_is_kernel_modules_blocklist)$(_is_fsverity_build_manifest_apk)$(_is_system_linker_config)) \
...
$(call dist-for-goals,droid,$(PRODUCT_OUT)/sbom.spdx)
