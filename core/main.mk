...
.PHONY: make-metadata
make-metadata: $(SOONG_OUT_DIR)/metadata/$(TARGET_PRODUCT)/make-metadata.csv make-modules
...
.PHONY: make-modules
make-modules: $(SOONG_OUT_DIR)/metadata/$(TARGET_PRODUCT)/make-modules.csv
$(SOONG_OUT_DIR)/metadata/$(TARGET_PRODUCT)/make-modules.csv:
	rm -f $@
	echo 'name,module_path,module_type' >> $@
	$(foreach m,$(ALL_MODULES), \
	  $(eval _module_name := $m) \
	  $(eval _module_path := $(strip $(sort $(ALL_MODULES.$(_module_name).PATH)))) \
	  $(eval _make_module_type := $(ALL_MODULES.$(_module_name).MAKE_MODULE_TYPE)) \
	  $(eval _soong_module_type := $(strip $(sort $(ALL_MODULES.$(_module_name).SOONG_MODULE_TYPE)))) \
	  $(if $(_soong_module_type),, \
		echo '$(_module_name),$(_module_path),$(_make_module_type)' >> $@; \
	  ) \
	)
