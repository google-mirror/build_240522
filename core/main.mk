...
$(PRODUCT_OUT)/sbom-metadata.csv: $(installed_files)
	rm -f $@
	@echo installed_file$(comma)module_path$(comma)soong_module_type$(comma)product_copy_files$(comma)kernel_module_copy_files$(comma)is_platform_generated >> $@
	$(foreach f,$(installed_files),\
	  $(eval _path_on_device := $(patsubst $(PRODUCT_OUT)/%,%,$f)) \
	  $(eval _module_path := $(strip $(sort $(ALL_MODULES.$(ALL_INSTALLED_FILES.$f).PATH)))) \
	  $(eval _soong_module_type := $(strip $(sort $(ALL_MODULES.$(ALL_INSTALLED_FILES.$f).SOONG_MODULE_TYPE)))) \
	  $(eval PRODUCT_COPY_FILES_WITHOUT_OWNER := $(foreach pcf,$(PRODUCT_COPY_FILES),$(call word-colon,1,$(pcf)):$(call word-colon,2,$(pcf)))) \
	  ...
	  @echo /$(_path_on_device)$(comma)$(_module_path)$(comma)$(_soong_module_type)$(comma)$(_product_copy_files)$(comma)$(_kernel_module_copy_files)$(comma)$(_is_platform_generated) >> $@ $(newline) \
	)
