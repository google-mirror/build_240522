...
license_kinds:=//legacy:by_exception_only
endif
...
ifeq (true,$(strip $(LOCAL_MODULE_IS_CONTAINER)))
is_container:=true
else ifeq (false, $(strip $(LOCAL_MODULE_IS_CONTAINER)))
is_container:=false
else ifneq (,$(strip $(filter %.zip %.tar %.tgz %.tar.gz %.apk %.img %.srcjar %.srcszip %.jar %.apex, $(LOCAL_BUILT_MODULE))))
is_container:=true
else
is_container:=false
endif
...
ALL_MODULES.$(my_register_name).LICENSE_PACKAGE_NAME := $(strip $(ALL_MODULES.$(my_register_name).LICENSE_PACKAGE_NAME) $(license_package_name))
