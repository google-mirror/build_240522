...
$(LOCAL_INTERMEDIATE_TARGETS) : PRIVATE_MODULE_STEM:= $(my_installed_module_stem)
...
ifeq (,$(test_config))
  ...
  # Even AndroidTest.xml exist but if it's not using the correct testtype
  # for hose-side native test. Always regenerate it if LOCAL_DISABLE_AUTO_GENERATE_TEST_CONFIG
  # not set.
  ...
  ifeq (,$(filter cts, $(LOCAL_COMPATIBILITY_SUITE)))
    ifneq (true, $(LOCAL_DISABLE_AUTO_GENERATE_TEST_CONFIG))
      ifeq (true,$(is_native))
        ifeq (true, $(LOCAL_IS_HOST_MODULE))
              _host_gtest_class := com.android.tradefed.testtype.HostGTest
              ifneq ($(_host_gtest_class), $(filter $(_host_gtest_class), $(_my_class_name)))
                include $(BUILD_SYSTEM)/autogen_test_config.mk
                test_config := $(autogen_test_config_file)
                autogen_test_config_file :=
              endif
