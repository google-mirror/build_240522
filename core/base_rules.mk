...
$(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
  $(eval my_compat_dist_$(suite) += \
    $(call compat-copy-pair,$(LOCAL_BUILT_MODULE),$(LOCAL_DEFAULT_TEST_MODULE))))
...
ifdef $(my_prefix)$(LOCAL_MODULE_CLASS)_$(LOCAL_MODULE)_compat_files
  ...
  $(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
    $(eval my_compat_dist_$(suite) += $(foreach f, $(LOCAL_COMPATIBILITY_SUPPORT_FILES), \
      ...
      $(foreach dir, $(call compatibility_suite_dirs,$(suite)), \
        $(call compat-copy-pair,$(s),$(dir)/$(n))))))

  ifneq (,$(test_config))
    $(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
      $(eval my_compat_dist_config_$(suite) += $(foreach dir, $(call compatibility_suite_dirs,$(suite)), \
        $(call compat-copy-pair,$(test_config),$(dir)/$(LOCAL_MODULE).config))))
  endif

  ifneq (,$(wildcard $(LOCAL_PATH)/DynamicConfig.xml))
    $(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
      $(eval my_compat_dist_config_$(suite) += $(foreach dir, $(call compatibility_suite_dirs,$(suite)), \
        $(call compat-copy-pair,$(LOCAL_PATH)/DynamicConfig.xml,$(dir)/$(LOCAL_MODULE).dynamic))))
  endif
  ...
  $(foreach extra_config, $(wildcard $(LOCAL_PATH)/$(LOCAL_MODULE)_*.config), \
    $(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
      $(eval my_compat_dist_config_$(suite) += $(foreach dir, $(call compatibility_suite_dirs,$(suite)), \
        $(call compat-copy-pair,$(extra_config),$(dir)/$(notdir $(extra_config)))))))
  endif
...
$(foreach pair, $(my_test_data_file_pairs), \
  ...
  $(foreach suite, $(LOCAL_COMPATIBILITY_SUITE), \
    $(eval my_compat_dist_$(suite) += $(foreach dir, $(call compatibility_suite_dirs,$(suite),$(arch_dir)), \
      $(call compat-copy-pair,$(src_path),$(call append-path,$(dir),$(file)))))))
endif
