...
ifneq (,$(LOCAL_FULL_TEST_CONFIG))
  ALL_MODULES.$(my_register_name).TEST_CONFIG := "$(LOCAL_FULL_TEST_CONFIG)"
else ifneq (,$(LOCAL_TEST_CONFIG))
  ALL_MODULES.$(my_register_name).TEST_CONFIG := "$(LOCAL_PATH)/$(LOCAL_TEST_CONFIG)"
else
  module_test_config := $(wildcard $(LOCAL_PATH)/AndroidTest.xml)
  ifeq (,$(module_test_config))
    ALL_MODULES.$(my_register_name).TEST_CONFIG :=
  else
    ALL_MODULES.$(my_register_name).TEST_CONFIG := "$(module_test_config)"
  endif
  module_test_config :=
endif
