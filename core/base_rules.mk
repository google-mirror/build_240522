...
ifeq ($(LOCAL_IS_HOST_MODULE)$(LOCAL_MODULE_CLASS),SHARED_LIBRARIES)
  ifneq ($(filter $(LOCAL_MODULE),$(addprefix lib,$(NDK_PREBUILT_SHARED_LIBRARIES))),)
    # For those who come here due to the error message below:
    # DO NOT modify NDK_PREBUILT_SHARED_LIBRARIES to avoid the error. The set of NDK libraries
    ...
    # For those who come here due to the error message below:
    # DO NOT modify VNDK_LIBRARIES or VNDK_INDIRECT_LIBRARIES in order to avoid the error.
    # They are fixed and cannot be changed. If your library needs to be shared between
    # system.img and vendor.img then define it as a VNDK-ext library. Use vndk_ext_library {...}
    ...
    # TODO(b/35410737) Provide a way to define VNDK-ext lib that does not extend existing VNDK lib
    lib_type := vndk_ext
...
# For those modules, we guess their pre_o_default_path by removing /framework, /vndk, etc.
...
ifeq ($(LOCAL_MODULE_CLASS),SHARED_LIBRARIES)
  ...
  my_module_pre_o_default_path := $(subst /framework,,$(my_module_pre_o_default_path))
...
ifeq ($(lib_type),vndk)
  ...
  my_module_default_path := $(my_module_default_path)/vndk-ext
else ifeq ($(lib_type),framework)
  my_module_default_path := $(my_module_default_path)/framework
...
ifeq ($(my_module_path),)
  ...
  ifneq ($(my_module_path),$(my_module_pre_o_default_path))
    _lib_installed_to_new_path := true
  endif
else
  ...
  ifeq ($(filter $(TARGET_OUT_DATA)%,$(my_module_path)),)
    ...
    ifeq ($(my_module_is_soong),false)
      ifneq ($(my_module_path),$(my_module_default_path))
        ...
        $(call log-warning,$(lib_type) library must be installed to \
...
ifeq ($(_lib_installed_to_new_path),true)
