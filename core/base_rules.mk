...
ifneq (true,$(LOCAL_UNINSTALLABLE_MODULE))
ifneq (,$(LOCAL_SOONG_INSTALLED_MODULE))
ifeq (,$(filter dexpreopt_bootjar.%,$(LOCAL_MODULE)))
my_use_partial_artifact := $(if $(filter-out $(addprefix $(PRODUCT_OUT)/,$(PRODUCT_EXCLUDE_IMPORTABLE_PATHS)),\
  $(filter $(addprefix $(PRODUCT_OUT)/,$(PRODUCT_IMPORTABLE_PATHS)),$(LOCAL_INSTALLED_MODULE))),true)
endif  # dexpreopt_bootjar
...
ifneq (,$(LOCAL_SOONG_INSTALLED_MODULE))
  ...
  $(LOCAL_BUILT_MODULE): PRIVATE_TARGET_PATH := $(LOCAL_SOONG_INSTALLED_MODULE)
  $(LOCAL_BUILT_MODULE): $(OUT_DIR)/stt_context
	@echo \"Install from artifact: $@\"
	$(hide) rm -f $@ && build/make/tools/source_tree_treble/import_artifact.sh --context $< --target $(PRIVATE_TARGET_PATH) --output $@
  else
