...
# Depends on installed boot.oat, boot-*.art, boot-*.oat, boot-*.vdex
...
# The rule to install boot.oat, boot-*.art, boot-*.oat, boot-*.vdex
...
$($(my_2nd_arch_prefix)DEFAULT_DEX_PREOPT_BUILT_IMAGE_FILENAME) : $(LIBART_TARGET_BOOT_DEX_FILES) $(DEX2OAT_DEPENDENCY)
	...
	@rm -f $(dir $($(PRIVATE_2ND_ARCH_VAR_PREFIX)LIBART_TARGET_BOOT_OAT_UNSTRIPPED))/*.vdex
	$(hide) ANDROID_LOG_TAGS="*:e" $(DEX2OAT) --runtime-arg -Xms$(DEX2OAT_IMAGE_XMS) \
		...
		--vdex-file=$(patsubst %.art,%.vdex,$@) \
		--vdex-location=$(patsubst %.art,%.vdex,$($(PRIVATE_2ND_ARCH_VAR_PREFIX)LIBART_BOOT_IMAGE_FILENAME)) \
