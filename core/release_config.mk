...
ifeq ($(findstring $(TARGET_RELEASE), $(_all_release_configs)),)
...
$(foreach config, $(_all_releae_configs), \
...
define declare-build-flag
    $(if $(filter-out $(_FLAG_PARTITIONS), $(strip $(1))), \
        $(error declare-build-flag: invalid partition: $(strip $(1))) \
    )
    $(if $(filter-out RELEASE_%, $(strip $(2))), \
        $(error declare-build-flag: Release flag names must start with RELEASE_: $(strip $(2))) \
    )
    $(if $(filter-out true false, $(strip $(3))), \
        $(error declare-build-flag: Flag defaults must be true or false: $(strip $(3))) \
    )
    $(eval _ALL_RELEASE_FLAGS += $(strip $(2)))
	$(foreach partition, $(strip $(1)), \
		$(eval _ALL_RELEASE_FLAGS.PARTITIONS.$(partition) := $(sort \
			$(_ALL_RELEASE_FLAGS.PARTITIONS.$(partition)) $(strip $(2)))) \
	)
    $(eval _ALL_RELEASE_FLAGS.$(strip $(2)).PARTITIONS := $(strip $(1)))
...
flag_declaration_files := build/make/release/flags.mk \
    $(if $(wildcard vendor/google/build/flags.mk), \
        vendor/google/build/flags.mk, \
        $(sort \
            $(wildcard device/*/build/flags.mk) \
            $(wildcard device/*/*/build/flags.mk) \
            $(wildcard vendor/*/build/flags.mk) \
            $(wildcard vendor/*/*/build/flags.mk) \
        ) \
    )
...
define set-build-flag
    ...
    $(if $(filter-out true false, $(strip $(2))), \
        $(error set-build-flag: Flag values must be true or false: $(strip $(2))) \
    )
