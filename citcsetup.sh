...
function repo {
  if [[ "${PWD}" == /google/cog/* ]]; then
    echo "repo command is disallowed within Citc workspaces."
    return 1
  fi
  ${ORIG_REPO_PATH} "$@"
}
...
function _create_out_symlink() {
  ...
  if [ -d "${OUT_DIR}" ]; then
    echo "Output directory ${OUT_DIR} cannot be present in a Citc workspace."
...
# executed by RBE on citc. This file should be sourced once per checkout of
# Android code.
